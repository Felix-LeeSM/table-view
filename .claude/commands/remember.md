---
description: 대화에서 배운 것·결정을 메모리 팔레스의 적절한 위치에 type 매트릭스 기준으로 기록. reframe / 부분 / 미이동 옵션 포함.
---

# /remember

인자(`$ARGUMENTS`) + 최근 대화 맥락을 분석해, **8 type 매트릭스** 중 가장 적합한 한 곳에 저장.
...
6. **인덱스 자동 갱신** — `bash scripts/regenerate-indexes.sh` 호출. `memory/index/by-task.md` + `memory/index/by-surface.md` 자동 재생성.
...
surface:  # 선택 — by-surface index input. 코드 영역 (예: src/stores, src-tauri/src/db/postgres)
task:  # 선택 — by-task index input. 작업 의도 키워드 (예: bug-fix, refactor, grill)
---
```

## 인지 layer 가이드 (god file 만 우선 적용)

- `trigger.layer: hook` 룰 — `scripts/check-<rule>.sh` 작성 + `.claude/settings.json` PostToolUse 등록 (god file 첫 사례).
- `trigger.layer: agent-prompt` 룰 — 해당 agent definition (`.claude/agents/*.md`) 의 시스템 prompt 에 인용. 신규 agent type 필요시 사용자 질의.
- `trigger.layer: index` 룰 — by-task / by-surface 자동 등록 — 본 skill 의 단계 6.

R2 (전면 자동 derive) 는 sprint-386 의 deferred work. 본 sprint 에서는 god file 만 layer enforce, 다른 룰은 frontmatter 만 기록.
...
- auto-memory 와 중복 저장 금지 — repo 가 source of truth. auto-memory 에 같은 항목 있으면 삭제 후 MEMORY.md 인덱스 갱신.

## 관련

- `/split-memory` — 200줄 초과 시 분할
- `memory/memory.md` — 팔레스 입구 (30초/5분 path)
- `scripts/regenerate-indexes.sh` — index 자동 갱신
- `docs/sprints/sprint-386/contract.md` — 본 skill 갱신의 sprint context
