...
Verdict label 필수: green → `gh pr edit <N> --add-label review:approved --remove-label review:changes-requested`, red → `gh pr edit <N> --add-label review:changes-requested`. reviewer write는 scorecard comment + verdict label + non-blocking 발견의 `gh issue create` 뿐 (`review:approved`가 `review-gate` required check pass 조건).
