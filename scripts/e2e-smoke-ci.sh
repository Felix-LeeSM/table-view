...
if [[ -n "$SPEC_TO_RUN" ]]; then
  ...
  run_wdio "$BASE_DATA_DIR/mssql-schema-filter" "e2e/smoke/mssql-schema-filter.spec.ts"
  ...
  run_wdio "$BASE_DATA_DIR/duckdb-schema-filter" "e2e/smoke/duckdb-schema-filter.spec.ts"
  ...
  run_wdio "$BASE_DATA_DIR/redis-key-detail-panel" "e2e/smoke/redis-key-detail-panel.spec.ts"
