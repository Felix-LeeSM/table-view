...
라운드 정의는 이 파일의 `round_events()` jq 함수 한 곳에만 있다. 자세히는
그 함수 위 주석 참고 (#1968 이 정의를 교체하려는 중이다).
EOF
}
...
# ── 라운드 정의 SOT ──────────────────────────────────────────────────────
# 이 저장소에서 "리뷰 라운드" 의 정의는 여기 하나다. 라운드를 세는 다른 코드를
# 만들지 말고 이 함수를 통과시켜라.
#
#   comments  현행 프록시. PR 코멘트 1건 = 1라운드.
#             `.github/workflows/review-gate.yml` 의 `Stop at review round 3`
#             이 쓰는 것과 같은 값 (`github.event.pull_request.comments`).
#             구현자 응답 / 세션 공지 / "닫는다" 코멘트까지 라운드로 센다.
#
...
#
# #1968 이 착지하면 기본값을 head-oid 로 바꾸는 한 줄이 전부여야 한다.
# ─────────────────────────────────────────────────────────────────────────
