...
#     Apple 에서 53,006 — 적용본보다 최대 +1.76%. sql 은 GNU 기준 -5.8%
#     (wasm-opt -Oz 가 raw 는 줄이면서 압축률은 떨어뜨린다: raw 308,615 →
#     280,833, gzip 96,212 → 102,131).
#   - 같은 소스 재빌드: mongo 는 체크인본과 byte 동일, sql 은 raw 34 byte /
#     gzip 20 byte 차 (+0.02%).
#
# 전부 합쳐 3% 안쪽이라 앞 3%p 가 환경 변동을 흡수하고 남는 ~17%p 가 기능
# 증가분이다. 이 게이트는 크기를 동결하는 장치가 아니라 계단식 증가(무거운
# 의존성 추가)를 잡는 장치다. 지웠던 옛 예산(`git show
# 6cced3ab^:scripts/check-wasm-size.sh`)은 양쪽 극단이었다 — SQL 200 KiB
# (204,800)는 기준값의 2.0배라 크기가 두 배가 되기 전엔 안 울리고, Mongo 53 KiB
# (54,272)는 기준값 위로 2.9% · 위 변동폭 최대치(53,006) 위로 2.4% 뿐이었다.
# 코드가 안 변해도 측정 환경이 바뀌면 울릴 수 있는 예산이다.
#
