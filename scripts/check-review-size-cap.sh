...
#   2,995 · 4,689 · 4,467 자였다. 하한은 그 최댓값 4,689 이고, 8,000 은 그 위로
#   약 70% 여유다. 동시에 그날 최댓값(PR body 7,619 · scorecard 6,060) 바로
#   위라 — 오늘 형태의 산출물은 안 죽이고 그보다 늘어나는 순간 문다. 재작성 원문과
#   실측 명령은 issue #2507 이 갖는다.
#
#   **표면이 둘인데 값은 하나다.** PR body 쪽 하한이 더 높게 관측되지는 않았지만
#   (재작성 4,467 < 4,689), red 를 푸는 값은 그쪽이 훨씬 비싸다 — 아래 「red 가
#   풀리는 법」대로 PR body 는 새 commit 이 있어야 풀리고 scorecard 는 re-run 이면
#   풀린다. 그 비대칭을 여유로 흡수하는 쪽을 골랐고, 표면별로 나누면 이 스크립트의
#   인자 계약과 호출자 둘이 같이 늘어난다.
#
