...
# 이 파일의 `exit 2` 가드는 저마다 스위트에 자기 케이스가 있다. 가드를 더하면 그
# 가드만 떨어뜨리는 케이스를 같이 넣어라 — 다른 가드가 대신 잡아 주는 가드는 있으나
# 마나다. 확인은 그 가드의 조건을 `if false` 로 바꾸고 스위트를 돌려 보면 된다.
# 권한이나 빈 파일 상태도 픽스처로 만들 수 있다 (`chmod 000`, 0바이트 `.yml`).
if [ "$awk_rc" -ne 0 ]; then
