...
# ponytail: 아래 두 가드는 픽스처로 못 만드는 상태를 받는다 — 스위트에서 각각을
# 무력화해도 전부 green 이다 (다른 가드가 같은 픽스처를 대신 잡는다). 나머지 여섯
# 가드는 자기만 잡는 케이스가 스위트에 있다. 여기 남겨 두는 이유는 awk 가 신호로
# 죽거나(OOM·SIGKILL) 출력이 잘리는 것이 파일 내용으로 재현되지 않을 뿐 실재하기
# 때문이고, 회귀를 잡는 것은 이 줄이 아니라 아래 파일 수 대조다.
if [ "$awk_rc" -ne 0 ]; then
