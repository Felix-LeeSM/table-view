...
# `docs/` 는 뺐다: 사람이 읽는 표면이고, 넣고 재 보면 자리가 쏟아진다 — 대부분이
# `docs/README.md` 의 검색 레시피 파이프라인이다 (2026-08-16 실측 36자리).
#
...
#     제거 → 첫 공백까지가 낱말
#
#   위반 3 (정규화한 스트림을 원문 조각으로 건다)
#     논리 줄이 `tr -s '[:space:]' ' '` 로 스트림을 접는데, 그 `tr` 뒤에 선 `grep`
#     중 하나라도 조각 자리(그 `grep` 부터 다음 파이프 전까지)에 `"$` 가 없다.
#     `tr` 은 탭·개행·연속 공백을 한 칸으로 **치환**하므로 원문 그대로의 조각은
#     **오타가 없어도 0** 을 낸다 — 그 0 이 「커밋 메시지에 없다」로 읽히면 못 고치는
#     자리가 못 고치는 노드에게 간다. 이 유형의 SOT 는
#     memory/workflow/review/memory.md 「행동 계약」이고, 처방은 조각을 같은 `tr` 에
#     먼저 통과시킨 변수로 받는 것이다 (`NEEDLE=`, issue #2409). 조각을 짧게 줄이는
#     것은 처방이 아니다 — 짧을수록 무관한 PR 에 걸린다.
#
...
#
# 알려진 천장 — 아래 불릿이 그 목록이다. 넘으려면 오탐을 다시 재야 하므로 넘기지
# 않고 적어 둔다. 수를 세지 않는 이유는 불릿이 늘 때 그 수가 거짓이 되어서다
# (memory/workflow/documentation/memory.md 「개수 서술 대신 자리를 나열한다」).
...
#     다시 조용해진다.
#   - 위반 3 은 `[:space:]` **뒤에** 선 `grep` 만 본다. 앞에 선 것은 접기 전 스트림을
#     거는 것이라 조각을 접을 이유가 없다. 그리고 조각이 공백을 안 품는 리터럴이면
#     접으나 마나 같은 값인데도 걸린다 — 지금 이 저장소에 그런 줄은 없고, 생기면
#     조각을 변수로 받는 쪽이 접힘 여부를 안 따져도 되므로 싸다.
#   - **위반 3 이 보는 것은 조각 자리에 `"$` 가 있느냐뿐이다** — 그 변수가 같은 `tr`
#     을 지났는지는 안 본다. 원문 그대로 받은 변수로 걸어도, 접기와 걸기를 논리 줄
#     둘로 쪼개도, 조각을 `grep` 아닌 도구로 걸어도 통과한다. 형태만 보는 판정이라
#     여기가 끝이다 — 그 변수가 어디서 왔는지는 셸 의미론이고, 위 「무엇을 세는가」에
#     안 적힌 성질은 판정에 안 들어간다.
#   - **조각 자리는 마스킹 안 한 원문에서 잰다** (아래 `rawneedle` 주석의 사유).
#     그래서 `grep` 이 명령 자리가 아니어도 조각 자리로 읽힌다 — 후행 주석에 든
#     `grep` 낱말이 그 형태이고, 조각이 이미 정규화된 변수여도 걸린다.
#
# 사용:
...
if ! hits="$(
	while IFS= read -r f; do
		...
		# 정규화한 스트림을 원문 조각으로 거는 자리. `tr` 뒤에 선 `grep` 마다 다음
		# 파이프 전까지를 보고, 거기 `"$` 가 없으면 조각이 리터럴이다. 마스킹한
		# 사본으로는 못 잰다 — `x` 로 덮이면 리터럴 조각과 변수 조각이 같은 모양이 된다.
		function rawneedle(line,   t, i, seg, bar) {
			if (index(line, "tr -s ") == 0) return 0
			t = index(line, "[:space:]")
			if (t == 0) return 0
			for (i = t; i <= length(line) - 3; i++) {
				if (substr(line, i, 4) != "grep") continue
				seg = substr(line, i)
				bar = index(seg, "|")
				if (bar > 0) seg = substr(seg, 1, bar - 1)
				if (index(seg, "\"$") == 0) return 1
			}
			return 0
		}
		...
		function judge(n, line,   m, i, c2, p, cut, pre, mpre, w) {
			...
			# 위반 3 — 접은 스트림은 접은 조각으로 건다 (issue #2409).
			if (rawneedle(line)) {
				report(n, "정규화한 스트림을 원문 조각으로 건다", line)
			}
...
if [ -n "$hits" ]; then
	...
	echo "::error::계약 문서의 명령 블록이 실패를 흘린다 — $(printf '%s\n' "$hits" | grep -c '^[^[:space:]]') 자리 (위 목록). 파이프 왼쪽 명령의 rc 가 필요하면 변수로 받고 가드한 뒤 \`printf '%s\\n' \"\$VAR\" | …\` 로 넘기거나 파이프를 없애라. \`ABORT\` 를 적은 자리는 0 아닌 rc 로 끝내라. \`tr -s '[:space:]' ' '\` 로 접은 스트림은 같은 \`tr\` 를 지난 조각으로 걸어라 — 조각을 짧게 줄이는 것은 처방이 아니다. 판정 규칙은 scripts/check-prompt-fail-silently.sh 헤더가 갖는다 (issue #2403 · #2409)." >&2
	exit 1
fi
