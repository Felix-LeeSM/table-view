#!/usr/bin/env bash
# Smoke tests for main-worktree source/app edit enforcement.
...
run_codex_hook_case() {
  ...
  output="$(
    printf '{"hook_event_name":"PreToolUse","tool_name":"Edit","tool_input":{"file_path":"%s"}}' "$path" |
      bash "$CODEX_HOOK" 2>&1
  )"
  ...
  if [ "$actual_decision" = "$expected_decision" ]; then
    ...
    record_fail "$name expected $expected_decision, got $actual_decision; output=$output"
  fi
}
