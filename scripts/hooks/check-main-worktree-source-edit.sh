#!/usr/bin/env bash
# Block source/app edits from the main worktree; no-op in linked worktrees.
...
deny_path() {
  local rel="$1"
  cat >&2 <<EOF
BLOCKED: source/app edit in main worktree: $rel
Main worktree is orchestration-only. Make implementation/review source edits from a linked worktree instead.
