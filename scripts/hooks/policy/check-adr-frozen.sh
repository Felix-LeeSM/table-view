#!/usr/bin/env bash
# check-adr-frozen.sh
# ADR 본문 동결 정책. docs/decisions/*/memory.md 의 frontmatter (--- 사이) 외
...
staged_adrs="$(git diff --cached --name-only --diff-filter=AM 2>/dev/null | grep -E "^docs/decisions/[^/]+/memory\.md$" || true)"
