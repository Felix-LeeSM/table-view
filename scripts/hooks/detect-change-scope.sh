#!/usr/bin/env bash
# Compatibility shim — forwards to analyze/detect-change-scope.sh. See pre-tool-use.sh for why these
...
exec bash "$(cd "$(dirname "$0")" && pwd)/analyze/detect-change-scope.sh" "$@"
