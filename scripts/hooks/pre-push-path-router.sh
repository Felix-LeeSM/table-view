#!/usr/bin/env bash
# Compatibility shim — forwards to apply/pre-push-path-router.sh. See pre-tool-use.sh for why these
...
exec bash "$(cd "$(dirname "$0")" && pwd)/apply/pre-push-path-router.sh" "$@"
