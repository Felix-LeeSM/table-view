#!/usr/bin/env bash
# Compatibility shim — forwards to policy/check-dangerous-bash.sh. See pre-tool-use.sh for why these
...
exec bash "$(cd "$(dirname "$0")" && pwd)/policy/check-dangerous-bash.sh" "$@"
