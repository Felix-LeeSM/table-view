...
if is_bash_tool; then
	stderr=""
	status=0
	stderr="$(bash "$ROOT/scripts/hooks/policy/check-dangerous-bash.sh" "$command" 2>&1 >/dev/null)" || status=$?
	if [ "$status" -ne 0 ]; then
		deny "$stderr"
		exit 0
	fi
...
policy_stderr=""
policy_status=0
policy_stderr="$(printf '%s' "$INPUT" | bash "$ROOT/scripts/hooks/apply/check-edit-policy.sh" 2>&1 >/dev/null)" || policy_status=$?
