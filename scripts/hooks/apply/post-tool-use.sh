		...
		case "$rel" in
			*.ts | *.tsx | *.rs)
				...
				jq -n --arg file "$ROOT/$rel" '{ tool_input: { file_path: $file } }' |
					CLAUDE_PROJECT_DIR="$ROOT" bash "$ROOT/scripts/hooks/policy/check-god-file.sh"
				;;
		esac
	done < "$paths_file"
}
...
if [ "$has_memory" = "1" ]; then
	run_advisory "memory-size" bash -c "cd \"$ROOT\" && bash scripts/hooks/policy/check-memory-size.sh 2>&1 | head -10"
	run_advisory "memory-structure" bash -c "cd \"$ROOT\" && bash scripts/hooks/policy/check-memory-structure.sh 2>&1 | head -10"
	run_advisory "memory-index" bash -c "cd \"$ROOT\" && bash scripts/regenerate-indexes.sh 2>&1 | tail -5"
fi

if [ "$has_adr" = "1" ]; then
	run_advisory "memory-adr" bash -c "cd \"$ROOT\" && bash scripts/hooks/policy/check-memory-adr.sh 2>&1 | head -30"
fi
...
if [ "$has_wrapper" = "1" ]; then
	run_advisory "wrapper-cap" bash -c "cd \"$ROOT\" && bash scripts/hooks/policy/check-wrapper-cap.sh 2>&1 | head -10"
fi

if [ "$has_docs" = "1" ]; then
	run_advisory "doc-size" bash -c "cd \"$ROOT\" && bash scripts/hooks/policy/check-doc-size.sh 2>&1 | head -10"
fi
