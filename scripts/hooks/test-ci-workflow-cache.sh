...
# gated automatically.
#
# This SUPERSEDES the 13-name enumeration #1817 added here on 2026-07-25. That
# loop existed to stop a future `integration-shard` split (tried and reverted:
# the `max-threads = 1` nextest groups made the slowest shard 1294s against
# 1028s undivided) from silently dropping a target, because a missing target
# lowers the coverage total rather than failing. `--test '*'` satisfies that
# intent strictly harder — there is no list left to drop from, and it also
# catches the 61 binaries the enumeration itself never named.
