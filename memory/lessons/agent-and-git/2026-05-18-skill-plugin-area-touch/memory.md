...
evaluator.md` 를 read 대상으로 가리키던 인식 — repo 의 evaluator 룰과 harness
skill 의 evaluator 룰이 같은 것으로 착각. 실제로는 *직교* — repo 룰은
`memory/workflow/review/memory.md`, skill 룰은 plugin 자체.

**재발 방지**:
- repo 가 `.claude/skills/<x>/` 의 파일을 *절대* 수정 안 함.
- 본 sprint 의 evaluator wrapper 는 `memory/workflow/review/memory.md` 만
