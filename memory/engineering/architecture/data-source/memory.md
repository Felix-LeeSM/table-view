...
TS/Rust strict profile parity 는 `tests/fixtures/data-source-profile-parity.report.json` 을
SOT 로, `src/types/dataSourceProfileParity.test.ts` 와 `src-tauri/tests/data_source_profile_parity.rs` 가 검증한다.
TS `capabilities` 와 Rust `adapter_contract` 는 strict parity field 가 아닌 runtime/support posture 이며,
소유는 `src/types/adapterConformance.ts` + Rust profile contract tests + `docs/contributor-guide/testing-and-quality.md` 다.
