...
SQLite 는 wired `SqliteAdapter` 가 네이티브 구조 DDL 을 실행 → `RelationalSchemaMutation` 선언 (#1804), Oracle 은 wired `OracleRuntimeAdapter` DDL Unsupported → 미선언 (full `OracleAdapter` 승격 #1072).
