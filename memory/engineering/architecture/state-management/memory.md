...
- `dirtyTabIds` 는 pending 편집 존재에서 파생되는 window-local signal 이다 (mount
  상태 아님, #1204). 편집은 cross-mount `dataGridEditStore`/`rawQueryGridEditStore`
  에 살므로 grid 훅은 unmount 시 marker 를 해제하지 않는다 — pending 이 비면 (commit/
  discard, mounted 중) 또는 explicit close (`removeTab`/`clearForConnection`) 에서만
  해제. Boot persisted marker 로 남기지 않는다.
- Window-local pending edit 를 폐기하는 모든 close 경로 (tab X, Cmd+W, native window
  ...
  연결/창 단위 dirty 판정 (`useConnectionHasDirtyTabs`) 은 3-source OR 다 (#1204):
  `dirtyTabIds` marker (hydration 시 누락 가능, #1091) OR `dataGridEditStore` non-empty
  pending entry OR `rawQueryGridEditStore` non-empty pending entry (둘 다 `${connId}::`
  prefix). 셋 다 pending 편집 존재 기반으로 파생되므로 inactive/unmounted 탭도 잡는다.
