...
소스코드 디렉토리에 **파일**(하위 디렉토리 제외)이 10개 이상이면 새 depth 를 추가해
기능/도메인/레이어 기준으로 그룹화한다. 하위 디렉토리 개수는 무시. `docs/` · `memory/`
등 문서 영역은 제외.

**Why:** 소스코드는 빈번히 탐색·수정하므로 파일이 한눈에 들어와야 한다. 디렉토리는
구조적 역할만 하므로 개수는 무관.
...
- [docs/decisions](../../../docs/decisions/memory.md) — historical ADR archive
