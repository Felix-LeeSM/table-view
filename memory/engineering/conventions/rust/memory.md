...
- 통합: 앱 패키지는 `src-tauri/tests/`, workspace member 는 자기 manifest 옆
  `src-tauri/<member>/tests/` (예: `src-tauri/tvw/tests/`). 둘 다
  `scripts/check-ci-test-calls.sh` 가 workflow `--test` 배선과 대조한다 — 안 부르는
  target 은 `ci-uncalled-tests.txt` 에 사유를 적어야 CI 가 green 이다 (#2336)
