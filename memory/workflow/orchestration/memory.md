...
보고에 담을 것: 라운드별 blocking 집합 변화 / 재발한 클래스와 라운드별 인스턴스
수 / 저자가 시도한 것 / 함께 정지된 PR 과 공유 파일 / 선택지(범위 축소·근본
분리·닫고 재설계).
...
- "~별로 판정" 같은 재량 항목은 판정 기준을 이슈에 박거나, 그 판단 자체를 별도
  조사 이슈로 뺀다.
- 클래스 단위로 연다. 인스턴스 10개는 이슈 10개가 아니라 이슈 1개다.

모집단 명령의 hit 수가 곧 작업 크기다 — 예측 없이 착수 전에 알 수 있다.
