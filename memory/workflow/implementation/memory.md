...
keywords: 착수 전 체크리스트, noise, narration, 전칭, fail-open, 역참조, mutation, RED, 반증 명령, 필터 검증, grep -P, 전제 뒤집기, 사용량 0
trigger:
...
| 스크립트 안 `grep -P` | 스크립트에서 rc=2 로 죽어 0건처럼 보인다. `git grep -P` 나 `-E` + POSIX 클래스를 쓴다 |
| mutation 이 대상 표기와 어긋남 | 가드 mutation 은 대상 파일의 실제 표기로 만든다. 대상이 안 쓰는 표기의 mutation 은 가장 흔한 회귀를 못 잡는다 |
| 전제를 뒤집고 결론을 방치 | 전제를 거짓으로 판정하면 그 전제로 내렸던 결론들을 같은 턴에 훑는다. 한 파일에 전제와 결론이 반대로 남는다 |
| 사용량 0 을 삭제 근거로 씀 | 지우기 전에 설계 문서에서 그 이름을 찾는다. 미착수 설계의 부품은 사용량이 전부 0 이다 |

**Why**: 처음 여섯 행은 2026-07 리뷰 라운드 폭증의 finding 공급원이었다. 전칭 ·
역참조 · fail-open 세 행은 2026-07-31 scorecard 재분석에서 blocking 의 주된
공급원으로 확인된 유형인데 처음 여섯 행이 못 덮는다 — 방법과 수치는 #2035 결정 5
가 소유한다. 마지막 다섯 행은 2026-07~08 세션에서 실측된 유형이다.
