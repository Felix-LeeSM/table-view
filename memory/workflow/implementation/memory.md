...
| 가드가 자기 청소분을 못 덮음 | 클래스 청소 + 재발 가드를 같이 넣는 PR 은 "이 가드를 base 에 걸었으면 내가 고친 N 건 중 몇 건이 걸렸겠나" 를 실행해 숫자로 낸다 |
| 인용된 줄만 고침 | finding 은 클래스로 일반화해 전수 스윕한다. 모집단은 명령 출력으로 정의한다 |
...
blocking 판정은 [review](../review/memory.md) 원칙 1 의 세 사유로 고정이고 이
표는 거기에 추가되지 않는다.
