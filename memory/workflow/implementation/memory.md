...
keywords: 착수 전 체크리스트, noise, narration, 전칭, fail-open, 역참조, mutation, RED, 반증 명령, 필터 검증, 새로 쓴 문장, 반증 가능한 명제, 모집단, 고쳐 쓴 문장
trigger:
...
| 인용된 줄만 고침 | blocking finding 은 유형으로 일반화해 전부 훑는다. 범위는 전수 명령의 출력으로 정의한다. 어느 severity 가 수정 라운드 작업인지는 [delivery](../delivery/memory.md) 「Node 별 계약」이 소유한다 |
...
| 내가 새로 쓴 문장이 쓸 때부터 거짓 | 잴 문장의 모집단은 낱말이 아니라 자리로 정한다. 내 diff 의 `+` 쪽에 있는 소스 주석 · docstring · 문서와, diff 밖에서 새로 쓰는 커밋 메시지 · PR body 가 그 자리에 든다. 새로 쓴 문장 하나하나를 반증 가능한 명제로 보고, 그 문장을 반증할 명령을 돌려서 잰다. 지울 수 있는 문장은 고쳐 쓰지 않고 지우는 것이 기본이다. 고쳐 쓴 문장도 다시 재야 하는 새 명제이기 때문이다 ([pr-artifacts](../pr-artifacts/memory.md) 「PR body」). 주석 기호나 한글로 `+` 줄을 거르는 명령(예: 이슈 #2477 본문의 명령)은 먼저 읽을 줄을 고르는 데에만 쓴다. 그 명령이 읽는 `git diff` 출력에는 커밋 메시지와 PR body 가 없으므로, 그 명령으로는 모집단을 정하지 못한다 |
...
| 전수 명령의 필터가 검증 안 됨 | exclude 절을 빼고 한 번 더 돌려 건수 차이로 그 절이 실제로 거른다는 것을 증명한다. 아무것도 안 거르면서 정밀해 보이던 필터가 있었다 |
| mutation 이 대상 표기와 어긋남 | 가드 mutation 은 대상 파일의 실제 표기로 만든다. 대상이 안 쓰는 표기의 mutation 은 가장 흔한 회귀를 못 잡는다 |

**Why**: 처음 네 행은 2026-07 리뷰 라운드 폭증의 finding 공급원이었다. 전칭 ·
역참조 · fail-open 세 행은 2026-07-31 scorecard 재분석에서 blocking 의 주된
공급원으로 확인된 유형인데 처음 네 행이 못 덮는다 (#2035 결정 5). 마지막 두
행은 2026-07~08 세션에서 실측된 유형이다. 「새로 쓴 문장」 행의 공급원은
2026-08-29 에 라운드 1 리뷰가 낸 blocking 이다 (#2579). 그 blocking 이 지목한
문장은 전칭 낱말을 쓰지 않았고 그 PR 이 스스로 쓴 문장이라서, 전칭 행에도
역참조 행에도 걸리지 않았다.
**행을 더하는 것 자체는 처방이 아니다** — blocking 판독에서 한 건도 안 걸린
다섯 행을 #2227 이 뺐다. 지운 유형이 실제로 다시 나오면 그때 다시 넣는다.
