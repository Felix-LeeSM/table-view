...
자리가 아니다. **잃는 것은 그대로 적는다 — 이슈 번호는 히스토리에서 사라진다.**
PR body 의 `Closes #<이슈>` 는 `COMMIT_MESSAGES` 를 안 타 squash body 로도 안
넘어가고([review](../review/memory.md) 「행동 계약」), 히스토리에 남는 링크는 착지
제목의 ` (#<PR>)` 하나다. **닫는 것은 종결자가 아니라 저자다** — 커밋 제목에
