...
- **외부 plugin / runtime 영역 침범**:
  ...
  - repo-owned `.claude/skills/<x>/`, `.codex/skills/<x>/` skill copy — 금지.
    `.agents/skills` 원본을 직접 참조한다.
...
- harness evaluator 와 repo `pr-reviewer` 룰 혼동. harness prompt 는 harness
  workflow 용이고, PR review 원본은 본 방이다.
...
- `memory/workflow/documentation/memory.md` — Docs 차원 원본
