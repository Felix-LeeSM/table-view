...
T5       delivery owner reflect/merge ← scorecard + CI 결과 input
```
...
| Signed commits | `scripts/check-signed-commits.sh` | pre-push |
| 회귀 (tsc / lint / clippy / test / coverage) | lefthook `pre-push` stages | pre-push |
| TDD 사이클 (code profile) | `scripts/check-tdd-cycle.sh` | pre-push |
...
**4. Documentation impact / topology**
...
|---|---|---|
| `code` | Mock + 정합성 + Scope + Docs | 공통 + TDD + Coverage threshold |
| `security` | 정합성 + Scope + Threat-model + Docs | 공통 + ADR 동결 강제 |
| `infra` | 정합성 + Scope + Platform-neutral + Docs | 공통 |
| `docs` | 정합성 + Scope + Docs | 공통 (Coverage / TDD N/A) |

**공통 자동 layer**: dangerous Bash / memory 구조 / wrapper cap / god file /
conventional commit / signed commits / pre-push stages.
...
- ✓ pre-push stages (signed commits / tsc / lint / tauri-check / cargo-deny / cargo-machete / rust-cov / ts-test / tdd-cycle)
...
3. **자율 머지 조건** (T6, delivery owner 책임):
   - 정성 4 차원 모두 ≥ 7/10
   - CI green (`gh pr checks` SUCCESS)
   - 사용자 명시 거부 없음
   - mergeable + branch policy unblocked
   → delivery owner 가 `gh pr merge --squash --delete-branch` 실행
...
- reviewer 가 commit / push / merge 수행. **금지** — reviewer 는 판단만,
  reflect/merge 는 delivery owner 책임.
