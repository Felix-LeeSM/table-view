...
Worktree: runtime wrapper 는 현재 repo/worktree root 를 계산한 뒤 worktree 사본의
`scripts/hooks/*.sh` 를 호출한다. Wrapper 정의는 tracked `.claude/settings.json`,
`.codex/hooks.json`, `.codex/hooks/*.sh` 가 소유한다.
...
- `.claude/settings.json` / `.codex/hooks.json` / `.codex/hooks/*.sh` — runtime hook wrappers
