...
- Redis 등 backend allowlist 가 실제 안전 경계인 패러다임은 frontend 분류기를 full 동기화하지 않고, 집합 둘만 mirror 해서 SQL 과 동일한 confirm 다이얼로그로 라우팅한다. 하나는 backend 가 확인 값을 요구하는 집합(`required_confirmation_key`)이고, 다른 하나는 backend 가 `RedisCommandEffect::Destructive` 로 분류하는 verb 전부다. 뒤의 집합은 #2513 이 더했다.
