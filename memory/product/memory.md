...
- **parity 축은 구문 형태가 아니라 "영향 범위 × 손실성".** 같은 "upsert" 라도: 행 단위·지정 컬럼(INSERT ON CONFLICT) = info / 테이블·컬렉션 단위 덮어쓰기($merge, $out, WHERE-less DML) = danger. 행 단위·전체 리셋(REPLACE INTO / INSERT OR REPLACE)은 이 축상 danger 가 **목표**지만 현재 `src/lib/sql/sqlSafety.ts` 에 REPLACE dispatch 가 없어 default `{kind:"other", severity:"info"}` 로 무경고 allow — danger 승격은 그 파일에 dispatch 를 더하는 일이다 (#1115 가 그 fail-open 을 실증). SOT 가 미구현 tier 를 사실로 서술하지 않도록 현 상태를 적어 둔다.
