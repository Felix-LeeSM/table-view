/**
 * Reason: AC-180-01, AC-180-02, AC-180-06 — shared overlay regression
 * suite. Exercises the threshold-controlled visibility, the Cancel callback
 * contract, the accessible-name + data-testid uniformity pin, and the
 * pointer-event hardening preserved inside the shared component.
 */
...
describe("AsyncProgressOverlay", () => {
  ...
  // Regression — the overlay's mouseDown handler must call
  // `e.preventDefault()`. Without this, drag-select on the rows underneath
  // would still start.
  ...
  // Regression — the overlay's click handler must call
  ...
  // Regression — the overlay's doubleClick handler must call
  ...
  // Regression — the overlay's contextMenu handler must call
