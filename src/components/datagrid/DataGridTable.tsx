...
const DataGridTable = forwardRef<DataGridTableHandle, DataGridTableProps>(
  function DataGridTable(
    ...
    const rowVirtualizer = useVirtualizer({
      ...
      // outrun the rendered window and flash blank rows. Kept at 24 (not the
      // proposed 40): each data row is heavy (multi-cell + pendingEdits
      // subscription), so 24 caps per-frame pre-render at ~48 extra rows —
      // enough to cover typical drag jumps without the constant-render cost 40
      // would add. Higher than SchemaTree's 8 because grid rows are shorter
      // (32px) and scrollbar drags jump farther than tree keyboard nav.
      overscan: 24,
    });
