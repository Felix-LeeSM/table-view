// AddColumnDialog test suite — AC-236-01, AC-236-03, AC-236-04,
// AC-236-09, AC-236-10.
//
...
//   #1626 — one representative case stays here.
...
describe("AddColumnDialog (Sprint 236)", () => {
  ...
  // in ./identifier.test.ts (issue #1626); embedded space is the
  ...
  // IDENTITY toggle. When on, the wire payload sets `is_identity: true`
  // on the column object so the backend's PG emitter writes `GENERATED
  // BY DEFAULT AS IDENTITY`. The dialog also force-disables NOT NULL +
  // DEFAULT inputs while identity is on (visual mirror of the backend's
  // NOT-NULL-and-drop-default semantics).
  ...
  // Purpose: audit-residual error-branch coverage for AddColumnDialog —
  // preview/commit reject must surface an inline alert while the modal
  // stays open (P4 parity with commit-success). Mirrors the
