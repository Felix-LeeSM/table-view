 ...
 *
 * `queryMode` argument removed. The Find/Aggregate toggle
 ...
 * mongosh expression can consume.
 *
 * D-04 (`docs/archives/phases/retired/phase-28-decision-log.md`) — the
 * parameter is dropped (not kept as a `"unified"` sentinel); keeping a
 * sentinel would be dead optionality with a fake degree of freedom in
 * the type signature.
 */
  ...
  return useMemo(
    () => [
      autocompletion({
        ...
        override: [
          // Admin command source runs *first* because its trigger pattern
          // (`db.runCommand({` + first key) is the narrowest. Falling
          // through to `createMongoshDbSource` on no-match keeps
          // backward-compat with the `db.<coll>.method` whitelist;
          // `createMongoCompletionSource` continues to own `$operator` /
          // quoted-key positions inside JSON bodies.
          createMongoAdminCommandSource(),
          createMongoshDbSource({
