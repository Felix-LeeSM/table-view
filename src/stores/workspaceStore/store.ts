/**
 * `useWorkspaceStore` — the zustand store instance for per-workspace state
 * keyed by `(connId, db)` (ADR 0027). Extracted from the root barrel so the
 * store instance is a leaf: `selectors.ts` reads it from here directly, and
 * the root `workspaceStore.ts` barrel re-exports it. This one-directional wiring
 * (barrel -> {store, selectors}, selectors -> store) breaks the former runtime
 * cycle where selectors imported the store back through the barrel (#1361).
 *
 * Active workspace identity is not owned here — selector hooks derive it from
