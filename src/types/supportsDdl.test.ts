...
// capability for the four schema-mutation entry points (Create Table / Alter
// Table / Create Index / Drop object). Grounds (adapter code):
...
//   - SQLite — #1804 opened the DDL SQLite runs natively (ADD/DROP COLUMN,
//     `RENAME TO`, CREATE/DROP INDEX, DROP TABLE) → the four base actions true.
...
//   - `modifyColumn` — #1804 split it off `alterTable`: every engine above runs
//     an in-place `ALTER COLUMN` except SQLite, which fixes a column's type,
//     NOT NULL and DEFAULT at CREATE TABLE time and needs a full table rebuild
//     to change one.
...
describe("supportsDdl (#1460)", () => {
  ...
  it("claims the natively-runnable DDL for SQLite but not the rebuild ones (#1804)", () => {
    expect(supportsDdl("sqlite", "createTable")).toBe(true);
    expect(supportsDdl("sqlite", "alterTable")).toBe(true);
    expect(supportsDdl("sqlite", "createIndex")).toBe(true);
    expect(supportsDdl("sqlite", "dropObject")).toBe(true);
    // #1804 — the rebuild boundary. SQLite can only declare a constraint at
    // CREATE TABLE time, and a column's type / NOT NULL / DEFAULT is fixed
    // then too, so both need the 12-step rebuild this app does not run.
    expect(supportsDdl("sqlite", "alterConstraint")).toBe(false);
    expect(supportsDdl("sqlite", "modifyColumn")).toBe(false);
  ...
  it("claims native structural DDL for DuckDB but not constraints (#1070 Stage 2)", () => {
    ...
    // #1804 — DuckDB does run a native in-place ALTER COLUMN, unlike SQLite.
    expect(supportsDdl("duckdb", "modifyColumn")).toBe(true);
