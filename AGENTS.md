...
| 한국어 산문 작성       | `.claude/output-styles/fluent-korean.md` — 조사·어미·문장 성분을 생략하지 않는 문체 기준. `.claude/settings.json` 이 기본 output style 로 걸지만 output style 은 main conversation 에만 실린다 — **spawn 된 노드에는 안 오니 직접 열어야 한다** |
...
아래 룰에서 **기계가 잡는 것은 하나뿐이다** — memory cap 의 크기 절반을 CI 가
