...
Parser WASM artifacts are checked into `src/lib/**/wasm/` so the app can build
without a local `wasm-pack` toolchain. Any change to those artifacts or their
Rust parser crates must keep `pnpm wasm:size` green: SQL parser WASM is capped
at 80 KiB gzip, and Mongo parser WASM is capped at 50 KiB gzip.
