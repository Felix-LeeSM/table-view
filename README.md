...
TablePlus처럼 빠르게 연결하고, 스키마와 컬렉션을 탐색하고, 데이터를 바로 확인·편집하는 흐름을 지향합니다. PostgreSQL과 MongoDB를 가장 깊은 1차 표면으로 다루며, MySQL/MariaDB, SQLite, DuckDB, Redis도 검증된 범위 안에서 연결, 탐색, 쿼리/값 조회 경로가 열려 있습니다.
...
- **DuckDB**: 파일 기반 `.duckdb` 연결, 로컬 CSV/Parquet/JSON/NDJSON preview/query 경로를 지원합니다. DDL/write parity는 아직 명시적으로 제한됩니다.
...
- **Redis**: Redis first slice로 DB 선택, key scan, value read, TTL 변경, guarded string write, bounded stream read를 지원합니다. Valkey, cluster, pub/sub, module 관리, consumer group 관리는 후속입니다.
...
- **MSSQL/Oracle**: 알려진 planned DBMS identity지만 현재 runtime adapter는 없습니다. 기본 dev/e2e fixture connection으로 추가되지 않습니다.

세부 쿼리/자동완성 범위는 [`docs/query-language-support.md`](./docs/query-language-support.md)에 정리되어 있습니다.
...
```bash
brew tap <tap-owner>/homebrew-table-view
