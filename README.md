...
- **Valkey**: 연결/profile, key scan/value preview, bounded Redis-compatible command query, `GET`/`HGETALL`/`XRANGE`와 bounded `SET`/`EXPIRE` smoke 경로가 있습니다. direct key mutation controls와 full Redis compatibility는 후속입니다.
...
- **Oracle**: 서비스명 기반 connection/query/catalog/edit/DDL metadata runtime, primary-key row edit, representative Safe Mode smoke, catalog-aware Oracle SQL editor assistance가 있습니다. SID, TNS alias, wallet/TLS, sequence/synonym DDL/admin, raw DDL/admin, full PL/SQL semantics는 후속입니다.
...
- PostgreSQL, MySQL/MariaDB, SQLite, DuckDB, MSSQL, Oracle은 DBMS별 SQL dialect highlighting과 bounded 테이블/컬럼 자동완성을 제공합니다.
...
MSSQL과 Oracle 컨테이너는 수동 연결, explicit fixture load, Runtime Happy Path
smoke에 사용됩니다. Fixture CLI의 기본 `all/default` target은
...
Oracle은 서비스명 기반 `XEPDB1` 경로가 기준입니다. SID/TNS alias/wallet/TLS,
raw admin, full PL/SQL semantics는 후속입니다.
...
MariaDB, MSSQL, Oracle, SQLite, DuckDB, MongoDB, Redis, Valkey,
Elasticsearch, OpenSearch runtime happy path를 검증합니다. GitHub Actions에서는
