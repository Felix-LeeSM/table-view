...
impl RdbAdapter for MysqlAdapter {
    ...
    /// Issue #1269 — capture `CONNECTION_ID()` on the connection running the
    /// browse so the grid Stop button can fire `KILL QUERY` against a long
    /// table scan. Concrete signature is `(table, schema)`; trait passes
    /// `(namespace, table)`.
    #[allow(clippy::too_many_arguments)]
