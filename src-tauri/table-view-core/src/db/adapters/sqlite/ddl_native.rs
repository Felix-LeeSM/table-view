...
//! here. `RENAME COLUMN` is native to SQLite too but has no request to carry
//! it: `ColumnChange` is `Add` / `Modify` / `Drop`, with no rename variant on
//! any engine, so no caller can reach it.
//!
