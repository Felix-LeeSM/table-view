...
mod tests {
    ...
    async fn kv_command_inners_dispatch_to_kv_adapter_contract() {
        ...
        assert_unsupported(list_kv_databases_inner(&state, "kv").await);
        assert_unsupported(scan_kv_keys_inner(&state, "kv", scan_request(), Some("scan")).await);
        assert_unsupported(get_kv_value_inner(&state, "kv", value_request(), Some("read")).await);
        assert_unsupported(
