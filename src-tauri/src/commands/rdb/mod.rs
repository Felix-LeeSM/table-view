...
//!
//! The former flat `commands/{schema,query}.rs` was split into three
...
//! copy with `commands/document/mod.rs` collapsed). This module keeps only a
//! re-export so the sub-files can keep their
//! `use super::{register_cancel_token, release_cancel_token}` pattern
//! unchanged.
//!
//! `ensure_expected_db` hoisted from
//! `schema.rs` to this module so the 11 DDL handlers can share the same
//! mismatch-probe helper as the 12 schema introspection handlers. `query.rs`
//! keeps the probe inline because its `cancel_handle` release ordering differs.
...
/// Shared mismatch probe. Issue #1087 — the caller now holds a
...
///
/// Hoisted this helper from `schema.rs` to
