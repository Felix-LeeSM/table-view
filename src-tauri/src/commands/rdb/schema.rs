...
pub async fn list_schemas(
    ...
    // Opt-in db-mismatch guard. See module doc.
...
pub async fn get_table_columns(
    ...
    table: String,
    schema: String,
    // AC-180-04: optional cancel-token id. Frontend can
...
pub async fn get_table_indexes(
    ...
    table: String,
    schema: String,
    // AC-180-04: optional cancel-token id (see get_table_columns).
    query_id: Option<String>,
    // Opt-in db-mismatch guard. See module doc.
    expected_database: Option<String>,
) -> Result<Vec<crate::models::IndexInfo>, AppError> {
...
/// List triggers attached to `(schema, table)`. PG impl
...
///
/// Opt-in `expected_database` mismatch guard.
#[tauri::command]
...
/// `pg_get_triggerdef(t.oid)` for one trigger. Non-PG
...
/// (Mongo) and non-PG RDB adapters (MySQL/SQLite/Oracle)
/// fail cleanly via `as_rdb()?` + the trait's default
/// `Unsupported` impl.
///
/// Opt-in db-mismatch guard via `expected_database`.
#[tauri::command]
...
mod tests {
    //! Rationale (2026-05-08): covers both the dispatch contract and the
    //! read-method routing for the read-only catalog commands of
    //! commands/rdb/schema.rs. The old `dispatch_*` helpers (a 1:1 mirror of
    //! the production body) left the prod handlers uncovered; that gap is
    //! closed by extracting the production code to `_inner(&AppState)` and
    //! having the tests call it directly.
    //!
    //! Scenario matrix, per catalog command:
    //!   - NotFound: the unregistered-connection path
    //!   - Unsupported(relational): the Document-paradigm branch
    //!   - Routing/Ok: method-specific sentinel via closure override
    //!   - Err propagation: witness — list_schemas
    //!   - Empty boundary: witness — list_schemas
    //!   - NamespaceInfo→SchemaInfo mapping verbatim: list_schemas only
    use super::*;
    ...
    // ── list_schemas witness — contract + boundary scenarios ──────────────
    ...
    // ── Unsupported(relational) tests on Document paradigm ───────────────
    ...
    // ── routing tests — schema-arg propagation ───────────────────────────
    //
    // The default StubRdbAdapter returns Ok(empty) for read methods. The
    // routing check plants a method-specific sentinel via a `closure
    // override` and confirms the dispatcher fetches it. Bonus: it also
    // verifies that schema/table parameters reach the trait unchanged (the
    // closure echoes its arguments).
    ...
    //
    // Reason (2026-05-08): for get_table_columns_inner /
    // get_table_indexes_inner / get_table_constraints_inner the path between
    // register and release is the prod handler's core. Called with query_id
    // Some → asserts that the registry is empty afterwards (i.e. release
    // ran).
    ...
    //
    // Rationale: asserts each schema introspection command's mismatch guard
    // verbatim. Byte-equivalent to the reference (the `execute_query_inner`
    // inline probe in `query.rs`) — the current_database probe must happen
    // *before* the trait call, and on mismatch the underlying trait method
    // (list_namespaces, list_tables, …) must not be called. The underlying
    // trait closure is made to panic, so a leaked guard surfaces immediately
    // as a failing panic.
    ...
    async fn get_table_columns_mismatch_releases_cancel_token() {
        // Even when the probe short-circuits *before* the trait call, the
        // registered token must be released so a retry can start cleanly
        // (the mismatch + cancel mirror).
        let mut s = mismatched_adapter();
    ...
    // ── match path + None fast-path witness ────────────────
    //
    // Rationale (2026-05-13): asserting only the mismatch path leaves doubt
    // about the happy/None paths' byte-equivalence. Adds 1 happy witness
    // (Some + match → trait call) and 1 none-fast-path witness (None → not
    // even a current_database probe).

    // ── list_triggers / get_trigger_source ──────────────────
    //
    // Rationale (2026-05-13): covers, for the two new _inner handlers, (a)
    // the 4 routing cases NotFound / Unsupported / happy / err, (b) the
    // mismatch panic-closure pattern (the probe blocks before the adapter
    // method call), and (c) that the schema / table / trigger_name arguments
    // reach the trait unchanged. The same pattern is byte-equivalent to the
    // existing list_functions_* / get_function_source_* tests, so it acts as
    // a regression guard.
    ...
    async fn list_triggers_inner_db_mismatch() {
        // Panic-closure pattern: probe rejects BEFORE the trait
