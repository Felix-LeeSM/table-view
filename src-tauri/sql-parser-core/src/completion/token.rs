...
fn quoted_identifier_prefix(before: &str) -> Option<(usize, String, char)> {
    quoted_identifier_prefix_for_pair(before, '`', '`')
        .or_else(|| quoted_identifier_prefix_for_pair(before, '[', ']'))
}

fn quoted_identifier_prefix_for_pair(
    before: &str,
    open: char,
    close: char,
) -> Option<(usize, String, char)> {
    if open == close && before.chars().filter(|ch| *ch == open).count() % 2 == 0 {
        return None;
    }
