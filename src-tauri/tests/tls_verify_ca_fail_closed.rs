...
//! guards (`connect_options_never_select_the_hostname_skipping_mode` in
//! `table-view-core/src/db/postgres/connection.rs` and
//! `.../db/mysql/connection.rs`), because `connect_options` is private to the
//! adapters. The write-boundary half lives next to its chokepoint in
//! `table-view-core/src/db/tls.rs`
//! (`verify_ca_without_a_ca_file_fails_closed`).
...
/// Whitespace-only is the hand-edited-JSON / raw-IPC variant of "absent". The
/// form writes `null` for an empty input, so only a payload that never went
/// through the form can carry `" "` — exactly the path a guard keyed on
/// `is_none()` alone would wave through.
#[tokio::test]
async fn whitespace_only_ca_path_counts_as_absent() {
    let mut config = verify_ca_without_ca_file(DatabaseType::Postgresql);
    config.ca_cert_path = Some("   ".into());
    let err = PostgresAdapter::test(&config)
        .await
        .expect_err("a whitespace-only CA path must not open a connection");
    assert_fails_closed(err);
}
