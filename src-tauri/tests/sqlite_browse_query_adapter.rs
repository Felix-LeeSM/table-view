...
use table_view_lib::models::{
    ...
    RenameTableRequest, SchemaChangeResult, SslMode,
};
...
async fn sqlite_contract_execute_query_returns_tabular_result_envelope() {
    ...
    assert_rdb_select_envelope(
        ...
        &["id", "email"],
        // id is an INTEGER column — wired as a string token per ADR 0026 (issue #1082).
        vec![vec![
            serde_json::json!("1"),
...
async fn sqlite_contract_probes_capabilities_and_runs_only_read_query_evidence() {
    ...
    if inventory.fts5 {
        ...
        assert_rdb_select_envelope(
            ...
            &["rowid", "title"],
            // rowid is an INTEGER — wired as a string token per ADR 0026 (issue #1082).
    ...
    if inventory.rtree {
        ...
        assert_rdb_select_envelope(
            ...
            &["id"],
            // id is an INTEGER — wired as a string token per ADR 0026 (issue #1082).
...
/// Issue #1082 (RED->GREEN) — a SQLite integer column is stored internally as an
/// i64 regardless of its declared type (type affinity), so wiring a value above
/// 2^53 as a raw JSON number lets the frontend's native JSON.parse demote it to
/// f64 and corrupt it silently. This extends ADR 0026's precision-sensitive
/// wire-string contract to the SQLite integer family — INTEGER/INT/BIGINT/
/// SMALLINT/TINYINT cells must serialize as JSON string tokens, and the
/// frontend's `wrapNumericCells` promotes them to BigInt.
#[tokio::test]
async fn sqlite_big_integers_serialize_as_wire_strings_issue_1082() {
    ...
    // A declared integer column (sqlx type_info().name() == "INTEGER") must be
    // wired as a precision-preserving string token.
    ...
    // A column or expression with no type information (sqlx
    // type_info().name() == "NULL") gives the frontend no way to promote it to
    // BigInt from data_type, because it falls outside the `wrapperFor` mapping.
    // Out of contract — it stays a raw JSON number. Stringifying it would be
    // worse: a raw string left in the grid regresses sorting, filtering, and
    // editing. Loss above 2^53 on the extreme path (expression results / untyped
    // columns) is documented as a known limitation.
    assert!(
        row[5].is_number(),
