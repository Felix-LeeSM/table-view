...
「라운드 대조」를 틀 안에 둔 이유는 `memory/workflow/review/memory.md` 「행동 계약」이
라운드 2 이상에 그 대조를 요구하는데 틀에 자리가 없어 라운드마다 임의 형식으로
붙었기 때문이다. 라운드 3 이상의 유형 재발 표는 위 「라운드 3 이상」이 요구하는
것이고 그 라운드에서 이 절을 대신한다.
