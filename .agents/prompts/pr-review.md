...
### 저자가 시도한 것   ← 라운드 3 이상에서만. 위 두 절과 나란히 선다
...
「라운드 대조」 · 「유형 재발」 · 「저자가 시도한 것」이 틀 안에 있는 이유는 같다 —
계약이 요구하는데 틀에 자리가 없어 라운드마다 임의 형식으로 붙거나 아예 빠졌다.
요구하는 자리는 `memory/workflow/review/memory.md` 「행동 계약」의 라운드 2 이상
대조와 `memory/workflow/orchestration/memory.md` §3 의 보고 항목이고, 마지막 절은
§3 이 세는 항목인데 이름 있는 자리를 못 받고 있던 쪽이다 (이슈 #2514).

**라운드 3 에서 위 세 절은 나란히 선다 — 어느 것도 다른 것을 대신하지 않는다.**
