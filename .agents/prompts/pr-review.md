...
형식」의 「유형 재발」 절이고 「라운드 대조」를 밀어내지 않는다. 사이클로 판정되면
...
### 라운드 대조        ← 라운드 2 이상에서만. 라운드 1 이면 절을 통째로 뺀다
- 직전 라운드 blocking / 이번 라운드 blocking / 둘 다에 있는 것. 사이클 판정은 안 한다.

### 유형 재발          ← 라운드 3 이상에서만. 위 「라운드 대조」와 나란히 선다
- 유형 × 라운드별 건수. 사이클로 판정되면 아래 「Verdict」의 정지 사유에 적는다.
...
- 정지 필요: 사이클로 판정했으면 그 사유. 아니면 이 줄을 뺀다
- fan-out: 관점 목록 / 불가로 단독 강등했으면 그 사실
```

「라운드 대조」와 「유형 재발」이 틀 안에 있는 이유는 같다 — 계약이 요구하는데 틀에
자리가 없어 라운드마다 임의 형식으로 붙었다. 요구하는 자리는 각각
`memory/workflow/review/memory.md` 「행동 계약」의 라운드 2 이상 대조와
`memory/workflow/orchestration/memory.md` §3 의 보고 항목이다.

**라운드 3 에서 둘은 나란히 선다 — 재발 표가 대조를 대신하지 않는다.** 위 대조를
요구하는 줄이 「라운드 2 이상」이라 3 을 포함하고, §3 은 「라운드별 blocking 집합
변화」와 「재발한 유형과 라운드별 건수」를 **각각** 보고 항목으로 센다. 게다가 §3
트리거 자체가 연속 두 라운드의 blocking 집합으로 정의되니, 그 트리거가 발화하는
라운드에서 대조를 빼면 판정의 입력이 없어진다. 위 「라운드 3 이상 — 회고 모드」의
「개별 지적 대신」이 갈아 끼우는 것은 개별 finding 수리 요구이지 이 대조가 아니다.
