...
가 `.git` 을 공유해 index.lock · FETCH_HEAD 충돌을 냈다)와 사본당 노드를 하나로
제한하는 「책임」. 읽기만 하는 리뷰라도 `git fetch` 한 번이 서 있는 사본의
`.git` 에 쓴다.
...
### 저자가 시도한 것   ← 라운드 3 이상에서만. 위 「라운드 대조」·「유형 재발」과 나란히 선다
- 재발한 유형에 저자가 무엇을 걸었고 그 처방이 어느 축을 안 덮었나. 「고쳤다 /
  안 고쳤다」가 아니라 시도의 축과 그 축 밖에 남은 자리를 적는다.
...
- 함께 정지: 사이클로 판정했으면 파일 교집합이 있는 in-flight PR 과 그 공유 파일.
  교집합이 없으면 "없음". 사이클이 아니면 이 줄을 뺀다
- 선택지: 사이클로 판정했으면 범위 축소 · 근본 분리 · 닫고 재설계 중 무엇을 왜
  권하나. 사이클이 아니면 이 줄을 뺀다
