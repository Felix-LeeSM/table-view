...
# 조용히 잘린다. #2254 의 「잘려도 표시가 없다」는 사라진 게 아니라 임계가 올라갔다
...
**새 형태는 옛 형태의 상위집합이 아니다.** 0 이면 문구를 **연속 공백이** 안 낀
조각으로 줄여 다시 재고, 그래도 0 이면 교정 대상에서 빼기 전에 위 원문 덤프를
육안으로 훑는다. 0 을 잘못 믿는 값이 한쪽으로만 크기 때문이다 (같은 방).

**한 칸 공백까지 빼지 마라 — `tr -s` 는 공백 run 만 접고 한 칸은 그대로 남는다.** 남은
한 칸이 조각의 판별력이다: 공백을 다 뺀 `39` 는 무관한 PR 의 SHA `539d05f3` 나 이슈
번호 `#2239` 에도 걸리지만 한 칸을 살린 `# 39` 는 안 걸린다. 자를 자리는 공백이 아니라
**연속 공백 구간**이다.
