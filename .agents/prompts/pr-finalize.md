...
이슈 번호 · 브랜치 · 회수할 사본 경로)는 이 파일에 없고 spawn 메시지가 싣는다.

**이 파일은 자동으로 전달되지 않는다.** spawn 하는 쪽이 이 파일을 첨부하거나,
harness 의 agent 정의(`.claude/agents/pr-finalize.md`)가 첫 행동으로 읽어야 노드에
도달한다.

**memory 계약 본문을 복제하지 않는다.** 이 파일에 있는 것은 절차 고정부뿐이고,
정의와 사유와 예외를 정하는 SOT 는 아래 read 목록에 있는 문서다. 이 파일과 그
문서가 어긋나면 memory 를 따른다.

## MANDATORY 첫 명령

회수 대상 사본 **안에서 실행하면 안 된다.** 그 사유는
`memory/runbook/worktree/memory.md` 「회수」가 소유한다.
...
- `.agents/skills/diagnosing-merge-gates/SKILL.md`: 라운드 게이트와
  `mergeStateStatus` 값의 뜻, BLOCKED 진단 순서, 하면 안 되는 대응이 있다.
- `memory/runbook/worktree/memory.md` 「회수」: 사본 삭제 판정이 있다.
- `.agents/skills/recovering-push-rejects/SKILL.md` 「PR close cleanup」: 브랜치
  정리 절차가 있다 (계약은 `memory/workflow/git-policy/memory.md`).

## 금지 / Write 예산

- 코드를 고치지 않는다. red 를 자기 손으로 해소하지 않으며, 구현자를 다시
  띄우는 것은 orchestrator 가 맡는다.
...
- 새 커밋을 push 하거나 `gh pr update-branch` 로 head SHA 를 건드리지 않는다.
  ...
  `.agents/skills/diagnosing-merge-gates/SKILL.md` 「잘못된 대응이 만드는 함정」이
  정한다.
- write 는 `reflect:done` label 과 머지, 브랜치와 사본 회수, 이슈 종결까지다.
...
절이 「머지 전 확인」보다 앞에 있는데, 관문부터 보면 바로 그 red 때문에 부착
지점에 도달하지 못하기 때문이다.

**라운드는 코멘트 수가 아니라 서로 다른 head 커밋에 붙은 리뷰 인계의 수다**
(#1968). 이 수를 세는 것은 게이트의 `Count review rounds by head OID` 스텝이고,
종결자는 다시 세지 않고 게이트가 낸 판정을 읽는다.

1. verdict 를 먼저 본다. `review:approved` 가 있고 `review:changes-requested`
   가 없어야 한다. red 면 여기서 종료하는데, 종결자는 red 를 진행시키지 않는다.
2. 코멘트 수가 **3 미만이면 라운드도 3 미만이므로** (라운드 ≤ 코멘트 수) 부착 없이
   2단계로 간다. 3 이상이면 최신 `review-gate` run 을 본다: `Stop at review round 3`
   에서 red 면 라운드 3 이상이고, 그 스텝이 `rounds=N` 을 에러로 찍는다.
3. 라운드 3 이상이면서 verdict 가 green 이면
   `gh pr edit <N> --add-label reflect:done` 을 실행한다. green 일 때 붙이는 것이
   종결자다. red 면 붙이지 않고 회고 모드 리뷰어와 interface 를 거친다.
...
rollup 은 이 한 곳에서만 쓰며 **required 판정 수단이 아니다.** required 는
2단계의 `mergeStateStatus` 로 확인한다. 그 구분과 라운드 게이트의 조건, 누가
붙이는가, 여기서 rollup 을 봐야 하는 이유는
`.agents/skills/diagnosing-merge-gates/SKILL.md` 가 정한다.
...
1. `mergeStateStatus` 가 `CLEAN` 또는 `UNSTABLE` 이어야 하며,
   required(`review-gate` 포함) 충족 판정은 이 값으로 읽는다. 두 값의 뜻과
   `BLOCKED` 진단 순서는 `.agents/skills/diagnosing-merge-gates/SKILL.md` 가
   정하므로, `BLOCKED` 면 머지를 시도하지 말고 그 skill 로 간다.
...
하나라도 충족되지 않으면 머지하지 않고 상태를 보고하고 종료한다.

## 3단계: 머지

**squash 가 main 히스토리에 남기는 표면은 제목과 body 둘이고, 어느 쪽도 PR body 가
아니다.** 리뷰 라운드가 뒤집은 주장이 둘 중 어디에 남아도 그대로 히스토리가
...
**제목 쪽 갈래를 가르는 것은 커밋 수 하나뿐이다.** 무엇이 착지하는지와 그 repo
...
둘 이상이면 모든 라운드를 대조한다. **무엇이 거짓인지 새로 판정하지 않고, 리뷰어가
이미 판정한 것을 위 두 표면에서 찾는다.**
...
연속 공백을 접을 뿐 아니라 탭과 개행도 한 칸으로 **치환**한다. 원문 그대로의 조각은
그래서 세 방향으로 어긋나는데, 들여쓴 이어짐(연속 공백)도 탭 하나도 0 이 나오고,
개행이 들어가면 `grep -F` 가 대안 패턴 둘로 읽어서 **뒷부분이 없어도 앞부분 출현
횟수가 그대로 값이 된다** (`aaa\nbbb` 를 `aaa aaa xxx` 에 걸면 2). 어긋났는데도 정상
hit 수와 구분이 되지 않으므로 값만 보고는 잡지 못한다. **새 형태는 옛 형태의
상위집합이 아니다.** 정규화하면 문구를 짧게 하지 않고도 걸린다.

**그래도 줄여야 한다면 자를 자리는 공백이 아니라 연속 공백 구간이다.** 한 칸까지
빼면 조각이 짧아지고, 짧을수록 무관한 PR 에 걸린다. `# 39` 를 `39` 로 줄이면 SHA
`539d05f3` 와 이슈 번호 `#2239` 에 걸리는데, 걸리게 만든 것은 뗀 공백이 아니라 같이
뗀 `#` 다. 「연속 공백이 안 낀」은 hit 의 필요조건이지 충분조건이 아니다.

**hit 0 은 「커밋 메시지에 없다」의 증명이 아니다.** 위 `ABORT` 가 조회 실패를
배제한 뒤에도, `--jq` 필드명 오타와 정규화가 어긋난 조각은 똑같이 0 을 낸다. 오타
쪽은 `gh` 가 rc 0 과 빈 값을 내므로 `ABORT` 를 지나지 않는다. 0 이 나오면 교정
대상에서 빼기 전에 위 원문 덤프를 육안으로 훑는데, 0 을 잘못 믿었을 때의 손실이
한쪽으로만 크기 때문이다 (같은 문서).

**그 값은 hit 여부가 아니라 자리 수이므로, N 이면 커밋 메시지 N 곳에 있고 N 곳을
다 고친다.** scorecard 가 커밋 하나를 지목했어도 그것을 자리 수로 읽지 마라:
...
**라운드 N 의 finding 이 지목한 주장이 라운드 N 이전 커밋 메시지나 `$TITLE` 에
그대로 남아 있으면 그것이 교정 대상이며**, 수치든 산문이든 저자의 철회문이든
같다. 걸린 표면마다 그 표면의 교정 지점을 쓴다.
...
**`--subject` 를 줬을 때 GitHub 이 ` (#<PR>)` 꼬리를 또 붙이는지는 실측이 없는데**,
재려면 실제 머지가 필요하기 때문이다. 실측된 것은 **기본** 계산이 그 꼬리를 붙인다는
것까지다 (#2369 는 커밋이 넷이라 PR 제목을 썼고 착지 제목이 그 제목 + ` (#2369)`
였다). **꼬리를 직접 넣어 넘기고, 머지 뒤에 착지 제목을 읽어 반환 형식에 그대로
싣는다.** 꼬리가 빠졌거나 둘이면 거기서 드러나고, 그 보고가 다음 종결자의 실측이
된다.
...
1. **브랜치** 는 `--delete-branch` 가 remote 쪽을 지운다. 사본에 체크아웃돼 있어서
   로컬 브랜치 삭제가 실패하면 경고만 나온다. **무해하므로 보고만 하고 넘어간다.**
2. **사본** 은 **`$CLONE` 하나가 아니라 그 사본이 들어 있는 루트 디렉토리를
   훑는다.** `$CLONE` 만 보면 리뷰어 일회용 사본과 다른 세션이 남긴 사본이 장부에
   아예 들어오지 않는데, 종결자는 그것들의 경로를 spawn 에서 받지 않기 때문이다.
   판정 규칙과 dirty 보존, 절대 안 지우는 대상, `review__` 축을 정하는 SOT 는
   `memory/runbook/worktree/memory.md` 「회수」이고, 이 preamble 은 그 판정에 필요한
   값을 **디렉토리마다** 모으는 데까지만 담당한다.
   ...
   각 행의 값을 `MERGED_OID` 와 함께 그 문서의 판정에 넣고, **지우기로 판정된 것만**
   `rm -rf` 한다. 판정이 서지 않으면 지우지 말고 보존 사유를 보고에 남긴다.
   **행 하나도 빠뜨리지 않고 보고한다.** 아래 반환 형식이 자리를 나열하게 하는
   이유가 이것이다. 자리 수가 `du` 출력보다 적으면 스윕이 잘린 것이다.
3. **이슈** 는 PR body 의 `Closes #<이슈>` 로 자동 종결됐는지 확인하고, 안 됐으면
   `gh issue close <이슈>` 로 닫는다. 삭제가 큰 머지였으면 지운 경로를 참조하는 열린
   이슈를 `git grep` 과 `gh issue list` 로 훑어서 보고에 싣는다.
