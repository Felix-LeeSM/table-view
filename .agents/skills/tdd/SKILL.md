...
- [ ] 어떤 인터페이스 변경이 필요한지 티켓의 파일 범위와 대조
- [ ] 어떤 동작을 테스트할지 티켓의 수용 기준에서 뽑기 (우선순위 매기기)
- [ ] [deep modules](deep-modules.md) (작은 인터페이스, 깊은 구현)의 기회를 식별
- [ ] [testability](interface-design.md)를 위한 인터페이스 설계
- [ ] 테스트할 동작 리스트 작성 (구현 단계 아님)
- [ ] 그 리스트가 티켓의 수용 기준을 덮는지 확인. 안 덮이는 게 있으면 **범위 밖**이다

**되묻지 마라.** 이 skill 의 주 소비자는 spawn 된 subagent 이고 그 자리에는
사용자에게 되물을 경로가 없다 — 물으면 답 없이 멈춘다. 판단이 필요한데 티켓에
없으면 그 사실을 반환값에 적고 멈춘다. 티켓에 안 적힌 것은 범위 밖이다
(`memory/workflow/orchestration/memory.md` 의 수용 기준 규율).

**모든 것을 테스트할 수 없어.** 티켓의 수용 기준이 무엇이 중요한지 정한다. 가능한 모든 엣지 케이스가 아니라, 핵심 경로와 복잡한 로직에 테스트 노력을 집중.
