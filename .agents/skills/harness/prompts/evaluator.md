...
5. **Score objectively** — use the rubric that matches the sprint type. The pass bar is `>= 8/10` on every dimension (sprint evaluation only — `.agents/skills/pr-review/SKILL.md` Verdict uses no scores, so this bar is not aligned with it); a 7/10 is genuinely good work that still falls short of pass, not an average score to hand out freely.
