...
5. **Score objectively** — use the rubric that matches the sprint type. The pass bar is `>= 8/10` on every dimension (sprint evaluation only — pr-review stopped using scores on 2026-07-27, so this bar is no longer aligned with it); a 7/10 is genuinely good work that still falls short of pass, not an average score to hand out freely.
