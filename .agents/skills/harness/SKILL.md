...
- `PASS_THRESHOLD`: 8.0 (each dimension must score ≥ 8/10 — sprint 평가 전용 기준. `.agents/skills/pr-review/SKILL.md` Verdict 는 점수를 쓰지 않으므로 정렬 대상이 아니다)
...
10. **Delivery handoff** — The harness builds and evaluates; it does not ship. After a PASS Final Report, hand the result to the implementer contract (commit → push → PR create): `memory/workflow/delivery/memory.md`. Review, merge and cleanup belong to later nodes, not to the harness.
