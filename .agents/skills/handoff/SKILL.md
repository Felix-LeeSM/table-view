---
name: handoff
description: node 사이의 인계를 쓰고 읽는다. 이슈 코멘트 append + label 갱신(write), 나에게 온 최신 인계 검증(read). 작업을 시작할 때와 끝내고 죽기 전에 사용.
---
...
#1918 §8 의 세 번째 연산 `state` (라우팅 표 · 다음 node 판정) 는 **아직 없다.**
#2006 이 소유한다 — 지금은 orchestrator 가 label 을 직접 보고 정한다.
...
| 2 | 외부 명령 실패 · 실행 환경 | `gh` 출력 그대로, 또는 `yaml` 모듈 부재. **입력을 고칠 일이 아니다** — `npm install` 이거나 `gh` 쪽이다 |
| 3 | `RETRY` | 판정이 낡았다 (`at` 이 PR head 의 조상). 리뷰를 다시 돈다 |
| 4 | `USER` | 사용자에게 올린다. **재시도로 안 고쳐진다** |

필드 누락의 재시도 상한은 **0** 이다 — `write` 가 같은 검증기를 이미 통과시켰으므로
읽을 때 누락이면 인계를 만든 경로가 이 스킬이 아니었다는 뜻이다 (알려진 경우: 스킬
우회 · 스키마 버전 불일치 · 스킬 버그. 그 셋이 전부라는 주장은 아니다).

**exit 2 와 exit 4 를 섞지 마라.** `yaml` 이 안 잡히는 자리에서 돌면 exit 2 다. 그게
exit 4 `앞 node 가 아예 안 돌았다` 로 나오면 #1918 §10 의 사망 탐지 신호가 거짓으로
울리고, exit 1 로 나오면 agent 가 멀쩡한 YAML 을 계속 고친다. 두 오진 다 실측됐고
지금은 케이스와 mutation 이 잠근다.
...
- **저장소가 PUBLIC 이라 신뢰 경계가 있다.** `read` 는 `authorAssociation` 이
  `OWNER`/`MEMBER`/`COLLABORATOR` 인 코멘트의 인계만 받는다. 나머지 5종
  (`CONTRIBUTOR` · `FIRST_TIME_CONTRIBUTOR` · `FIRST_TIMER` · `MANNEQUIN` ·
  `NONE`) 과 필드 부재는 거부하고, stderr 로 누구 것을 왜 버렸는지 낸다 — 인계는
  받는 node 가 돌릴 `action.cmd` 를 실어 나르므로 아무나 쓴 것을 권위 있는 입력으로
  볼 수 없다. 8종이 전부인 근거(`__type(name:"CommentAuthorAssociation")` 조회)는
  `scripts/handoff.mjs` 의 `TRUSTED_ASSOCIATIONS` 주석에 명령째로 있다.
...
- `base_oid` 는 **기록만** 한다. 무효화 트리거로 쓰면 재리뷰가 57% 로 뛰고 정작
  그 사고(#1937)는 못 잡는다 — 그 실측은 **#1918 §7 이 소유한다.**
- 코드펜스는 다섯 변종을 받는다(들여쓰기 0-3칸 · `~~~` · info string 뒤의 말 ·
  닫는 펜스 뒤 공백 · info string 대소문자). **GitHub 렌더러가 그것과 같게 취급하는
  변종을 다 덮는다는 주장은 하지 않는다** — 그 집합은 여기서 열거할 수 없다. 목록과
  확인 명령은 `scripts/handoff.mjs` 의 `FENCE` 주석에 있다.
- agent 정의(`.claude/agents/` · `.codex/agents/`)의 `skills:` 배선은 #1928 이
  소유한다. 이 스킬은 스스로를 걸지 않는다.

## Related

- `scripts/handoff.mjs` — 두 연산의 구현 (SOT)
- `scripts/hooks/policy/test-handoff.sh` — 회귀 + mutation 증명 (양성/음성 대조 포함)
- `memory/workflow/delivery/memory.md` — node 별 행동 계약
- `.agents/skills/pr-review/SKILL.md` — 리뷰어의 판정 방법론
- #2006 — 빠져 있는 `state` 연산 (라우팅 표 · 다음 node 판정)
