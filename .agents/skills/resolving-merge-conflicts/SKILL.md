...
**이 저장소에서 충돌은 예외가 아니라 기본값이다.** 2026-07-25 실측에 따르면 동시
...
`git pull` 은 모든 변종이 금지이므로 fetch 와 merge 를 나눠 쓴다.
...
도 돌린다. Rust crate 는 `src-tauri` 를 root 로 하는 하나의 workspace 에 속하고,
`--all` 과 `--workspace` 가 member 전부를 잡는다 (#2161).
파서 코어를 건드렸으면 WASM 을 다시 만든다 (`pnpm build:sql-wasm` /
`pnpm build:mongosh-wasm`). 소스만 고치고 WASM 산출물을 그대로 두면 런타임에서
동작이 갈라지기 때문이다.

실패가 나오면 그것이 해소가 틀렸다는 신호이므로 3으로 돌아간다.
...
merge 로 온 이상 결과 트리가 의도와 같다는 것은 **트리 비교로 증명하며**,
...
non-fast-forward 로 거부되면 `git reset --hard` 나 `git pull --rebase` 로 가지 말고
