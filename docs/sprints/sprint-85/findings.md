...
- **Scope drift (low)**: `docs/archives/lessons/memory-lessons-2026-05-26/memory.md` and the new `docs/archives/lessons/memory-lessons-2026-05-26/agent-and-git/2026-04-24-parallel-agent-commit-isolation/` directory are present in the working tree but were not declared in the Sprint 85 WRITE scope. They are not in the forbidden list and add a single lesson entry, so they do not fail the invariants, but they should ideally live on a separate commit/branch. Worth flagging to the orchestrator so the Sprint 85 commit isolates only the feature diff.
...
1. **Scope hygiene** — The `docs/archives/lessons/memory-lessons-2026-05-26/*` additions ride on the same working tree. Before the orchestrator commits Sprint 85, verify the Sprint 85 commit only touches the declared WRITE files and rehome the lesson into a separate commit.
   - Current: `git diff --stat HEAD` shows `docs/archives/lessons/memory-lessons-2026-05-26/memory.md | 1 +` and an untracked `docs/archives/lessons/memory-lessons-2026-05-26/agent-and-git/2026-04-24-parallel-agent-commit-isolation/` directory.
...
- Open P1/P2: 0
- Safe to commit: yes, *provided* the orchestrator isolates the sprint-85 feature commit from the `docs/archives/lessons/memory-lessons-2026-05-26/*` churn as noted in feedback #1.
