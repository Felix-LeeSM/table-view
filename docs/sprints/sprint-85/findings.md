...
1. **Scope hygiene** — The `docs/archives/incidents/*` additions ride on the same working tree. Before the orchestrator commits Sprint 85, verify the Sprint 85 commit only touches the declared WRITE files and rehome the lesson into a separate commit.
   - Current: `git diff --stat HEAD` shows `docs/archives/incidents/memory.md | 1 +` and an untracked `docs/archives/incidents/agent-and-git/2026-04-24-parallel-agent-commit-isolation/` directory.
...
- Open P1/P2: 0
- Safe to commit: yes, *provided* the orchestrator isolates the sprint-85 feature commit from the `docs/archives/incidents/*` churn as noted in feedback #1.
