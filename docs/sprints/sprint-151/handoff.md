...
- **TDD red-state log is captured pre-implementation** but the orchestrator commits the module + test together (per `.agents/skills/harness` workflow). Two-commit ordering would be a stronger signal; the durable log file is the contract-allowed alternative.
