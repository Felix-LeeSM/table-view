---
sprint: 388
title: review infra — 자동 layer + 정성 pr-reviewer + profile 분기
date: 2026-05-18
owner: orchestrator
status: in_progress
...
scope) 만 pr-reviewer agent 가 평가. agent spawn = PR 당 1회.
...
8. **`.claude/agents/pr-reviewer.md` 갱신** — read 리스트 교체:
...
10. **harness skill 분리** — `.claude/skills/harness/prompts/evaluator.md` (skill
    ...
    선택한 이유 = skill 의 `evaluator` 와 명확히 구분.

### Out of scope (deferred)

- 외부 brain (Codex / Cursor) 용 pr-reviewer wrapper — 본 sprint 는 Claude Code
  의 pr-reviewer 만 갱신. Codex 호환은 별 sprint.
...
- `AC-388-09` `.claude/settings.json` PostToolUse Edit|Write 에
  ...
  매치).
- `AC-388-10` `.claude/agents/pr-reviewer.md` ≤ 15 줄, read 리스트가 새 source
  3개.
...
- handoff.md 에 Codex pr-reviewer / coverage dynamic threshold deferred 명시
