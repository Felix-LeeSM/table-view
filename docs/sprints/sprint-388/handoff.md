...
layer (evaluator agent 1 spawn). 다음 PR 부터 본 룰 적용.
...
1. `docs/sprints/sprint-<N>/contract.md` frontmatter 에 `review-profile:
   code | security | infra | docs` 추가 (없으면 evaluator 가 기본 매트릭스
   적용, 단 TDD 등 자동 검사 영향).
...
4. agent A 가 `evaluator` agent spawn:
   - evaluator 가 `memory/workflow/review/memory.md` read
...
| ID | 작업 | 근거 |
|---|---|---|
| D1 | Codex / Cursor 의 evaluator wrapper 형식 | sprint-387 deferred 와 동일 — multi-brain 확장 |
...
| D5 | evaluator 의 codex 외부 리뷰 자동 spawn (현재 사용자 명시 시만) | 정책 변경 시 |
...
- contract frontmatter 의 `review-profile` 누락 시 evaluator 가 기본 매트릭스
  적용. 단 명시 권장 (보고서 noise 감소).

## 다음 시점 검증 항목

- code profile sprint 실제 사용 시 — `[RED]` commit 강제 동작 확인.
- evaluator agent 가 정성 3 차원 + profile 분기 정확히 적용하는지 PR comment
  품질 점검.
