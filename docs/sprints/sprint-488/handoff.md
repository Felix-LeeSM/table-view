...
- `cargo test --manifest-path src-tauri/sql-parser-core/Cargo.toml --quiet`
  - passed: 546 tests
- `pnpm vitest run src/stores/schemaStore.test.ts src/stores/schemaStore.db-aware.test.ts src/stores/schemaStore.dbMismatch.test.ts src/stores/schemaStore.clearForConnection.test.ts src/stores/schemaStore.scope.test.ts src/lib/sql/sqlCompletionContext.test.ts src/lib/sql/sqlCompletionRequest.test.ts src/lib/sql/sqlCompletionWasm.test.ts src/components/query/QueryTab.dialect.test.tsx --reporter=dot`
  - passed: 80 tests across 9 files
...
- `bash scripts/check-wasm-size.sh`
  - passed: SQL wasm gzip 86058 bytes, Mongo wasm gzip 52169 bytes
- `git diff --check`
  - passed
