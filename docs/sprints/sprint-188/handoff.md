...
| `docs/sprints/sprint-188/findings.md` | 설계 결정 + AC→테스트 매핑 + Sprint 187 §10 가정 정정 + Phase 24 후속 후보. |
...
- **Sprint 188-followup (proposal)**: RDB 5 사이트 inline gate 를
  `useSafeModeGate` 로 마이그레이션. 회귀 risk 격리 단위 — 각 사이트가
  `ConfirmDangerousDialog` state 를 다르게 관리하므로 sprint 단위로 분리.
- **Phase 24 (proposal)**: Mongo bulk-write Tauri command 신규
  ...
  이므로 인풋 analyzer 만 추가됨.
- **단건 mutate 정책 미결**: `insert_document` / `update_document` /
  `delete_document` 단건 + RDB row-level single delete 는 통합 정책 결정
  후 일괄 적용. Phase 24 후보 또는 user feedback 추가 수집 후 결정.
- **Sprint 189–192 (이미 PLAN.md 등록)**: FB-1b prod-auto SafeMode,
  FB-3 DB 단위 export, FB-4 Quick Look 편집, FB-5b query history source
  필드.
