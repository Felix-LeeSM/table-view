...
별도 결정 단위로 분리. (Phase 24 후보 또는 user feedback 추가 수집
후 결정.)

## 8. Phase 24 후보 — Mongo bulk-write 신규 명령
...
인풋만 새 analyzer (`analyzeMongoOperation` 류) 가 추가된다.
...
- **Sprint 188-followup (proposal)**: RDB 4 사이트 inline gate 를
  `useSafeModeGate` 로 마이그레이션. 회귀 risk 분리 단위.
- **Phase 24 (proposal)**: Mongo bulk-write Tauri command 신규 +
  UI 진입점 + 분석/게이트.
- **Sprint 189–192 (이미 PLAN.md 등록)**: FB-1b prod-auto SafeMode,
  FB-3 DB 단위 export, FB-4 Quick Look 편집, FB-5b query history
  source 필드.
