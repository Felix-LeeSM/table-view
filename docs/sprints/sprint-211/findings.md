...
3. (architecture lessons) `docs/archives/lessons/memory-lessons-2026-05-26/` 에 "JSX 가 들어가는 모듈을 `.ts` 로 명시하면 안 된다" lesson 등록 후보.
...
4. **sprint-212 candidate 2 (planner protocol 보강)** — F-001 lesson 을 `docs/archives/lessons/memory-lessons-2026-05-26/` 에 등록 (`spec 에 "live in helpers.ts" 표현이 있을 때 plugin/loader JSX-parse 제약 사전 검토`). spec 작성 시 `JSX-bearing module 은 반드시 .tsx`, `pure-helper module 은 .ts` 분리 의도 명시. 후속 spec author 가 같은 함정 피하도록.
