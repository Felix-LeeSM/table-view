---
sprint: 387
title: multi-agent infra — lazy wrappers + platform-neutral hooks/rules/skills
date: 2026-05-18
owner: orchestrator
status: in_progress
---
...
따라서 wrapper / rule / skill 은 **lazy 패턴** — 본문은 `memory/`, `scripts/`,
`.agents/skills/` 에 source-of-truth 1부, platform-specific 디렉토리
(`.claude/`, `.codex/`, `.cursor/`) 에는 필요한 포인터만 둔다.
...
1. **Agent wrapper 압축** — `.claude/agents/*.md` 9개 파일 50-90 줄
   ...
   `memory/engineering/conventions/` 의 기존 source-of-truth 로 redirect.
...
4. **Hook 평면화** — `.claude/hooks/pre-bash.sh` 를
   ...
   호출 모두 호환.
5. **Rules 평면화** — `.claude/rules/*.md` 6개 본문을 `memory/engineering/conventions/`
...
6. **Agent skill 통합** — `remember`, `split-memory` 본문은
   `.agents/skills/<name>/SKILL.md` 로 통합. `.claude/commands/<name>.md`
   wrapper 는 삭제해 command source 를 중복하지 않는다.
...
- `AC-387-06` `.claude/commands/{remember,split-memory}.md` 없음.
- `AC-387-07` `.agents/skills/remember/SKILL.md`,
  `.agents/skills/split-memory/SKILL.md` 존재, sprint-386 의 본문 보존.
...
1. `wc -l .claude/agents/*.md .claude/rules/*.md CLAUDE.md AGENTS.md` — 줄수 cap 확인.
2. `test ! -e .claude/commands/remember.md && test ! -e .claude/commands/split-memory.md`
3. `bash scripts/check-memory-structure.sh`
4. `bash scripts/check-god-file.sh` (영향 없음 확인)
5. `bash scripts/regenerate-indexes.sh` (memory/ 새 방 추가 후 index 재생성)
6. Hook smoke: AC-387-11, AC-387-12.
7. Worktree script smoke: `bash scripts/worktree-spawn.sh --help` exit 0.
8. `pnpm tsc --noEmit`
9. `pnpm lint`
10. `pnpm vitest run`
11. `cd src-tauri && cargo clippy --all-targets --all-features -- -D warnings && cargo test`
12. `gh pr create` (생성만, 머지 NOT)

### Required Evidence

- 줄수 cap 통과한 wrapper / rule 목록과 삭제된 Claude command wrapper 확인.
