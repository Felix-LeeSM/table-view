...
> **Sprint 171 재분류 (2026-04-30)**: 9 항목 모두 권위 component test 가
> 이미 존재함이 확인됨. 신규 작성 불필요 → **DELETE 로 강등**, e2e 만 제거.
> 사유: sprint-138 ~ sprint-149 사이에 미리 작성된 컴포넌트 테스트가 e2e
> 시나리오를 *그대로 또는 더 강하게* 커버하고 있었다. P1 (피라미드) 재확인.

| # | spec | 시나리오 | 권위 component test (이미 존재) |
|---|---|---|---|
| 10 | `feedback #3` | PG autocomplete | `SqlQueryEditor.test.tsx` AC-S139-02 |
| 11 | `feedback #3` | Mongo autocomplete | `MongoQueryEditor.test.tsx` AC-S139-01b (cross-contamination guard 포함) |
| 13 | `feedback #4` | MySQL default user | `ConnectionDialog.test.tsx` AC-S138-01/03 |
| 14 | `feedback #4` | SQLite form (host/port 숨김) | `ConnectionDialog.test.tsx` AC-S138-04 |
| 15 | `feedback #4` | MongoDB default user | `ConnectionDialog.test.tsx` AC-S138-01 Mongo |
| 19 | `feedback #5` | plaintext NOT offered | `ImportExportDialog.ac149.test.tsx` AC-149-5 |
| 20 | `feedback #7` | no sprint copy in tooltips | `src/__tests__/no-stale-sprint-tooltip.test.ts` AC-141-2 (static guard, 4 regex) |
| 21 | `feedback #7` | schema selection 한 곳 | `WorkspaceToolbar.test.tsx` AC-S135-01 (legacy SchemaSwitcher chip 부재) |
| 27 | `feedback #11` | Functions sidebar layout | `SchemaTree.dbms-shape.test.tsx` AC-145-3 |
...
- **sprint-170** — DELETE 8 + DEFERRED 0 + EVAL 결정 적용. 가벼움. ✅
- **sprint-171** — MOVE 9 재분류 → DELETE 9 (권위 이미 존재) + e2e 정리. 매우
  가벼움. SchemaTree context menu component test 도 신규 작성 불필요
  (`SchemaTree.test.tsx` AC-CM-01 ~ AC-CM-17 가 이미 커버). ✅
