...
Top 5 (sprint-403b, 404-407) 은 자세히, 6~17 은 간략. 실제 sprint 진입 전 `docs/sprints/sprint-N/` 으로 옮기고 contract finalize.

Sprint 당 < 800 LOC churn target. Risk 등급 H 인 sprint 는 wave 분할 가능 (phase 1/2).

---

## sprint-403b — sprint-392 sqlSafety kind/severity drift 정정
...
**AC**:
- AC-403b-01: `classify("INSERT INTO t VALUES (1)")` → `{ kind: "dml-insert", severity: "info" }` ✓
- AC-403b-02: `classify("UPDATE t SET a=1")` → `{ kind: "dml-update", severity: "warn" }` ✓
- AC-403b-03: `classify("DELETE FROM t")` → `{ kind: "dml-delete", severity: "warn" }` ✓
- AC-403b-04: sprint-392 contract 의 line 25 / 188 / 245 내부 모순 해결 (한 prefix 로 통일)
- AC-403b-05: TS callsite `lib/safeMode.ts` / `useQueryExecution.ts` 의 분기 모두 새 kind 와 정합
- AC-403b-06: e2e: insert 실행 시 confirm dialog *뜨지 않음* (contract `info` 의도)
