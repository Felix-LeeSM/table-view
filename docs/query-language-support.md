...
- MySQL: completion vocabulary는 current UI group 기준 100%; runtime adapter는
  ...
  `LIMIT offset, count`를 포함한다.
- MariaDB: MySQL family completion/runtime path 공유; client parser semantics는
  MySQL-family `LIMIT offset, count`를 포함하며 MariaDB-only delta는 후속.
...
- SQL semantic widening: MySQL/MariaDB `ON DUPLICATE KEY UPDATE`,
  `CALL`, `LOAD DATA`, `DELIMITER`/procedure body, transaction/control-flow
  scripting.
