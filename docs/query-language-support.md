...
- expressions: literals, column refs, simple unqualified function calls in
  SELECT-list and predicate/value positions, `CASE`, window functions with
  `OVER`, scalar subqueries.
...
- schema-qualified functions, nested function arguments, `DISTINCT`, and
  arbitrary arithmetic/string expression arguments are still out of scope.
