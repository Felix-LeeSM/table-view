...
현재 parser 의미 지원은 MySQL 섹션과 같다. `LIMIT offset, count`는
MySQL-family client parser subset에 포함된다.
...
- MySQL: completion vocabulary는 current UI group 기준 100%; runtime adapter는
  ...
  `LIMIT offset, count`, `ON DUPLICATE KEY UPDATE`를 포함한다.
- MariaDB: MySQL family completion/runtime path 공유; client parser semantics는
  MySQL-family `LIMIT offset, count`, `ON DUPLICATE KEY UPDATE`를 포함하며
  MariaDB-only delta는 후속.
...
- SQL semantic widening: MySQL/MariaDB `CALL`, `LOAD DATA`,
  `DELIMITER`/procedure body, transaction/control-flow scripting, plus broader
  `ON DUPLICATE KEY UPDATE` RHS expressions beyond literals/placeholders/
  `VALUES(column)`.
