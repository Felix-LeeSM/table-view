...
Last updated: 2026-05-16 (Sprint 353–376 plan 진입 — code smell audit 잔여 4건 등록: RISK-038/039/040/041)
...
| RISK-039 | `dataGridEditStore.entryKey(connId, schema, table)` 가 `workspaceStore` 의 `(connId, db)` 키 공간보다 얕음 — db 차원 누락. 다중 db 에서 같은 `public.users` 존재 시 db1 의 pending edit 가 db2 에 잘못 commit 될 invariant 누수 (audit L1, 🔴 High). Sprint 353–376 범위 밖 — `useDataGridEdit.key` (line 456) + `purgeKey` / `purgeForConnection` / `entryKey` API 갱신 ~5사이트 마이그레이션 필요 | active | frontend/logic | code-smell-audit-2026-05-15 (L1) | 종결: `entryKey(connId, db, schema, table)` 확장 또는 `entryKey(workspaceKey, tabId)` 상위 추상화 + reproduction 시나리오 회귀 잠금 |
