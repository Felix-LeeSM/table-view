...
과거 lesson/incident 기록을 보존하는 archive 다. 작업 시작 시 기본으로 읽지 않는다.
현재 적용 원칙은 `memory/product`, `memory/engineering`, `memory/workflow`,
`memory/runbook` 에 반영한다.
...
- 도메인 cross-cut incident 는 한 도메인에만 두고 인덱스에서 cross-link.
- 새 도메인 임계: 한 incident 만 든 도메인 신설 금지 (조기 분기). 2-3건 쌓이면 분기 검토.
