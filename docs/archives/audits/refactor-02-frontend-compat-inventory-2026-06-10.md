...
| `src/lib/sql/sqlSafetyClassifier.ts` | legacy regex safety classifier fallback | permanent-wire-compatibility | SQL Safe Mode classifier | Preserve until parser coverage proves every retained branch can be removed safely. | `src/lib/sql/sqlSafety.test.ts` | #737 characterization, #758 ledger |
