...
| `src/lib/sql/aliasColumnCompletion.ts` | legacy flat schema-list completion input | migration-only | SQL completion context | Normalize under completion feature boundary. | `src/lib/sql/sqlCompletionContext.test.ts` | #736 completion migration, #742 SOT |
| `src/lib/sql/cteColumnCompletion.ts` | legacy flat schema-list completion input | migration-only | SQL completion context | Normalize under completion feature boundary. | `src/lib/sql/sqlCompletionContext.test.ts` | #736 completion migration, #742 SOT |
| `src/lib/sql/sqlHybridCompletionSource.ts` | legacy CodeMirror completion source fallback list | migration-only | SQL completion adapter | Revisit when completion feature API is public and all callers route through it. | `src/lib/sql/sqlHybridCompletionSource.test.ts` | #736 completion migration |
...
| `src/lib/mongo/mongoshAst.ts` | backward-compat shim for mongosh AST exports | migration-only | Mongo query parser facade | Remove after query feature public API replaces parser compatibility facade. | `src/lib/mongo/mongoshAst.test.ts` | #762 and #764 remove query compatibility paths |
| `src/lib/mongo/mongoshParser.ts` | parser-dispatch compatibility facade and legacy error taxonomy | migration-only | Mongo query parser facade | Remove after Mongo query extraction routes through feature API. | `src/lib/mongo/mongoshParser.test.ts` | #762 and #764 remove query compatibility paths |
...
| `src/lib/sql/sqlAst.ts` | SqlWhereExpr backwards-compatible type alias | migration-only | SQL AST facade | Remove after SQL parser/completion consumers import canonical AST names. | `src/lib/sql/sqlAst.test.ts` | #736 and #737 migration |
...
| `src/components/query/QueryTab/useQueryExecution.ts` | legacy queryMode, history filter, and parser-dispatch compatibility | migration-only | query execution hook | Remove when query hook is thinned and dispatch lives behind query feature API. | `src/components/query/QueryTab/useQueryExecution.parserDispatch.test.tsx`<br>`src/components/query/QueryTab/useQueryExecution.runCommand.test.tsx` | #737, #761, #762, #763, #764 extraction and cleanup |
...
| `src/components/schema/CreateTableDialog.tsx` | legacy caller omission of workspace database | migration-only | create-table dialog | Remove after catalog/schema feature dialog slots pass workspace database explicitly. | `src/components/schema/CreateTableDialog.test.tsx` | #738 catalog/schema migration |
