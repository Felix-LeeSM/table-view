...
**재발 방지**:
- 외부 배포 plugin 에서 온 `.claude/skills/<x>/` 파일을 repo 작업 중 수정하지 않음.
- repo-owned runtime skill wrapper 는 memory source 를 가리키는 얇은 파일만 허용.
...
- 2026-05-20: repo-owned skill prune 이 필요해 `.claude/skills/**` hard block 은 제거.
  대신 review 정합성에서 외부 plugin 침범 여부를 판단한다.
...
- `.claude/skills/<x>/` 중 외부 배포 plugin 에서 온 파일
...
- `memory/workflow/review/memory.md` — 외부 plugin 침범 판단
