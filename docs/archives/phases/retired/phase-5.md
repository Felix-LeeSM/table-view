...
> execution order lives in `../../../ROADMAP.md`; completed/retired phase indexing
> lives in `../README.md`.
>
...
## F3.6: 데이터 상세 보기 (Quick Look) *(Sprint 56 완료, Sprint 61 확장)*
...
- [x] Cell Detail Dialog: 셀 더블클릭 또는 우클릭 → 전체 값 확인 + 클립보드 복사 (Sprint 61, DataGrid + raw query 결과 모두 지원)
...
## Raw Query 편집 *(Sprint 61 완료)*

- [x] raw query 결과에서 단일 테이블 SELECT + PK 조건 충족 시 Editable 배지 표시
- [x] Editable 상태에서 셀 더블클릭 → inline 편집, 우클릭 → 행 삭제
- [x] Commit 전 SQL Preview 모달 (Cmd+S)
- [x] Join/서브쿼리/UNION 등 복합 쿼리는 Read-only 배너 + 이유 표시
- [x] trailing `;` 붙은 raw query 문법 에러 수정 (`strip_trailing_terminator`)
