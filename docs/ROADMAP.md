...
| Search fixture slice | `src-tauri/src/db/search.rs`, `src-tauri/src/db/search_destructive.rs`, `src-tauri/src/db/search_dsl.rs`, `src-tauri/src/db/fixtures.rs`, `src-tauri/tests/fixture_harness.rs`, `src/lib/tauri/search.ts`, `src/components/search/SearchResultView.tsx` | Elasticsearch/OpenSearch fixture-backed result contracts exist. Elasticsearch live HTTP connection/catalog/query exists with backend-bounded Search DSL request validation, OpenSearch live HTTP connection/catalog exists, and fixture/live delete-by-query safety plans estimate matching docs before exact-target confirmation. OpenSearch live query and actual live Search admin execution remain unsupported. |
| OpenSearch live connection first slice | `src-tauri/src/db/search_http.rs`, `src-tauri/src/db/search.rs`, `src-tauri/src/db/search/tests.rs`, `src-tauri/src/commands/connection/crud.rs`, `src/types/connection.ts`, `src/types/dataSource.ts`, `src/components/connection/ConnectionDialog.test.tsx`, `src/components/connection/ConnectionDialog.urlInput.test.tsx`, #501 | OpenSearch connection UI/runtime now covers URL/auth/TLS root probe, product/version/distribution detection, Elasticsearch endpoint rejection, and user-facing auth/network failures. Catalog is promoted by #502; OpenSearch query/runtime smoke/admin execution remain future gates. |
| OpenSearch live catalog workbench slice | `src-tauri/src/db/search_http.rs`, `src-tauri/src/db/search.rs`, `src-tauri/src/db/search/tests.rs`, `src/types/dataSource.ts`, `src/components/workspace/SearchSidebar.test.tsx`, `src/components/search/SearchIndexDetailPanel.test.tsx`, `scripts/e2e-pre-smoke-release-gate.ts`, #502 | OpenSearch catalog UI/runtime now covers live indexes, aliases, data streams, mappings, settings/analyzers, composable/legacy templates, field paths, Search-native sidebar/detail rendering, and query/admin guardrails. OpenSearch live query, Runtime Happy Path smoke, actual admin execution, and observability remain future gates. |
| Elasticsearch support-claim closure audit | `docs/product/README.md`, `docs/product/query-language-support.md`, `docs/product/known-limitations.md`, `docs/contributor-guide/testing-and-quality.md`, #500 | Final support-claim audit confirms Elasticsearch support is live connection/catalog/query/destructive-plan safety plus wired Runtime Happy Path smoke. Search fixture files stay contract evidence, OpenSearch query remains fixture-only, and actual live admin execution, broader observability, profile/explain workflow, and editor parser/completion ownership remain future gates. |
...
| Search | Keep actual Elasticsearch admin execution and OpenSearch query fixture-backed or deferred. Elasticsearch live connection/catalog/query, backend-bounded Search DSL validation, delete-by-query safety planning, and OpenSearch live connection/catalog are active; promote broader live HTTP only after OpenSearch query, admin execution policy, observability/error surface, editor parser/completion, and product-specific delta contracts are explicit. |
...
8. OpenSearch live query and Search admin HTTP promotion.
