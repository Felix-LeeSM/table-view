...
쓴다. ERD 캔버스(React Flow + elkjs, #1655/ADR 0054), migration-impact
summaries, and cached read-only schema diff 는 이 shared graph/catalog path 를
재사용한다. Selected-table dependency readout 은 현재 pure selector 레이어에만
있고, ERD 표면 복귀는 후속 (#1657+) 이다. Future
...
| Reusable graph model | `src/lib/schemaGraph.ts`, `src/lib/schemaGraphRelationships.ts`, `src/components/schema/erdCanvasModel.ts`, `src/components/schema/erdCanvasLayout.ts`, `src/components/schema/erdCanvasModel.test.ts`, `src/components/schema/erdCanvasLayout.test.ts` | Graph extraction, relationship normalization, the pure `SchemaGraph` -> canvas model, and the elkjs `layered` placement adapter are reusable beyond one visual panel. The former layout/search/selection helper module was removed with the hand-rolled SVG renderer (#1655, ADR 0054). |
| Read-only dependency view | `src/lib/schemaGraphSelectors.ts`, `src/lib/schemaGraphSelectors.test.ts` | Selected-table dependency readouts (incoming/outgoing FK tables/columns, related indexes/constraints, CHECK expressions, metadata diagnostics) exist as pure `SchemaGraph` selectors, currently consumed by the migration-impact dialogs. The ERD-side dependency sidebar was removed with the canvas foundation (#1655) and returns through the ERD epic follow-ups (#1657 facet filter, #1666 inspector); no ERD dependency-panel claim ships today. |
...
| FK navigation boundary | `src/components/datagrid/DataGridTable.fk-navigation.test.tsx`, `docs/product/known-limitations.md` | Current FK row navigation is the DataGrid cell/icon path. ERD zoom/pan/fit-to-view are local canvas interactions, not row navigation claims; ERD selection/search/focus/highlight do not ship on the canvas foundation (#1655) and are tracked in #1657/#1666. |
...
| Smoke/verification matrix | `docs/contributor-guide/testing-and-quality.md`, `e2e/smoke/erd-dense.spec.ts` | Dense ERD desktop/narrow screenshot smoke is wired for seeded PostgreSQL canvas evidence: table nodes with columns, FK edges, a zoom-scale change, and fit-to-view. It does not claim ERD search/selection, FK row navigation through ERD, schema diff, migration impact, or data compare. |
...
| ERD/schema graph | 현재 schemaStore cache owner 범위는 schemas/tables/views/functions/postgresExtensions/tableColumnsCache/tableIndexesCache/tableConstraintsCache/triggers 이다. Production ERD/`SchemaGraph` input 은 schema/table/column cache 와 cached/fetched explicit index/constraint metadata 를 함께 쓰며, column-level FK info 는 synthetic fallback 으로 남아 있다. FK navigation 은 현재 DataGrid cell/icon path 이며 ERD interaction claim 이 아니다. ERD 는 React Flow + elkjs `layered` read-only 캔버스 (#1655, ADR 0054) — table node/columns, FK edge, zoom/pan/fit-to-view 까지다. Selected-table dependency readout (incoming/outgoing FK, index, constraint, CHECK expression, metadata diagnostics) 은 pure selector 레이어에 있고 ERD 표면 복귀는 #1657/#1666 후속이다. Migration impact summaries and read-only cached schema diff reuse the shared `SchemaGraph`/catalog input path. Dense ERD desktop/narrow screenshot smoke is wired for canvas render/zoom/fit evidence; follow-up 은 data compare (#1796) 다 — 2026-07-25 오너 grill 이 읽기전용 row diff 로 lock 했다 (쓰기/동기화 SQL 생성 없음, row cap 내에서만 비교하고 초과 시 잘림 배너). Duplicate catalog parsing 금지. |
