...
| H4 | RDBMS intelligence | ERD/schema diff/data compare/migration preview 는 같은 RDB catalog graph 를 재사용한다. | Production ERD 는 현재 `SchemaGraph` 의 schema/table/column graph 를 먼저 사용한다. H4 종료 신호는 explicit constraint/index graph input owner, dependency view, migration impact analysis 를 별도 follow-up 으로 연결하는 것이다. |
...
| ERD/schema graph | 현재 schemaStore cache owner 범위는 schemas/tables/views/functions/postgresExtensions/tableColumnsCache/triggers 이고, indexes/constraints 는 cache 가 아니라 live IPC delegate 로 소비한다. Follow-up 은 production `SchemaGraph` 의 explicit constraint/index input owner 를 정한 뒤 graph wiring, dependency view, migration impact analysis, dense-view screenshot smoke 를 연결하는 것이다 (#189, #200, smoke 는 #247). |
