...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  query/admin 은 deferred 로 유지한다. MSSQL 은 lifecycle-only connection probe,
  bounded T-SQL editor completion vocabulary, bounded static parser/Safe Mode
  metadata 까지만 active 이고 query/catalog/edit/runtime smoke 는 deferred 다.
  Oracle 은 bounded static SQL editor autocomplete vocabulary 만 active 로
  분리하고 runtime/query/catalog/edit/parser/Safe Mode promotion 은 planned
  상태로 둔다.
...
| H6 | 더 넓은 paradigm | Cassandra, DynamoDB, graph DB, vector DB, stream source 는 active work 전 명확한 workflow proof 가 필요하다. | MSSQL 은 SQL authentication connection test + `SERVERPROPERTY` version probe + bounded T-SQL editor completion vocabulary + bounded static parser/Safe Mode metadata 만 active 로 정렬되고, Oracle 은 bounded static editor autocomplete vocabulary 와 planned RDBMS identity 계약으로, wider source 는 candidate-only 계약으로 정렬된다. Profile target, connection kind, language, catalog model, result envelope, safety policy, fixture strategy 가 문서화되고 MSSQL connection probe/MSSQL editor-assistance completion/MSSQL static parser-Safe Mode metadata/Oracle editor-assistance vocabulary 밖의 runtime support claim 은 생기지 않는다. |
...
runtime support 로 승격하지 않는다.

| Gate | Current owner | H6 boundary |
|---|---|---|
| MSSQL lifecycle-plus-completion-plus-parser support | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/mssql.rs`, `src/lib/sql/sqlDialectProfile.test.ts`, `src/lib/sql/sqlCompletionRequest.test.ts`, `src/lib/sql/sqlDialectKeywords.test.ts`, `src/lib/sql/sqlSafety.test.ts`, `src/lib/sql/sqlWasmArtifact.test.ts`, `src-tauri/sql-parser-core/src/completion/mssql_completion_tests.rs`, `src-tauri/sql-parser-core/src/completion/token.rs`, `src-tauri/sql-parser-core/src/parser/tests.rs`, `src-tauri/tests/mssql_connection_routing.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs` | `mssql` has `server` connection kind, `sql` language, `rdb` catalog, `tabular` result, `rdb-default` safety, `declared-rdb` backend identity, SQL authentication connection testing, bounded TCP/TDS/version-probe timeouts, TLS required/trust-server-certificate routing, live Docker evidence for `SERVERPROPERTY('ProductVersion')`, bounded editor completion evidence for curated T-SQL keywords/functions, catalog procedure candidates, SQLCMD exclusion, bracket identifier quoting, and bounded static T-SQL parser/Safe Mode metadata for selected `SELECT`/DML/DDL slices plus unsupported scripting/admin boundaries. Runtime query/catalog/edit, desktop E2E smoke, instance discovery, runtime fixture smoke, runtime Safe Mode/destructive preview evidence, and full T-SQL semantic completion are not claimed. |
...
| Parser/completion/runtime non-claim | `docs/product/query-language-support.md`, `docs/product/known-limitations.md` | Deferred language ids do not create active parser, completion, connection, query, catalog, edit, or E2E claims by themselves. MSSQL connection probe, MSSQL editor-assistance completion, MSSQL static parser/Safe Mode metadata, and Oracle's bounded static editor autocomplete vocabulary are explicit exceptions; they do not create query/catalog/edit/runtime fixture/live/desktop E2E claims. Runtime changes must land in later source-specific PRs with matching smoke evidence. |
| H6 smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current E2E smoke proves PostgreSQL, MySQL, MariaDB, SQLite, DuckDB `.duckdb`, and MongoDB journeys only. MSSQL has focused connection-routing, live Docker probe, static parser/Safe Mode, and completion evidence, not current desktop E2E smoke. Oracle has focused profile/request/WASM/core completion evidence and a future runtime smoke inventory, not current desktop E2E claims. Wider candidates have future smoke inventories only. |
...
| MSSQL/Oracle | Keep MSSQL to lifecycle-only SQL authentication connection testing, `SERVERPROPERTY` version probe evidence, bounded editor completion vocabulary, and bounded static parser/Safe Mode metadata until a later PR splits query/catalog/edit/runtime smoke support. Keep Oracle capability-empty for runtime while allowing bounded static SQL editor autocomplete vocabulary only. Future promotion must split SQL Server and Oracle auth/dialect/catalog/safety contracts and add fixture/live evidence before any runtime support claim widens. |
