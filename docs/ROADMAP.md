...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  와 bounded T-SQL editor completion vocabulary 까지만 active 이고
  query/catalog/edit/parser/Safe Mode/runtime smoke 는 deferred 다. Oracle 은
  service-name connection-only lifecycle 과 bounded static SQL editor
  autocomplete vocabulary 까지만 active 이고
...
| H6 | 더 넓은 paradigm | Cassandra, DynamoDB, graph DB, vector DB, stream source 는 active work 전 명확한 workflow proof 가 필요하다. | MSSQL 은 SQL authentication connection test + `SERVERPROPERTY` version probe + bounded T-SQL editor completion vocabulary 만 active 로 정렬되고, Oracle 은 service-name connection test + bounded static editor autocomplete vocabulary 로 정렬된다. Wider source 는 candidate-only 계약으로 정렬된다. Profile target, connection kind, language, catalog model, result envelope, safety policy, fixture strategy 가 문서화되고 MSSQL connection probe/MSSQL editor-assistance completion/Oracle service-name lifecycle/Oracle editor-assistance vocabulary 밖의 runtime support claim 은 생기지 않는다. |
...
vocabulary 까지만 active 이고, Oracle 은 planned RDBMS identity 에 service-name
...
connection probe 및 Oracle service-name lifecycle 밖의 새 runtime support 를
출시하지 않으며, MSSQL/Oracle completion 은 runtime support 로 승격하지 않는다.

| Gate | Current owner | H6 boundary |
|---|---|---|
| MSSQL lifecycle-plus-completion support | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/mssql.rs`, `src/lib/sql/sqlDialectProfile.test.ts`, `src/lib/sql/sqlCompletionRequest.test.ts`, `src/lib/sql/sqlDialectKeywords.test.ts`, `src-tauri/sql-parser-core/src/completion/mssql_completion_tests.rs`, `src-tauri/sql-parser-core/src/completion/token.rs`, `src-tauri/tests/mssql_connection_routing.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs` | `mssql` has `server` connection kind, `sql` language, `rdb` catalog, `tabular` result, `rdb-default` safety, `declared-rdb` backend identity, SQL authentication connection testing, bounded TCP/TDS/version-probe timeouts, TLS required/trust-server-certificate routing, live Docker evidence for `SERVERPROPERTY('ProductVersion')`, and bounded editor completion evidence for curated T-SQL keywords/functions, catalog procedure candidates, SQLCMD exclusion, and bracket identifier quoting. Runtime query/catalog/edit, T-SQL parser/Safe Mode, desktop E2E smoke, instance discovery, runtime fixture smoke, and full T-SQL semantic completion are not claimed. |
