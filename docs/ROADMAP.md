...
| Credential/privacy boundary | `memory/engineering/architecture/state-management/memory.md`, `docs/product/README.md`, `docs/product/known-limitations.md` | Table View remains local-first. Connection export omits passwords, imported connections require password re-entry, and local file analytics public payloads redact absolute paths. 2026-07-25 오너 grill 이 실측한 실제 공백은 단위 커버리지가 아니라 e2e 부재다 — `src-tauri/src/commands/connection/io.rs` 단위 테스트가 세 불변식을 이미 덮지만 `e2e/smoke/` 에 import/export spec 이 0건이라 `ImportExportDialog` UI 배선이 끊겨도 통과한다. export/import 왕복 e2e + 키 경로 Rust integration 은 #1815 가 소유한다. Path C 디스크 fallback 을 거친 file-key 를 keyring 복귀 시 재키잉하는 작업은 #1814 다. |
