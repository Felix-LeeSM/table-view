...
| Fixture and E2E smoke | `e2e/fixtures/seed.sql`, `e2e/smoke/postgres.spec.ts`, `e2e/smoke/postgres-safe-mode.spec.ts`, `e2e/smoke/postgres-explain.spec.ts`, `e2e/smoke/postgres-extension-completion.spec.ts`, `scripts/e2e-smoke-ci.sh` | Runtime Happy Path currently proves PostgreSQL connect -> browse seeded `users` -> edit -> query result, Safe Mode info/warn/destructive confirmation, raw DDL preview/confirm, grid-edit preview/confirm, Explain plan-inspection/source-label, and installed-extension-gated completion for seeded `pgcrypto` on Ubuntu. It does not cover structured DDL flows, broader history-source labeling, cancellation, ERD, admin, or profiler/activity scenarios. |
...
| Runtime E2E smoke | `scripts/e2e-smoke-ci.sh`, `wdio.smoke.conf.ts`, `e2e/smoke/postgres.spec.ts`, `e2e/smoke/mongodb.spec.ts` | GitHub Runtime Happy Path builds the app on Ubuntu and executes PostgreSQL and MongoDB smoke specs only. Other specs under `e2e/smoke/**` are not automatically part of the remote runtime gate unless the smoke script wires them. |
