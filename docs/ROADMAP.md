...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  bounded SELECT/DML query runtime, and bounded T-SQL editor completion vocabulary
  까지만 active 이고 catalog/edit/parser/Safe Mode/runtime smoke 는 deferred 다. Oracle 은
  service-name connection-only lifecycle 과 bounded static SQL editor autocomplete
  vocabulary 까지만 active 이고
...
| H6 | 더 넓은 paradigm | Cassandra, DynamoDB, graph DB, vector DB, stream source 는 active work 전 명확한 workflow proof 가 필요하다. | MSSQL 은 SQL authentication connection test + `SERVERPROPERTY` version probe + bounded SELECT/DML query runtime + bounded T-SQL editor completion vocabulary 로 정렬되고, Oracle 은 service-name connection test + bounded static editor autocomplete vocabulary 로 정렬된다. Wider source 는 candidate-only 계약으로 정렬된다. Profile target, connection kind, language, catalog model, result envelope, safety policy, fixture strategy 가 문서화되고 MSSQL catalog/edit/parser/Safe Mode/fixture-smoke/E2E/Oracle catalog/query/edit/SID/TNS/wallet/TLS 밖의 runtime support claim 은 생기지 않는다. |
...
Server `SERVERPROPERTY` version probe, bounded SELECT/DML query runtime, and
bounded T-SQL editor completion vocabulary 까지만 active 이고, Oracle 은 service-name
connection lifecycle 과 bounded static SQL editor autocomplete vocabulary 만 더한다.
...
catalog/edit/parser/Safe Mode/runtime fixture/E2E support 및 Oracle
catalog/query/edit/SID/TNS/wallet/TLS support 를 출시하지 않으며, MSSQL/Oracle
completion 은 runtime support 로 승격하지 않는다.

| Gate | Current owner | H6 boundary |
|---|---|---|
| MSSQL bounded query support | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/mssql.rs`, `src-tauri/src/db/mssql/runtime.rs`, `src/lib/sql/sqlDialectProfile.test.ts`, `src/lib/sql/sqlCompletionRequest.test.ts`, `src/lib/sql/sqlDialectKeywords.test.ts`, `src-tauri/sql-parser-core/src/completion/mssql_completion_tests.rs`, `src-tauri/sql-parser-core/src/completion/token.rs`, `src-tauri/tests/mssql_connection_routing.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs` | `mssql` has `server` connection kind, `sql` language, `rdb` catalog, `tabular` result, `rdb-default` safety, RelationalQuery backend capability, SQL authentication connection testing, bounded TCP/TDS/version-probe timeouts, TLS required/trust-server-certificate routing, live Docker evidence for `SERVERPROPERTY('ProductVersion')`, bounded SELECT tabular envelopes, DML/DDL execution, transactional DML batch commit/dry-run, cancellation/error surfacing, and bounded editor completion evidence for curated T-SQL keywords/functions, catalog procedure candidates, SQLCMD exclusion, and bracket identifier quoting. Runtime catalog/edit, T-SQL parser/Safe Mode, desktop E2E smoke, instance discovery, runtime fixture smoke, row-edit key projection, and full T-SQL semantic completion are not claimed. |
...
| Parser/completion/runtime non-claim | `docs/product/query-language-support.md`, `docs/product/known-limitations.md` | Deferred language ids do not create active parser, completion, connection, query, catalog, edit, or E2E claims. Oracle's bounded static editor autocomplete vocabulary is the explicit editor-assistance exception and does not create query/catalog/edit, parser/Safe Mode, fixture, live, or desktop E2E claims. Broader runtime changes must land in later source-specific PRs with matching smoke evidence. |
| H6 smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current E2E smoke proves PostgreSQL, MySQL, MariaDB, SQLite, DuckDB `.duckdb`, and MongoDB journeys only. MSSQL has focused connection-routing, live Docker probe, and bounded query runtime evidence, not current desktop E2E smoke. Oracle has focused service-name connection evidence plus profile/request/WASM/core completion evidence, not current desktop E2E claims. Wider candidates have future smoke inventories only. |
...
| MSSQL/Oracle | MSSQL now has lifecycle-backed SQL authentication, `SERVERPROPERTY` version probe evidence, bounded editor completion vocabulary, and query runtime/result-envelope support. Keep MSSQL catalog/edit/parser/Safe Mode widening split behind their own evidence and smoke routing. Keep Oracle to service-name connection lifecycle plus bounded static SQL editor autocomplete vocabulary until follow-up issues split SID/TNS/wallet/TLS, dialect/catalog/query/edit/safety contracts and add fixture/live evidence before any support claim widens. |
...
2. connect/browse/query 만 노출하는 runtime 을 하나 더 붙이는 것보다, 기존 runtime
   깊이를 우선한다.
3. Runtime promotion freeze: Search admin execution, remaining MSSQL catalog/edit/parser
