...
- 새 `DatabaseType` 추가는 기존 지원 DBMS 하나가 데스크톱 DB 클라이언트 수준의
  ...
  #1072 로 그 runtime slice 를 해소한 full adapter 로 연다. SQL Server auth/TDS/T-SQL
...
| Oracle runtime slice guardrail | `docker-compose.yml`, `scripts/fixtures/oracle.ts`, `scripts/fixtures/oracle.test.ts`, `src/types/dataSource.ts`, `src/types/connection.ts`, `src/types/adapterConformance.test.ts`, `src-tauri/src/models/data_source.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs`, `src-tauri/src/db/oracle.rs`, `src-tauri/src/db/oracle/catalog.rs`, `src-tauri/tests/oracle_integration.rs`, `src/lib/sql/oracleSafety.test.ts`, `src/components/datagrid/sqlGenerator.dialects.test.ts`, `.github/workflows/e2e-smoke.yml`, `scripts/e2e-smoke-ci.sh`, `e2e/smoke/oracle.spec.ts`, `docs/product/README.md`, `docs/product/query-language-support.md`, `docs/product/known-limitations.md`, this roadmap, #905/#906/#907/#1072 | `oracle` keeps source-specific identity, backend adapter id, dialect id, labels, service-name defaults, URL parsing, and seed/spec inventory. #905 allows service-name lifecycle, catalog metadata, SELECT/DML batch execution, cooperative cancellation, and tabular table-data query through a bounded wrapper. #906 adds key-projected editRows through the frontend SQL batch path, tested SELECT/DML/DDL Safe Mode classification, and bounded editor assistance. #907 wires representative Runtime Happy Path smoke for service-name connect, seeded catalog/routine browse, SELECT/DML, destructive Safe Mode confirmation, cancellation, and grid edit. #1072 dissolves the bounded wrapper into the full adapter: switch database (a service-name re-dial — Oracle has no `USE <db>` — whose picker offers only names the same credentials can dial, with SID profiles failing closed), bounded structured table/index/constraint DDL, and read-only trigger listing are claimed. TNS, wallet-less TLS, advanced auth, raw DDL/admin, full parser/completion promotion, and PLSQL body/package authoring stay unclaimed until matching evidence lands. |
...
H6 umbrella closure means MSSQL bounded runtime/smoke, the Oracle full adapter
(#1072 dissolves the runtime/edit/Safe Mode slice), wider
candidate contracts, and smoke routing are aligned. It does not mean MSSQL
DDL/parser/completion/admin widening, or Oracle raw DDL/full-parser-completion/
raw-admin widening, has shipped.
