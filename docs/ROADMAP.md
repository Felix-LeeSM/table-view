...
| OpenSearch destructive plan workflow | `src-tauri/src/db/search.rs`, `src-tauri/src/db/search_destructive.rs`, `src-tauri/src/db/search_live_destructive.rs`, `src-tauri/src/db/search/tests/destructive.rs`, `docs/product/README.md`, `docs/product/query-language-support.md`, `docs/product/known-limitations.md`, `docs/contributor-guide/testing-and-quality.md`, #505 | OpenSearch live delete-by-query safety planning now matches the Elasticsearch plan workflow: preview uses a safe `_search` estimate with `size: 0` and `track_total_hits`, unsupported body/target shapes reject before dispatch, and execution is unsupported. Actual live `_delete_by_query` execution remains deferred. |
