...
compatibility, Redis full UI/editor parity, Search live HTTP, or MongoDB
...
| Non-RDBMS runtime | Redis 와 MongoDB 는 runtime slice 가 있다. Valkey 는 connection/key scan/value preview, bounded command query runtime slice, proven-row completion, Runtime Happy Path smoke 가 있다. Elasticsearch/OpenSearch 는 live HTTP 전까지 fixture-backed 다. Cassandra/Scylla, DynamoDB, graph, vector, stream 은 gated candidate 다. 새 runtime promotion 은 active one-DBMS parity lane 뒤로 둔다. | `memory/engineering/architecture/data-source/memory.md`, `docs/phases/phase-28.md` |
...
| Search | Keep Elasticsearch/OpenSearch catalog/query fixture-backed. Elasticsearch live connection root probe is active; promote broader live HTTP only after OpenSearch connection, catalog/search execution, response parsing, admin/destructive policy, observability/error surface, and product-specific delta contracts are explicit. |
...
8. Elasticsearch/OpenSearch live catalog/query HTTP promotion.
