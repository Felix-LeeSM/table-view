...
| Search fixture slice | `src-tauri/src/db/search.rs`, `src-tauri/src/db/search_dsl.rs`, `src-tauri/src/db/fixtures.rs`, `src-tauri/tests/fixture_harness.rs`, `src/lib/tauri/search.ts`, `src/components/search/SearchResultView.tsx` | Elasticsearch/OpenSearch fixture-backed result and destructive plan contracts exist. Elasticsearch live HTTP connection/catalog/query exists with backend-bounded Search DSL request validation; live admin remains unsupported. |
...
| Search | Keep Elasticsearch admin and OpenSearch catalog/query fixture-backed or deferred. Elasticsearch live connection/catalog/query and backend-bounded Search DSL validation are active; promote broader live HTTP only after OpenSearch connection/catalog/query, admin/destructive policy, observability/error surface, editor parser/completion, and product-specific delta contracts are explicit. |
