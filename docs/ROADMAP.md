...
| H5 | First-class non-RDBMS | Redis/Valkey, Elasticsearch/OpenSearch, MongoDB 가 가장 명확한 non-RDBMS 사용자 workflow 를 덮는다. | MongoDB 는 whitelisted document workflow 로, Redis 는 bounded KV browser/value mutation + command/query/completion + representative smoke 로, Valkey 는 connection/key scan/value preview + bounded command query runtime slice + proven-row command completion + Runtime Happy Path smoke 로, Elasticsearch 는 live connection/catalog + fixture-backed query/admin slice 로, OpenSearch 는 fixture-backed Search slice 로 support claim 이 정렬돼 있다. Search live query/admin 은 active parity lane 을 약화시키지 않고 promotion gate 를 통과할 때까지 deferred 다. |
...
| Search fixture slice | `src-tauri/src/db/search.rs`, `src-tauri/src/db/fixtures.rs`, `src-tauri/tests/fixture_harness.rs`, `src/lib/tauri/search.ts`, `src/components/search/SearchResultView.tsx` | Elasticsearch/OpenSearch fixture-backed search result and destructive plan contracts exist. Elasticsearch live HTTP connection/catalog exists; live query/admin remains unsupported. |
| Search live query/admin promotion | `docs/product/query-language-support.md`, `docs/product/known-limitations.md`, `docs/contributor-guide/testing-and-quality.md` | Future promotion must cover OpenSearch connection/catalog, Elasticsearch/OpenSearch search execution and response parsing, admin destructive policy, observability/error surface, CI fixture/live smoke, and active RDBMS lane freeze. |
...
| Non-RDBMS runtime | Redis 와 MongoDB 는 runtime slice 가 있다. Valkey 는 connection/key scan/value preview, bounded command query runtime slice, proven-row completion, Runtime Happy Path smoke 가 있다. Elasticsearch 는 live connection/catalog + fixture-backed query/admin slice 가 있고, OpenSearch 는 fixture-backed 다. Cassandra/Scylla, DynamoDB, graph, vector, stream 은 gated candidate 다. 새 runtime promotion 은 active one-DBMS parity lane 뒤로 둔다. | `memory/engineering/architecture/data-source/memory.md`, `docs/phases/phase-28.md` |
...
| Search | Keep Elasticsearch query/admin and OpenSearch catalog/query fixture-backed or deferred. Elasticsearch live connection/catalog is active; promote broader live HTTP only after OpenSearch connection/catalog, Elasticsearch/OpenSearch search execution, response parsing, admin/destructive policy, observability/error surface, and product-specific delta contracts are explicit. |
...
8. OpenSearch live catalog and Elasticsearch/OpenSearch live query/admin HTTP promotion.
