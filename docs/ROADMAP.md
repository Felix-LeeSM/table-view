...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  browse, live cached catalog-aware T-SQL editor completion, and bounded static
  parser/Safe Mode metadata 까지만 active 이고 row edit/runtime Safe Mode smoke/
...
envelope, catalog/workbench metadata browse, bounded T-SQL editor completion
...
row edit/runtime Safe Mode smoke/runtime fixture/E2E/TLS-required workflow support 및 Oracle
...
|---|---|---|
| MSSQL bounded query-plus-catalog-plus-parser support | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/mssql.rs`, `src-tauri/src/db/mssql/runtime.rs`, `src-tauri/src/db/mssql/catalog.rs`, `src-tauri/src/db/mssql/catalog/*`, `src/lib/sql/sqlDialectProfile.test.ts`, `src/lib/sql/sqlCompletionContext.test.ts`, `src/lib/sql/sqlCompletionRequest.test.ts`, `src/lib/sql/sqlCompletionWasm.test.ts`, `src/lib/sql/sqlHybridCompletionSource.test.ts`, `src/lib/sql/sqlDialectKeywords.test.ts`, `src/lib/sql/sqlSafety.test.ts`, `src/lib/sql/sqlWasmArtifact.test.ts`, `src-tauri/sql-parser-core/src/completion/mssql_completion_tests.rs`, `src-tauri/sql-parser-core/src/completion/token.rs`, `src-tauri/sql-parser-core/src/parser/tests.rs`, `src-tauri/tests/mssql_connection_routing.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs`, #516 | `mssql` has `server` connection kind, `sql` language, `rdb` catalog, `tabular` result, `rdb-default` safety, RelationalCatalog and RelationalQuery backend capability, SQL authentication connection testing, bounded TCP/TDS/version-probe timeouts, TLS required/trust-server-certificate routing, live Docker evidence for `SERVERPROPERTY('ProductVersion')`, bounded SELECT tabular envelopes, DML/DDL execution, transactional DML batch commit/dry-run, cancellation/error surfacing, catalog/workbench metadata for databases/schemas/tables/views/procedures/functions/columns/indexes/constraints/FKs, live cached catalog-aware completion for database/schema/table/view/procedure/column candidates, curated T-SQL keywords/functions, SQLCMD exclusion, bracket identifier quoting, and bounded static T-SQL parser/Safe Mode metadata for selected `SELECT`/DML/DDL slices plus unsupported scripting/admin boundaries. Row edit, desktop E2E smoke, instance discovery, runtime fixture smoke, runtime Safe Mode/destructive preview evidence, TLS-required workflow evidence, row-edit key projection, SQLCMD/admin execution support, and full T-SQL semantic completion are not claimed. |
...
| Parser/completion/runtime non-claim | `docs/product/query-language-support.md`, `docs/product/known-limitations.md` | Deferred language ids do not create active parser, completion, connection, query, catalog, edit, or E2E claims by themselves. MSSQL connection probe, bounded query runtime/result envelope, catalog/workbench metadata browse, editor-assistance completion, static parser/Safe Mode metadata, and Oracle service-name connection plus bounded SELECT/DML query runtime/result envelope plus bounded static editor autocomplete vocabulary plus bounded static parser/Safe Mode metadata are explicit exceptions; they do not create MSSQL row edit/runtime Safe Mode smoke/runtime fixture/live/desktop E2E/TLS-required workflow or Oracle catalog/edit/structured DDL/runtime Safe Mode smoke/runtime fixture/live/desktop E2E claims. Runtime changes must land in later source-specific PRs with matching smoke evidence. |
| H6 smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current E2E smoke proves PostgreSQL, MySQL, MariaDB, SQLite, DuckDB `.duckdb`, and MongoDB journeys only. MSSQL has focused connection-routing, live Docker probe, bounded query runtime/result-envelope, catalog/workbench metadata, static parser/Safe Mode, and completion evidence, not current desktop E2E smoke. Oracle has focused service-name connection evidence, bounded backend query runtime unit/profile evidence, static parser/Safe Mode evidence, and profile/request/WASM/core completion evidence, not fixture/live or current desktop E2E claims. Wider candidates have future smoke inventories only. |
...
| MSSQL/Oracle | MSSQL now has lifecycle-backed SQL authentication, `SERVERPROPERTY` version probe evidence, bounded query runtime/result-envelope support, catalog/workbench metadata browse, live cached catalog-aware editor completion, and bounded static parser/Safe Mode metadata. Oracle now has service-name connection lifecycle, bounded SELECT/DML query runtime/result-envelope support, bounded static SQL editor autocomplete vocabulary, and bounded static parser/Safe Mode metadata. Keep MSSQL row edit/runtime Safe Mode smoke/runtime fixture/E2E/TLS-required workflow/SQLCMD/admin/full T-SQL semantic widening split behind their own evidence and smoke routing. Keep Oracle catalog/edit/structured DDL/runtime Safe Mode smoke/SID/TNS/wallet/TLS/fixture/live/E2E/full PL/SQL executable-semantics widening split behind source-specific evidence before any support claim widens. |
...
2. connect/browse/query 만 노출하는 runtime 을 하나 더 붙이는 것보다, 기존 runtime
   깊이를 우선한다.
3. Runtime promotion freeze: Search admin execution, remaining MSSQL row edit/runtime
