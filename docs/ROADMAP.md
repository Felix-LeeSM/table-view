...
| Valkey bounded command runtime | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src/components/connection/ConnectionDialog/ConnectionDialogBody.tsx`, `src/components/workspace/KvSidebar.tsx`, `src/components/query/QueryTab/useQueryExecution.ts`, `src/lib/redis/redisCommandCompletion.ts`, `src/components/query/RedisCommandEditor.tsx`, `src-tauri/src/commands/connection.rs`, `src-tauri/src/db/redis/mod.rs`, `src-tauri/src/models/data_source.rs`, `src-tauri/tests/redis_integration.rs`, `docs/product/README.md`, `docs/product/query-language-support.md`, `docs/product/known-limitations.md`, #488/#489/#490 | Active `valkey` `DatabaseType`/profile identity now has server connection kind, product label, KV paradigm, Valkey backend adapter, connection/key-browser/value-preview capabilities, `query.query` capability, focused local testcontainer evidence for connect/key scan/value preview plus bounded Redis-compatible command query dispatch, and TypeScript command completion for proven local-runtime rows with current-keyspace hints. Direct mutation controls, Runtime Happy Path smoke, and full Redis compatibility remain future gates. |
| Valkey fixture/live evidence strategy | `e2e/fixtures/seed.valkey.json`, `scripts/fixtures/dbms-seeds.test.ts`, `src-tauri/tests/redis_integration.rs`, `docs/contributor-guide/testing-and-quality.md`, `docs/product/README.md`, `docs/product/known-limitations.md`, #486/#488/#489/#490 | Valkey static Redis-compatible deterministic seed/reset contract remains fixture inventory. Current runtime support is local-first and covers connection/key scan/value preview plus selected read/write/TTL/stream/destructive command query families; current completion support is editor assistance for the proven local-runtime rows only. Future promotion must wire Valkey-specific smoke and separately prove direct mutation controls, broader Redis compatibility, and full compatibility before those claims widen. |
| Valkey Redis compatibility matrix | `e2e/fixtures/valkey.redis-compatibility.json`, `scripts/fixtures/dbms-seeds.test.ts`, `docs/product/query-language-support.md`, `docs/product/README.md`, `docs/product/known-limitations.md`, `docs/contributor-guide/testing-and-quality.md`, #487/#489/#490 | Matrix covers the bounded Redis command families, fixture reset commands, Valkey identity-detection requirement, known Table View Valkey deltas, and rejected Redis assumptions. Local Valkey runtime evidence marks only selected rows as proven; command completion uses that subset and keeps key hints hidden for unpromoted families. Admin/server-control, broad destructive, cluster, pub/sub, modules/functions, scripting, consumer-group workflows, direct mutation UI, smoke, and full compatibility remain separate promotion gates. |
...
| Non-RDBMS smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current desktop E2E claim covers MongoDB and Redis representative workflows. Search E2E scenarios are inventoried as future promotion gates; Valkey has no smoke claim. |

H5 umbrella closure means support claims, runtime contracts, and smoke routing are
aligned for non-RDBMS sources. It does not mean Valkey smoke/full compatibility,
Redis full UI/editor parity, Search live HTTP, or MongoDB full-support parity has
shipped.
...
| Non-RDBMS runtime | Redis 와 MongoDB 는 runtime slice 가 있다. Valkey 는 connection/key scan/value preview 와 bounded command query runtime slice 및 static fixture inventory 가 있다. Elasticsearch/OpenSearch 는 live HTTP 전까지 fixture-backed 다. Cassandra/Scylla, DynamoDB, graph, vector, stream 은 gated candidate 다. 새 runtime promotion 은 active one-DBMS parity lane 뒤로 둔다. | `memory/engineering/architecture/data-source/memory.md`, `docs/phases/phase-28.md` |
...
| Redis/Valkey | Redis first slice is backend KV primitives, key browser/value preview/edit UI, bounded command query runtime/completion, current-DB/type-filtered key suggestions, and representative connect/scan/preview/GET/guarded-write/TTL/delete smoke. Valkey first slice is connection/key scan/value preview plus bounded command query with focused local testcontainer evidence and proven-row command completion. Define contracts and evidence for stream consumer UI, full CLI/admin parity, language-core parser/completion ownership, broader command coverage, cluster, pub/sub, modules, consumer-group management, Valkey direct mutation controls, Valkey smoke, and full Redis compatibility before broader support claims. |
