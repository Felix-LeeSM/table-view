...
| DuckDB test coverage recheck | `docs/contributor-guide/testing-and-quality.md`, `src-tauri/tests/duckdb_browse_query_adapter.rs`, `src-tauri/tests/duckdb_file_analytics.rs`, `src-tauri/src/db/duckdb.rs`, `src/lib/sql/sqlCompletionRequest.test.ts`, `src-tauri/sql-parser-core/src/completion/completion_tests.rs`, `scripts/e2e-smoke-ci.sh`, `e2e/smoke/duckdb.spec.ts`, #536 | Final test recheck maps DuckDB runtime/query/source-equivalent backend tests, parser/Safe Mode unsupported-boundary tests, autocomplete vocabulary/context tests, fixture/live evidence, and smoke routing before parity closure. This is evidence mapping only; #469 remains a separate promotion gate and is not closed by this documentation update. |
