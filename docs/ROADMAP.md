...
| Fixture and E2E smoke | `e2e/fixtures/seed.sql`, `e2e/smoke/postgres.spec.ts`, `e2e/smoke/postgres-safe-mode.spec.ts`, `scripts/e2e-smoke-ci.sh` | Runtime Happy Path currently proves PostgreSQL connect -> browse seeded `users` -> edit -> query result, plus Safe Mode info/warn/destructive, raw DDL preview, and grid-edit confirmation paths on Ubuntu. It does not cover extension completion, structured DDL flows, Explain UI, history-source labeling, cancellation, ERD, admin, or profiler scenarios. |
