...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  query/admin 은 deferred 로 유지하고, MSSQL 은 bounded static parser/Safe Mode
  boundary 가 있는 planned identity 로, Oracle 은 planned identity 로 유지한다.
...
|---|---|---|
| MSSQL planned RDBMS identity | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/tests/backend_adapter_contract_profile.rs`, `src-tauri/sql-parser-core/src/parser/tests.rs`, `src/lib/sql/sqlSafety.test.ts` | `mssql` has `server` connection kind, `sql` language, `rdb` catalog, `tabular` result, `rdb-default` safety, `declared-rdb` backend identity, and bounded static T-SQL parser/Safe Mode metadata for selected `SELECT`/DML/DDL slices plus unsupported scripting/admin boundaries. Capabilities are empty; connection UI, runtime query/catalog/edit, completion, SQL Server auth/TLS/encryption/instance behavior, runtime fixture/live evidence, and desktop E2E smoke are not claimed. |
...
| MSSQL/Oracle | Keep MSSQL as a capability-empty declared RDBMS identity with bounded static parser/Safe Mode metadata only; keep Oracle as a capability-empty declared RDBMS identity. Future promotion must split SQL Server and Oracle connection/auth/dialect/catalog/safety contracts and add fixture/live evidence before any runtime support claim widens. |
