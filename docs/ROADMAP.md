...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  까지만 active 이고 catalog/edit/parser/Safe Mode/runtime smoke 는 deferred, Oracle 은 bounded
  static SQL editor autocomplete vocabulary 만 active 로 분리하고
...
bounded T-SQL editor completion vocabulary 까지만 active 이고, Oracle 은 planned
RDBMS identity 에 bounded static SQL editor autocomplete vocabulary 만 더한다.
...
MSSQL catalog/edit/parser/Safe Mode/runtime fixture/E2E support 를 출시하지 않으며,
MSSQL/Oracle completion 은 runtime support 로 승격하지 않는다.
...
| Parser/completion/runtime non-claim | `docs/product/query-language-support.md`, `docs/product/known-limitations.md` | Deferred language ids do not create active parser, completion, connection, query, catalog, edit, or E2E claims. Oracle's bounded static editor autocomplete vocabulary is the explicit editor-assistance exception and does not create runtime, parser/Safe Mode, fixture, live, or desktop E2E claims. Runtime changes must land in later source-specific PRs with matching smoke evidence. |
...
| MSSQL/Oracle | MSSQL now has lifecycle-backed SQL authentication, `SERVERPROPERTY` version probe evidence, bounded editor completion vocabulary, and query runtime/result-envelope support. Keep MSSQL catalog/edit/parser/Safe Mode widening split behind their own evidence and smoke routing. Keep Oracle capability-empty for runtime while allowing bounded static SQL editor autocomplete vocabulary only. Future promotion must split SQL Server and Oracle auth/dialect/catalog/safety contracts and add fixture/live evidence before any runtime support claim widens. |
