...
| H5 | First-class non-RDBMS | Redis/Valkey, Elasticsearch/OpenSearch, MongoDB 가 가장 명확한 non-RDBMS 사용자 workflow 를 덮는다. | MongoDB 는 whitelisted document workflow 로, Redis 는 bounded KV browser/value mutation + command/query/completion + representative smoke 로, Valkey 는 connection/key scan/value preview + bounded command query runtime slice + proven-row command completion + Runtime Happy Path smoke 로, Elasticsearch 는 live connection/catalog/query + backend-bounded Search DSL validator + TypeScript editor completion + Runtime Happy Path smoke + fixture/live delete-by-query safety planning 으로, OpenSearch 는 fixture-backed Search slice 로 support claim 이 정렬돼 있다. Actual Search live admin execution, OpenSearch live query/catalog/completion, and full editor parser ownership 은 active parity lane 을 약화시키지 않고 promotion gate 를 통과할 때까지 deferred 다. |
...
| Search fixture slice | `src-tauri/src/db/search.rs`, `src-tauri/src/db/search_destructive.rs`, `src-tauri/src/db/search_dsl.rs`, `src-tauri/src/db/fixtures.rs`, `src-tauri/tests/fixture_harness.rs`, `src/lib/tauri/search.ts`, `src/lib/search/searchDslCompletion.ts`, `src/components/query/SearchQueryEditor.tsx`, `src/components/search/SearchResultView.tsx`, `e2e/smoke/elasticsearch.spec.ts` | Elasticsearch/OpenSearch fixture-backed result contracts exist. Elasticsearch live HTTP connection/catalog/query exists with backend-bounded Search DSL request validation including supported sort/source-filter keys, Elasticsearch-only TypeScript editor completion, a wired Runtime Happy Path smoke, and fixture/live delete-by-query safety plans that estimate matching docs before exact-target confirmation. Actual live Search admin execution remains unsupported. |
| Search live query/admin promotion | `docs/product/query-language-support.md`, `docs/product/known-limitations.md`, `docs/contributor-guide/testing-and-quality.md` | Future promotion must cover OpenSearch connection/catalog/query, admin destructive policy, observability/error surface expansion, full parser ownership, and active RDBMS lane freeze. |
...
| Non-RDBMS runtime | Redis 와 MongoDB 는 runtime slice 가 있다. Valkey 는 connection/key scan/value preview, bounded command query runtime slice, proven-row completion, Runtime Happy Path smoke 가 있다. Elasticsearch 는 live connection/catalog/query, bounded editor completion, Runtime Happy Path smoke, fixture-backed admin planning slice 가 있고, OpenSearch 는 fixture-backed 다. Cassandra/Scylla, DynamoDB, graph, vector, stream 은 gated candidate 다. 새 runtime promotion 은 active one-DBMS parity lane 뒤로 둔다. | `memory/engineering/architecture/data-source/memory.md`, `docs/phases/phase-28.md` |
...
| Search | Keep actual Elasticsearch admin execution and OpenSearch catalog/query fixture-backed or deferred. Elasticsearch live connection/catalog/query, backend-bounded Search DSL validation, Elasticsearch-only TypeScript editor completion, wired representative smoke, and delete-by-query safety planning are active; promote broader live HTTP only after OpenSearch connection/catalog/query, admin execution policy, observability/error surface, full editor parser ownership, and product-specific delta contracts are explicit. |
