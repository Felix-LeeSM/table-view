...
| Non-RDBMS runtime | Redis 와 MongoDB 는 runtime slice 가 있다. Valkey 는 connection/key scan/value preview, bounded command query runtime slice, proven-row completion, Runtime Happy Path smoke 가 있다. Elasticsearch 는 live connection/catalog/query + fixture-backed admin slice 가 있고, OpenSearch 는 live connection + fixture-backed Search contract 다. Cassandra/Scylla, DynamoDB, graph, vector, stream 은 gated candidate 다. 새 runtime promotion 은 active one-DBMS parity lane 뒤로 둔다. | `memory/engineering/architecture/data-source/memory.md`, `docs/phases/phase-28.md` |
...
| Search | Keep actual Elasticsearch admin execution and OpenSearch catalog/query fixture-backed or deferred. Elasticsearch live connection/catalog/query, backend-bounded Search DSL validation, delete-by-query safety planning, and OpenSearch live connection root probe are active; promote broader live HTTP only after OpenSearch catalog/query, admin execution policy, observability/error surface, editor parser/completion, and product-specific delta contracts are explicit. |
...
8. OpenSearch live catalog/query and Search admin HTTP promotion.
