...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  browse, bounded T-SQL editor completion vocabulary, and bounded static parser/Safe
  Mode metadata 까지만 active 이고 row edit/runtime Safe Mode smoke/runtime fixture/
  E2E/TLS-required workflow 는 deferred 다. Oracle 은 service-name connection
...
| H6 | 더 넓은 paradigm | Cassandra, DynamoDB, graph DB, vector DB, stream source 는 active work 전 명확한 workflow proof 가 필요하다. | MSSQL 은 SQL authentication connection test + `SERVERPROPERTY` version probe + bounded SELECT/DML query runtime/result envelope + catalog/workbench metadata browse + bounded T-SQL editor completion vocabulary + bounded static parser/Safe Mode metadata 로 정렬되고, Oracle 은 service-name connection test + bounded SELECT/DML query runtime/result envelope + bounded static editor autocomplete vocabulary + bounded static parser/Safe Mode metadata 로 정렬된다. Wider source 는 candidate-only 계약으로 정렬된다. Profile target, connection kind, language, catalog model, result envelope, safety policy, fixture strategy 가 문서화되고 MSSQL row edit/runtime Safe Mode smoke/runtime fixture/E2E/TLS-required workflow/Oracle catalog/edit/structured DDL/runtime Safe Mode smoke/SID/TNS/wallet/TLS/fixture/live/E2E 밖의 runtime support claim 은 생기지 않는다. |
...
|---|---|---|
| MSSQL bounded query-plus-catalog-plus-parser support | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/mssql.rs`, `src-tauri/src/db/mssql/runtime.rs`, `src-tauri/src/db/mssql/catalog.rs`, `src-tauri/src/db/mssql/catalog/*`, `src/lib/sql/sqlDialectProfile.test.ts`, `src/lib/sql/sqlCompletionRequest.test.ts`, `src/lib/sql/sqlDialectKeywords.test.ts`, `src/lib/sql/sqlSafety.test.ts`, `src/lib/sql/sqlWasmArtifact.test.ts`, `src-tauri/sql-parser-core/src/completion/mssql_completion_tests.rs`, `src-tauri/sql-parser-core/src/completion/token.rs`, `src-tauri/sql-parser-core/src/parser/tests.rs`, `src-tauri/tests/mssql_connection_routing.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs` | `mssql` has `server` connection kind, `sql` language, `rdb` catalog, `tabular` result, `rdb-default` safety, RelationalCatalog and RelationalQuery backend capability, SQL authentication connection testing, bounded TCP/TDS/version-probe timeouts, TLS required/trust-server-certificate routing, live Docker evidence for `SERVERPROPERTY('ProductVersion')`, bounded SELECT tabular envelopes, DML/DDL execution, transactional DML batch commit/dry-run, cancellation/error surfacing, catalog/workbench metadata for databases/schemas/tables/views/procedures/columns/indexes/constraints/FKs, bounded editor completion evidence for curated T-SQL keywords/functions, catalog procedure candidates, SQLCMD exclusion, bracket identifier quoting, and bounded static T-SQL parser/Safe Mode metadata for selected `SELECT`/DML/DDL slices plus unsupported scripting/admin boundaries. Row edit, desktop E2E smoke, instance discovery, runtime fixture smoke, runtime Safe Mode/destructive preview evidence, TLS-required workflow evidence, row-edit key projection, and full T-SQL semantic completion are not claimed. |
...
| MSSQL/Oracle | MSSQL now has lifecycle-backed SQL authentication, `SERVERPROPERTY` version probe evidence, bounded query runtime/result-envelope support, catalog/workbench metadata browse, bounded editor completion vocabulary, and bounded static parser/Safe Mode metadata. Oracle now has service-name connection lifecycle, bounded SELECT/DML query runtime/result-envelope support, bounded static SQL editor autocomplete vocabulary, and bounded static parser/Safe Mode metadata. Keep MSSQL row edit/runtime Safe Mode smoke/runtime fixture/E2E/TLS-required workflow widening split behind their own evidence and smoke routing. Keep Oracle catalog/edit/structured DDL/runtime Safe Mode smoke/SID/TNS/wallet/TLS/fixture/live/E2E/full PL/SQL executable-semantics widening split behind source-specific evidence before any support claim widens. |
