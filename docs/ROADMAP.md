...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  bounded SELECT/DML query runtime/result envelope, bounded T-SQL editor
  completion vocabulary, and bounded static parser/Safe Mode metadata 까지만
  active 이고 catalog/edit/runtime Safe Mode smoke/runtime fixture/E2E 는
  deferred 다. Oracle 은 service-name connection lifecycle, bounded SELECT/DML
  query runtime/result envelope, bounded static SQL editor autocomplete
  vocabulary, and bounded static parser/Safe Mode metadata 까지만 active 이고
  catalog/edit/runtime Safe Mode smoke/runtime fixture/live promotion 은 planned
  상태로 둔다.
...
| H6 | 더 넓은 paradigm | Cassandra, DynamoDB, graph DB, vector DB, stream source 는 active work 전 명확한 workflow proof 가 필요하다. | MSSQL 은 SQL authentication connection test + `SERVERPROPERTY` version probe + bounded SELECT/DML query runtime/result envelope + bounded T-SQL editor completion vocabulary + bounded static parser/Safe Mode metadata 로 정렬되고, Oracle 은 service-name connection test + bounded SELECT/DML query runtime/result envelope + bounded static editor autocomplete vocabulary + bounded static parser/Safe Mode metadata 로 정렬된다. Wider source 는 candidate-only 계약으로 정렬된다. Profile target, connection kind, language, catalog model, result envelope, safety policy, fixture strategy 가 문서화되고 MSSQL catalog/edit/runtime Safe Mode smoke/runtime fixture/E2E/Oracle catalog/edit/structured DDL/runtime Safe Mode smoke/SID/TNS/wallet/TLS/fixture/live/E2E 밖의 runtime support claim 은 생기지 않는다. |
...
envelope, bounded T-SQL editor completion vocabulary, and bounded static
parser/Safe Mode metadata 까지만 active 이고, Oracle 은 service-name connection
lifecycle, bounded SELECT/DML query runtime/result envelope, bounded static SQL
editor autocomplete vocabulary, and bounded static parser/Safe Mode metadata 만
더한다.
...
catalog/edit/runtime Safe Mode smoke/runtime fixture/E2E support 및 Oracle
...
|---|---|---|
| MSSQL bounded query-plus-parser support | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/mssql.rs`, `src-tauri/src/db/mssql/runtime.rs`, `src/lib/sql/sqlDialectProfile.test.ts`, `src/lib/sql/sqlCompletionRequest.test.ts`, `src/lib/sql/sqlDialectKeywords.test.ts`, `src/lib/sql/sqlSafety.test.ts`, `src/lib/sql/sqlWasmArtifact.test.ts`, `src-tauri/sql-parser-core/src/completion/mssql_completion_tests.rs`, `src-tauri/sql-parser-core/src/completion/token.rs`, `src-tauri/sql-parser-core/src/parser/tests.rs`, `src-tauri/tests/mssql_connection_routing.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs` | `mssql` has `server` connection kind, `sql` language, `rdb` catalog, `tabular` result, `rdb-default` safety, RelationalQuery backend capability, SQL authentication connection testing, bounded TCP/TDS/version-probe timeouts, TLS required/trust-server-certificate routing, live Docker evidence for `SERVERPROPERTY('ProductVersion')`, bounded SELECT tabular envelopes, DML/DDL execution, transactional DML batch commit/dry-run, cancellation/error surfacing, bounded editor completion evidence for curated T-SQL keywords/functions, catalog procedure candidates, SQLCMD exclusion, bracket identifier quoting, and bounded static T-SQL parser/Safe Mode metadata for selected `SELECT`/DML/DDL slices plus unsupported scripting/admin boundaries. Runtime catalog/edit, desktop E2E smoke, instance discovery, runtime fixture smoke, runtime Safe Mode/destructive preview evidence, row-edit key projection, and full T-SQL semantic completion are not claimed. |
...
| Parser/completion/runtime non-claim | `docs/product/query-language-support.md`, `docs/product/known-limitations.md` | Deferred language ids do not create active parser, completion, connection, query, catalog, edit, or E2E claims by themselves. MSSQL connection probe, bounded query runtime/result envelope, editor-assistance completion, static parser/Safe Mode metadata, and Oracle service-name connection plus bounded SELECT/DML query runtime/result envelope plus bounded static editor autocomplete vocabulary plus bounded static parser/Safe Mode metadata are explicit exceptions; they do not create MSSQL catalog/edit/runtime Safe Mode smoke/runtime fixture/live/desktop E2E or Oracle catalog/edit/structured DDL/runtime Safe Mode smoke/runtime fixture/live/desktop E2E claims. Runtime changes must land in later source-specific PRs with matching smoke evidence. |
| H6 smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current E2E smoke proves PostgreSQL, MySQL, MariaDB, SQLite, DuckDB `.duckdb`, and MongoDB journeys only. MSSQL has focused connection-routing, live Docker probe, bounded query runtime/result-envelope, static parser/Safe Mode, and completion evidence, not current desktop E2E smoke. Oracle has focused service-name connection evidence, bounded backend query runtime unit/profile evidence, static parser/Safe Mode evidence, and profile/request/WASM/core completion evidence, not fixture/live or current desktop E2E claims. Wider candidates have future smoke inventories only. |
...
| MSSQL/Oracle | MSSQL now has lifecycle-backed SQL authentication, `SERVERPROPERTY` version probe evidence, bounded query runtime/result-envelope support, bounded editor completion vocabulary, and bounded static parser/Safe Mode metadata. Oracle now has service-name connection lifecycle, bounded SELECT/DML query runtime/result-envelope support, bounded static SQL editor autocomplete vocabulary, and bounded static parser/Safe Mode metadata. Keep MSSQL catalog/edit/runtime Safe Mode smoke/runtime fixture/E2E widening split behind their own evidence and smoke routing. Keep Oracle catalog/edit/structured DDL/runtime Safe Mode smoke/SID/TNS/wallet/TLS/fixture/live/E2E/full PL/SQL executable-semantics widening split behind source-specific evidence before any support claim widens. |
...
2. connect/browse/query 만 노출하는 runtime 을 하나 더 붙이는 것보다, 기존 runtime
   깊이를 우선한다.
3. Runtime promotion freeze: Search admin execution, remaining MSSQL catalog/edit/runtime
   Safe Mode smoke support, Oracle, 기타 새 DBMS lane 은 현재 지원 DBMS 하나가
