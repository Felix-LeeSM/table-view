...
| `.duckdb` raw SQL path | `src-tauri/src/db/duckdb.rs`, `src-tauri/src/db/duckdb/ddl.rs`, `src-tauri/tests/duckdb_browse_query_adapter.rs` | Statement-level raw SQL reads and DML are active. Native structural DDL (table create/drop/rename, column add/drop/type, index create/drop) is active as ADR 0051 Stage 2 (#1070); constraint add/drop (Stage 2b rebuild-swap) and dry-run/multi-statement transactions (Stage 3) are not yet implemented. |
