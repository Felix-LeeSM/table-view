...
| Fixture and E2E smoke | `e2e/fixtures/seed.sql`, `e2e/smoke/postgres.spec.ts`, `e2e/smoke/postgres-explain.spec.ts`, `e2e/smoke/postgres-extension-completion.spec.ts`, `scripts/e2e-smoke-ci.sh` | Runtime Happy Path currently proves PostgreSQL connect -> browse seeded `users` -> edit -> query result, Explain plan-inspection/source-label, and installed-extension-gated completion for seeded `pgcrypto` on Ubuntu. It does not cover Safe Mode confirmations, DDL structure flows, broader history-source labeling, cancellation, ERD, admin, or profiler/activity scenarios. |
