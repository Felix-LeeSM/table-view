...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  까지만 active 이고 query/catalog/edit/parser/completion/runtime smoke 는 deferred,
  Oracle 은 bounded static SQL editor autocomplete vocabulary 만 active 로 분리하고
...
| H6 | 더 넓은 paradigm | Cassandra, DynamoDB, graph DB, vector DB, stream source 는 active work 전 명확한 workflow proof 가 필요하다. | MSSQL 은 SQL authentication connection test + `SERVERPROPERTY` version probe 만 active 로 정렬되고, Oracle 은 bounded static editor autocomplete vocabulary 와 planned runtime identity 계약으로, wider source 는 candidate-only 계약으로 정렬된다. Profile target, connection kind, language, catalog model, result envelope, safety policy, fixture strategy 가 문서화되고 MSSQL connection probe 와 Oracle editor-assistance vocabulary 밖의 support claim 은 생기지 않는다. |
...
Server `SERVERPROPERTY` version probe 까지만 active 이고, Oracle 은 planned RDBMS
identity 에 bounded static SQL editor autocomplete vocabulary 만 더한다.
...
connection probe 밖의 새 runtime support 를 출시하지 않으며, Oracle completion 은
runtime support 로 승격하지 않는다.

| Gate | Current owner | H6 boundary |
|---|---|---|
| MSSQL lifecycle-only connection support | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/mssql.rs`, `src-tauri/tests/mssql_connection_routing.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs` | `mssql` has `server` connection kind, `sql` language, `rdb` catalog, `tabular` result, `rdb-default` safety, `declared-rdb` backend identity, SQL authentication connection testing, bounded TCP/TDS/version-probe timeouts, TLS required/trust-server-certificate routing, and live Docker evidence for `SERVERPROPERTY('ProductVersion')`. Runtime query/catalog/edit, T-SQL parser/completion, desktop E2E smoke, instance discovery, and runtime fixture smoke are not claimed. |
...
| MSSQL/Oracle | Keep MSSQL to lifecycle-only SQL authentication connection testing and `SERVERPROPERTY` version probe evidence until a later PR splits query/catalog/edit/parser/completion/smoke support. Keep Oracle capability-empty for runtime while allowing bounded static SQL editor autocomplete vocabulary only. Future promotion must split SQL Server and Oracle connection/auth/dialect/catalog/safety contracts and add fixture/live evidence before any runtime support claim widens. |
