...
| Query/result boundary | Keep typed envelopes as the UI-facing boundary. 2026-07-25 오너 grill 이 `native tabular envelope 전환` 항목을 YAGNI 로 걷어냈다. 근거: `src-tauri/src/models/query.rs` 의 `QueryResult` 는 이미 `#[serde(rename_all = "camelCase")]` 라, 그 경로에서 전환으로 사라지는 것은 `src/lib/wireCamelCase.ts` 의 `normalizeQueryResult` 안 `totalCount`/`executionTimeMs` 이중 수용 (죽은 방어) 뿐이다. 반면 `BulkWriteResult` (`src-tauri/src/db/types.rs`) 와 `TableData`/`ColumnInfo` (`src-tauri/src/models/schema/core.rs`) 는 `rename_all` 없이 snake_case 로 나가는 **live wire** 라 `normalizeBulkWriteResult` 는 snake 키만 읽고 `numericWrap.ts` 의 `c.dataType ?? c.data_type` snake 분기는 테이블 브라우즈마다 발화한다 — 전면 전환은 `QueryResult` 하나가 아니라 IPC 타입 다수를 건드리는 작업이고, `wrapNumericCells` 는 JSON 에 BigInt 타입이 없어 무엇을 해도 남는다. 비용이 더 큰 쪽으로 확인됐으므로 YAGNI 결론을 유지한다. 실제 유지 고통이 생기면 그때 다시 올린다. |
...
| MongoDB | Keep support to tested whitelisted document workflows (`src/lib/mongo/mongoshMethods.ts` 의 16개 메서드). 2026-07-25 오너 grill 실측에서 기존 서술 1개가 stale 로 판명됐다: 집계 파이프라인은 allowlist 가 아니라 denylist 라 `src/lib/mongo/mongoSafety.ts` 가 `$out`/`$merge` 만 destructive 로 잡고 나머지는 이미 열려 있다. `document-first panels` 서술은 **browse/edit 표면에만 해당한다** — `src/components/document/` 의 15개 top-level 컴포넌트 (`DocumentDataGrid`, `DocumentTreePanel`, `MongoStructurePanel`, `MongoIndexesPanel`, `ValidatorPanel`, `AddDocumentModal`, `BsonTypeEditor` 등; `CollectionDdlDialog`/`NestedExpandPopover` 는 import 0건 죽은 컴포넌트다) 는 컬렉션 browse/edit 만 덮고, **query result 표면은 여전히 tabular** 다: `src/components/query/QueryTab/mongoQueryExecution.ts` 가 응답을 `JSON.stringify` 해 1컬럼 grid 로 밀어 넣는다. `docs/product/**` 의 `native document-first result panels remain unsupported` 는 그대로 유효하다. 실제 남은 공백은 세 축이다: (1) server version/topology capability 승격 (#1821 — 나머지 축의 선행. `buildInfo`/`serverStatus` 는 `src-tauri/src/db/mongodb/schema.rs` 가 수집해 `ServerInfoPanel` 이 표시까지 하므로 없는 것은 소비처가 아니라 **capability gate 소비처**다), (2) change streams `watch()` (#1822 — replica set 이상 필요, #1808 이 놓는 스트리밍 인프라 위에 쌓는다), (3) 다중 문서 트랜잭션 (#1823 — 세션 수명 관리, Safe Mode confirm 지점 미결). `db.runCommand` 직접 실행 (#1824) 은 **이미 shipped** 다 — `run_mongo_command` (Sprint 381/384/475) 가 `READ_ONLY_MONGO_RUN_COMMANDS` 읽기 allowlist 를 두고 미분류/쓰기 명령은 `safety_confirmed` 없이 거부하므로 "명령 tier 분류 + 미분류 destructive 기본값" 이 구현돼 있다. #1824 는 잔여 delta 로 재스코핑하거나 닫아야 한다. arbitrary JavaScript/shell behavior 는 여전히 미지원이지만 `$function` 서버사이드 JS 는 **차단 게이트가 없다** — `mongoSafety.ts` 가 danger boundary 미정의로 통과시키고 completion vocabulary 는 오히려 제안한다. 미지원이 아니라 미분류 상태이므로 별도 결정이 필요하다. |
| Search | Keep actual Elasticsearch/OpenSearch index/settings admin execution deferred. Elasticsearch/OpenSearch live connection/catalog/query, backend-bounded Search DSL validation, delete-by-query safety planning, live `_delete_by_query` execution behind the Safe Mode confirm gate (#1076), bounded TypeScript Search DSL editor assistance, and representative Runtime Happy Path smoke are active; the smoke covers connect/auth/TLS contract, selected metadata, bounded render, delete-plan preview, live delete execution, and error surface. Promote broader live HTTP only after index/settings admin execution policy, broader observability/profile workflows, full language-core parser/completion ownership, and product-specific delta contracts are explicit. 2026-07-25 오너 grill 이 observability 축을 둘로 분해했다: 실행 시간 분해 profile (#1818 — gate 선행조건은 이쪽이다) 과 문서별 스코어 계산 내역 `_explain` (#1819 — gate 를 당기지 않는다). 두 축 모두 **결과 표면은 이미 있다** — `src/types/search.ts` 가 `explain`/`profile` 필드를 갖고 `SearchResultView` 가 둘 다 payload 로 렌더한다. 실제 막힌 지점은 `src-tauri/src/db/search_dsl.rs` 의 `validate_search_body` 로, top-level 허용 키가 `query`/`aggs`|`aggregations`/`sort`/`_source`/`from`|`size`/`track_total_hits` 6 갈래뿐이라 `profile` 과 `explain` 을 `Unsupported` 로 거부한다. #1818 은 추가로 `ExplainViewer` 의 search 브랜치가 필요하다 — 현재 `paradigm === "document" ? explainMongoFind : explainRdbQuery` 2분기라 search 연결이 조용히 RDB explain 경로로 샌다. |
