...
| Non-RDBMS runtime | Redis/Valkey 와 MongoDB 는 runtime slice 가 있다. Elasticsearch/OpenSearch 는 live HTTP 전까지 fixture-backed 다. Cassandra/DynamoDB/graph/vector 는 gated candidate 다. 새 runtime promotion 은 active one-DBMS parity lane 뒤로 둔다. | `docs/data-source-architecture.md`, `docs/phases/phase-28.md` |
...
| Security | Credential/key handling, role/user management, auth mechanism expansion, destructive action policy. | `.agents/skills/grill-me/SKILL.md`, `docs/contributor-guide/testing-and-quality.md` |
...
| Quality gates | Promote a11y, perf, E2E isolation, and platform smoke gaps from `testing-and-quality.md` when they block an active feature lane. |
| Refactor backlog | Promote code-smell audit candidates only when they intersect active feature work or remove current maintenance cost. |
...
| SOT check | `docs/product/README.md`, `docs/product/known-limitations.md`, architecture/contributor docs, memory link 를 업데이트하거나 변경 없음으로 선언. |
| Follow-up check | 현재 제한은 product, 미래 work item 은 roadmap, 구조 제약은 architecture, 개발/운영 제약은 contributor docs, 과거 사건은 archives 로 라우팅한다. |
