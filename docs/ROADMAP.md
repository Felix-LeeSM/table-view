...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  Oracle 은 planned identity 로만 유지한다.
...
| H6 | 더 넓은 paradigm | Cassandra, DynamoDB, graph DB, vector DB, stream source 는 active work 전 명확한 workflow proof 가 필요하다. | MSSQL 은 SQL authentication connection test + `SERVERPROPERTY` version probe 만 active 로 정렬되고, Oracle 은 planned RDBMS identity 계약으로, wider source 는 candidate-only 계약으로 정렬된다. Profile target, connection kind, language, catalog model, result envelope, safety policy, fixture strategy 가 문서화되고 MSSQL connection probe 밖의 runtime support claim 은 생기지 않는다. |
...
identity 다. Cassandra/Scylla, DynamoDB, graph, vector, stream 은 아직 active
`DatabaseType`/profile/runtime 이 없는 candidate 다. H6 closure 는 MSSQL
connection probe 밖의 새 runtime support 를 출시하지 않는다.
...
| MSSQL/Oracle | Keep MSSQL to lifecycle-only SQL authentication connection testing and `SERVERPROPERTY` version probe evidence until a later PR splits query/catalog/edit/parser/completion/smoke support. Keep Oracle as a capability-empty declared RDBMS identity. Future promotion must split SQL Server and Oracle auth/dialect/catalog/safety contracts and add fixture/live evidence before any support claim widens. |
