...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  query/admin 은 deferred 로 유지하고, MSSQL 은 planned identity 로만 유지한다.
...
이미 `DatabaseType`/profile identity 가 있는 planned RDBMS 이고, Oracle 의
bounded static SQL editor autocomplete vocabulary 는 runtime support 로 승격하지
않는다. Cassandra/Scylla, DynamoDB, graph, vector, stream 은 아직 active
`DatabaseType`/profile/runtime 이 없는 candidate 다. H6 closure 는 어떤 새
runtime support 도 출시하지 않는다.
...
| H6 smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current E2E smoke proves PostgreSQL, MySQL, MariaDB, SQLite, DuckDB `.duckdb`, and MongoDB journeys only. MSSQL/Oracle and wider candidates have future smoke inventories, not current desktop E2E claims. |
...
| MSSQL/Oracle | Keep MSSQL as a capability-empty declared RDBMS identity. Keep Oracle capability-empty for runtime while allowing bounded static SQL editor autocomplete vocabulary only. Future promotion must split SQL Server and Oracle connection/auth/dialect/catalog/safety contracts and add fixture/live evidence before any runtime support claim widens. |
...
2. connect/browse/query 만 노출하는 runtime 을 하나 더 붙이는 것보다, 기존 runtime
   깊이를 우선한다.
3. Runtime promotion freeze: Search live query/admin, MSSQL, Oracle, 기타 새 DBMS lane 은
