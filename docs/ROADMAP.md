...
| MariaDB autocomplete delta gate | `src/lib/sql/sqlCompletionRequest.ts`, `src/lib/sql/sqlCompletionWasm.ts`, `src-tauri/sql-parser-core/src/completion/**`, `src/lib/sql/sqlCodeMirrorCompletionAdapter.test.ts`, #454 | MariaDB completion requests keep distinct `mariadb` identity while reusing the MySQL family and `mysql-client` shell. Known MariaDB versions below `10.5.0` suppress the `RETURNING` suggestion; unknown versions and `>= 10.5.0` keep the profile delta. This is completion assistance only, not a runtime support guarantee. |
