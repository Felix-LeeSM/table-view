...
|---|---|---|
| MongoDB whitelist workflow | `src/types/dataSource.ts`, `src/lib/mongo/**`, `src/components/query/QueryTab/useQueryExecution.parserDispatch.test.tsx`, `src-tauri/tests/mongo_integration.rs`, `src-tauri/tests/cancel_mongo.rs`, `e2e/smoke/mongodb.spec.ts` | Connection, catalog, whitelisted query/edit/bulk/index/validator slices, find projection, aggregate cursor-chain lowering (`sort`/`skip`/`limit`), cancellation, and destructive Safe Mode gates are current support. Arbitrary JavaScript, shell helpers, multiple statements, cross-db shell navigation, version/deployment gates, and native document-first panels remain unsupported. Standalone transaction-style paths must fail clearly instead of silently committing partial work. |
