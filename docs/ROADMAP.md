...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  E2E/TLS-required workflow 는 deferred 다. Oracle 은 service-name connection-only lifecycle 과 bounded static
  SQL editor autocomplete vocabulary 까지만 active 이고
  runtime/query/catalog/edit/parser/Safe Mode promotion 은 planned 상태로 둔다.
...
lifecycle 과 bounded static SQL editor autocomplete vocabulary 만 더한다.
...
query/catalog/edit/SID/TNS/wallet/TLS support 를 출시하지 않으며, MSSQL/Oracle
...
| Oracle lifecycle-plus-completion support | `src/components/connection/forms/OracleFormFields.tsx`, `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/oracle.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs`, `src/lib/sql/sqlDialectProfile.test.ts`, `src/lib/sql/sqlCompletionRequest.test.ts`, `src/lib/sql/sqlWasmArtifact.test.ts`, `src-tauri/sql-parser-core/src/completion/completion_tests.rs` | `oracle` has `server` connection kind, `sql` language, `rdb` catalog, `tabular` result, `rdb-default` safety, a backend `oracle` adapter with `Lifecycle` only, service-name connection UI/runtime test/connect, and bounded static SQL editor autocomplete vocabulary for selected Oracle keywords, functions, package symbols, sequences, synonyms, and bind placeholders. SID, TNS alias, wallet/TLS, runtime query/catalog/edit, Oracle SQL/PL/SQL parser/Safe Mode, runtime fixture/live evidence, desktop E2E, and full PL/SQL executable semantics are not claimed. |
...
| MSSQL/Oracle | MSSQL now has lifecycle-backed SQL authentication, `SERVERPROPERTY` version probe evidence, bounded query runtime/result-envelope support, catalog/workbench metadata browse, bounded editor completion vocabulary, and bounded static parser/Safe Mode metadata. Keep MSSQL row edit/runtime Safe Mode smoke/runtime fixture/E2E/TLS-required workflow widening split behind their own evidence and smoke routing. Keep Oracle to service-name connection lifecycle plus bounded static SQL editor autocomplete vocabulary until follow-up issues split SID/TNS/wallet/TLS, dialect/catalog/query/edit/safety contracts and add fixture/live evidence before any support claim widens. |
