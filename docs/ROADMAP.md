...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  lifecycle, bounded SELECT/DML query runtime/result envelope, bounded static SQL
  editor autocomplete vocabulary, and bounded static parser/Safe Mode metadata
  까지만 active 이고 catalog/edit/structured DDL/runtime Safe Mode smoke/runtime
  fixture/live promotion 은 planned 상태로 둔다.
...
SELECT/DML query runtime/result envelope, bounded static SQL editor autocomplete
vocabulary, and bounded static parser/Safe Mode metadata 만 더한다.
...
TLS-required workflow/SQLCMD/admin/security/backup/jobs/users/roles support 및 Oracle
catalog/edit/structured DDL/runtime Safe Mode smoke/SID/TNS/wallet/TLS/runtime fixture/live/E2E support 를 출시하지 않으며, MSSQL/Oracle
...
| Oracle bounded query-plus-parser/completion support | `src/components/connection/forms/OracleFormFields.tsx`, `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/oracle.rs`, `src-tauri/src/db/oracle/runtime.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs`, `src/lib/sql/sqlDialectProfile.test.ts`, `src/lib/sql/sqlCompletionRequest.test.ts`, `src/lib/sql/sqlSafety.test.ts`, `src/lib/sql/sqlWasmArtifact.test.ts`, `src-tauri/sql-parser-core/src/completion/completion_tests.rs`, `src-tauri/sql-parser-core/src/parser/tests.rs` | `oracle` has `server` connection kind, `sql` language, `rdb` catalog, `tabular` result, `rdb-default` safety, a backend `oracle` adapter with `Lifecycle` and `RelationalQuery`, service-name connection UI/runtime test/connect, bounded SELECT tabular envelopes, transactional DML batch commit/dry-run, cancellation/error surfacing, bounded static SQL editor autocomplete vocabulary for selected Oracle keywords, functions, package symbols, sequences, synonyms, and bind placeholders, and bounded static parser/Safe Mode metadata for common SELECT/DML/DDL safety classification with PL/SQL package/admin paths unsupported. SID, TNS alias, wallet/TLS, catalog/edit/structured DDL, runtime Safe Mode smoke, runtime fixture/live evidence, desktop E2E, and full PL/SQL executable semantics remain deferred. |
...
| Parser/completion/runtime non-claim | `docs/product/query-language-support.md`, `docs/product/known-limitations.md` | Deferred language ids do not create active parser, completion, connection, query, catalog, edit, DDL, or E2E claims by themselves. MSSQL connection probe, bounded query runtime/result envelope, primary-key-scoped row edit, structured table/index/constraint DDL, catalog/workbench metadata browse, representative Runtime Happy Path smoke, editor-assistance completion, static parser/Safe Mode metadata, and Oracle service-name connection plus bounded SELECT/DML query runtime/result envelope plus bounded static editor autocomplete vocabulary plus bounded static parser/Safe Mode metadata are explicit exceptions; they do not create MSSQL TLS-required workflow/SQLCMD/admin/security/backup/jobs/users/roles support or Oracle catalog/edit/structured DDL/runtime Safe Mode smoke/runtime fixture/live/desktop E2E claims. Runtime changes must land in later source-specific PRs with matching smoke evidence. |
| H6 smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current E2E smoke proves PostgreSQL, MySQL, MariaDB, MSSQL, SQLite, DuckDB `.duckdb`, MongoDB, Redis, Valkey, Elasticsearch, and OpenSearch journeys only. MSSQL has focused connection-routing, live Docker probe, bounded query runtime/result-envelope, primary-key-scoped row edit, structured DDL preview/unit evidence, catalog/workbench metadata, static parser/Safe Mode, completion evidence, and representative desktop E2E smoke. Oracle has focused service-name connection evidence, bounded backend query runtime unit/profile evidence, static parser/Safe Mode evidence, and profile/request/WASM/core completion evidence, not fixture/live or current desktop E2E claims. Wider candidates have future smoke inventories only. |
...
| MSSQL/Oracle | MSSQL now has lifecycle-backed SQL authentication, `SERVERPROPERTY` version probe evidence, bounded query runtime/result-envelope support, primary-key-scoped row edit, bounded structured table/index/constraint DDL, catalog/workbench metadata browse, representative Runtime Happy Path smoke, live cached catalog-aware editor completion, and bounded static parser/Safe Mode metadata. Oracle now has service-name connection lifecycle, bounded SELECT/DML query runtime/result-envelope support, bounded static SQL editor autocomplete vocabulary, and bounded static parser/Safe Mode metadata. Keep MSSQL TLS-required workflow/SQLCMD/admin/security/backup/jobs/users/roles/full T-SQL semantic widening split behind their own evidence and smoke routing. Keep Oracle catalog/edit/structured DDL/runtime Safe Mode smoke/SID/TNS/wallet/TLS/fixture/live/E2E/full PL/SQL executable-semantics widening split behind source-specific evidence before any support claim widens. |
