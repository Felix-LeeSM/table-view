...
| H5 | First-class non-RDBMS | Redis/Valkey, Elasticsearch/OpenSearch, MongoDB 가 가장 명확한 non-RDBMS 사용자 workflow 를 덮는다. | MongoDB 는 whitelisted document workflow 로, Redis 는 bounded KV browser/value mutation + command/query/completion + representative smoke 로, Valkey 는 connection/key scan/value preview + bounded command query runtime slice + proven-row command completion + Runtime Happy Path smoke 로, Elasticsearch 는 live connection/catalog/query + backend-bounded Search DSL validator + Runtime Happy Path smoke + fixture/live delete-by-query safety planning 으로, OpenSearch 는 live URL/auth/TLS root probe + live catalog + bounded live Search query + live delete-by-query safety planning + bounded TypeScript editor completion 으로 support claim 이 정렬돼 있다. Actual Search live admin execution, OpenSearch Runtime Happy Path smoke, full language-core parser/completion ownership, and broader Search smoke 는 active parity lane 을 약화시키지 않고 promotion gate 를 통과할 때까지 deferred 다. |
...
| Search live admin/smoke promotion | `docs/product/query-language-support.md`, `docs/product/known-limitations.md`, `docs/contributor-guide/testing-and-quality.md` | Future promotion must cover OpenSearch Runtime Happy Path smoke, actual admin destructive execution policy, observability/error surface, CI fixture/live smoke, and active RDBMS lane freeze. |
| Non-RDBMS smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current desktop E2E claim covers MongoDB, Redis, Valkey, and Elasticsearch representative workflows. OpenSearch live connection/catalog/query/destructive-plan/autocomplete has focused runtime/UI/editor tests only; OpenSearch Runtime Happy Path smoke, actual Search admin execution, and broader Search error/observability smoke remain future promotion gates. |
...
| Search | Keep actual Elasticsearch/OpenSearch admin execution deferred. Elasticsearch/OpenSearch live connection/catalog/query, backend-bounded Search DSL validation, delete-by-query safety planning, and bounded TypeScript Search DSL editor assistance are active; promote broader live HTTP only after OpenSearch Runtime Happy Path smoke, admin execution policy, observability/error surface, full language-core parser/completion ownership, and product-specific delta contracts are explicit. |
...
8. OpenSearch Runtime Happy Path smoke and Search admin HTTP promotion.
