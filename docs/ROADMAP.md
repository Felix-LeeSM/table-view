...
- Redis/Valkey, Elasticsearch/OpenSearch, and MongoDB are first-class non-RDBMS
  targets after the data-source extension architecture is in place; Search
  remains fixture-backed until live HTTP support lands.
...
| H5 | First-class non-RDBMS | Redis/Valkey, Elasticsearch/OpenSearch, and MongoDB cover the clearest non-RDBMS user workflows. | KV/Document runtime contracts are live, Search is fixture-backed until HTTP support, and paradigm-specific workbenches are green for claimed runtime paths. |
...
| Non-RDBMS runtime | Redis/Valkey and MongoDB have runtime slices; Elasticsearch/OpenSearch are fixture-backed until live HTTP; Cassandra/DynamoDB/graph/vector are gated candidates. | `docs/data-source-architecture.md`, `docs/phases/phase-28.md` |
