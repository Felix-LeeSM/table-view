...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  browse, primary-key-scoped row edit, live cached catalog-aware T-SQL editor
  completion, and bounded static parser/Safe Mode metadata 까지만 active 이고
  runtime Safe Mode smoke/
  runtime fixture/E2E/TLS-required workflow/SQLCMD/admin/full semantic support 는
...
envelope, primary-key-scoped row edit, catalog/workbench metadata browse, bounded T-SQL editor completion
...
runtime Safe Mode smoke/runtime fixture/E2E/TLS-required workflow support 및 Oracle
...
| MSSQL/Oracle | MSSQL now has lifecycle-backed SQL authentication, `SERVERPROPERTY` version probe evidence, bounded query runtime/result-envelope support, primary-key-scoped row edit, catalog/workbench metadata browse, live cached catalog-aware editor completion, and bounded static parser/Safe Mode metadata. Oracle now has service-name connection lifecycle, bounded SELECT/DML query runtime/result-envelope support, bounded static SQL editor autocomplete vocabulary, and bounded static parser/Safe Mode metadata. Keep MSSQL runtime Safe Mode smoke/runtime fixture/E2E/TLS-required workflow/SQLCMD/admin/full T-SQL semantic widening split behind their own evidence and smoke routing. Keep Oracle catalog/edit/structured DDL/runtime Safe Mode smoke/SID/TNS/wallet/TLS/fixture/live/E2E/full PL/SQL executable-semantics widening split behind source-specific evidence before any support claim widens. |
