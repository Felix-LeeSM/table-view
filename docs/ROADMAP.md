...
장기 방향을 잃지 않기 위한 전략 문서다. `docs/PLAN.md` 는 active-only
실행 순서이고, 이 문서는 그 뒤에 남는 제품/아키텍처 지평을 관리한다.
...
- 기존 지원 DBMS 하나를 TablePlus-style query/workbench parity 까지 끌어올리는
...
- Active plan 은 짧게 유지한다. 완료/비활성 문서는 `docs/archives/` 로 이동한다.
...
| H2 | RDBMS parity | 현재 아키텍처가 가장 강한 영역이고, 사용자에게 보이는 gap 이 TablePlus 전환 blocker 다. | DBMS 하나씩 query/workbench parity gate 를 통과한 뒤 다음 DBMS/runtime 승격을 시작한다. |
| H3 | DuckDB + file analytics | Local-first file analytics 는 새 paradigm 없이 RDBMS 작업을 확장한다. | `.duckdb`, CSV, Parquet, JSON preview/query basics 와 file privacy rule 이 green 이다. |
| H4 | RDBMS intelligence | ERD/schema diff/data compare/migration preview 는 같은 RDB catalog graph 를 재사용한다. | `SchemaGraph` 가 ERD, FK navigation, dependency view, migration impact analysis 를 구동한다. |
...
| H7 | 운영, 보안, 신뢰성 | 넓은 source support 는 관찰 가능하고 안전하며 반복 검증 가능해야 한다. | Active risk register 가 줄고, 핵심 ops/security/a11y/perf smoke path 가 routine gate 가 된다. |

## 트랙 맵

| 트랙 | 장기 방향 | 현재 기준 |
|---|---|---|
| Data-source architecture | 새 DBMS/support surface 는 profile, capability, adapter, language, catalog, result envelope, safety contract 를 통해 들어온다. | `docs/data-source-architecture.md`, `docs/adding-a-data-source.md`, ADR 0046 |
| RDBMS runtime | 불확실한 paradigm 을 넓히기 전에 PostgreSQL, MySQL, MariaDB, SQLite, DuckDB/file analytics support 를 강하게 만든다. | `docs/PLAN.md`, `docs/phases/phase-18.md`, `docs/phases/phase-19.md` |
| Non-RDBMS runtime | Redis/Valkey 와 MongoDB 는 runtime slice 가 있다. Elasticsearch/OpenSearch 는 live HTTP 전까지 fixture-backed 다. Cassandra/DynamoDB/graph/vector 는 gated candidate 다. 새 runtime promotion 은 active one-DBMS TablePlus-parity lane 뒤로 둔다. | `docs/data-source-architecture.md`, `docs/phases/phase-28.md` |
| Language core | 가능한 범위에서 Rust/WASM 이 hot-path parse/completion vocabulary, context routing, capability gate 를 소유한다. | ADR 0045, `docs/query-language-support.md`, `docs/archives/phases/completed/phase-31.md` |
| Query editor | Query surface 는 legacy `queryMode` 가 아니라 `queryLanguage` 와 workbench paradigm 으로 고른다. | `docs/data-source-architecture.md`, `docs/phases/phase-28.md` Slice A |
| Data editing | Preview/commit/discard, bulk operation, paradigm 별 edit semantics. | completed Phases 22-23, Phase 28 |
| Schema / DDL | RDB DDL parity 는 대부분 닫혔고, ERD/schema graph 가 다음 reusable intelligence layer 다. | completed Phases 24-27, `docs/data-source-architecture.md` |
| Operations | Core parity 이후 Explain/activity/stats/server info/profiler surface 를 다룬다. | `memory/roadmap/unified-followups/memory.md` |
...
| Quality | CI, E2E smoke, perf/a11y baseline, testing reliability, refactor backlog burn-down. | `docs/RISKS.md`, `docs/archives/audits/code-smell-audit-2026-05-15.md` |
...
6. Parser/Safe Mode/completion support 는 명시돼야 한다. Unsupported syntax 는
   `docs/query-language-support.md` 에 문서화된 경우에만 허용한다.
7. 새 DBMS 는 구현 시작 전 `docs/adding-a-data-source.md` 를 만족해야 한다.
...
10. 완료/비활성 planning 은 archive 로 이동한다. `docs/PLAN.md` 는 active-only 로
   유지한다.
...
| SOT check | 기존 `docs/PLAN.md`, phase docs, risk docs, memory link 를 업데이트하거나 변경 없음으로 선언. |
| Risk check | 새 risk 는 `docs/RISKS.md` 에 추가하거나 기존 risk 를 참조. |
...
현재 상태: Sprint 481 이후 implementation sprint 는 배정하지 않았다. 다음 작업을
고를 때 이 목록과 `docs/phases/phase-32.md` 부터 본다. Active lane 이 선택되기
전까지 sprint sequence 를 새로 만들지 않는다.
...
이 순서를 바꾸면 이 파일과 `docs/PLAN.md` 를 같이 업데이트한다.
