...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  Oracle 은 service-name connection-only lifecycle 로 제한한다.
...
| H6 | 더 넓은 paradigm | Cassandra, DynamoDB, graph DB, vector DB, stream source 는 active work 전 명확한 workflow proof 가 필요하다. | MSSQL 은 planned RDBMS identity 계약으로, Oracle 은 connection-only lifecycle 계약으로, wider source 는 candidate-only 계약으로 정렬된다. Profile target, connection kind, language, catalog model, result envelope, safety policy, fixture strategy 가 문서화되고 catalog/query/edit runtime support claim 은 생기지 않는다. |
...
이미 `DatabaseType`/profile identity 가 있는 planned RDBMS 이고, Oracle 은
service-name connection-only lifecycle 로만 승격된 RDBMS 이다. Cassandra/Scylla,
DynamoDB, graph, vector, stream 은 아직 active `DatabaseType`/profile/runtime 이
없는 candidate 다. H6 closure 자체는 catalog/query/edit runtime support 를 출시하지 않는다.
...
| Oracle connection-only RDBMS lifecycle | `src/components/connection/forms/OracleFormFields.tsx`, `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/oracle.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs` | `oracle` has `server` connection kind, `sql` language identity, `rdb` catalog model, `tabular` result, `rdb-default` safety, and a backend `oracle` adapter with `Lifecycle` only. Connection UI/runtime test/connect uses Host/Port/User/Password/Service name. SID, TNS alias, wallet/TLS, runtime query/catalog/edit, Oracle SQL/PL/SQL parser/completion, fixture smoke, and live evidence are not claimed. |
...
| MSSQL/Oracle | Keep MSSQL as a capability-empty declared RDBMS identity. Keep Oracle at connection-only lifecycle until follow-up issues split SID/TNS/wallet/TLS, dialect/catalog/query/edit/safety contracts and add fixture/live evidence before any support claim widens. |
