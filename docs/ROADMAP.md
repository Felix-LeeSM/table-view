...
| Redis KV + command first slice | `src-tauri/src/db/redis/**`, `src-tauri/tests/redis_integration.rs`, `src/lib/tauri/kv.ts`, `src/lib/redis/redisCommandCompletion.ts`, `src/components/query/RedisCommandEditor.tsx`, `src/components/query/QueryTab/useQueryExecution.kvDispatch.test.tsx`, `src/components/workspace/KvSidebar.tsx`, `src/components/workspace/KvMutationPanel.tsx`, #477/#478/#480/#481 | Backend KV primitives cover database/key scan, typed value reads, guarded string set, delete confirmation, TTL expire/persist, and bounded stream read. Backend command safety classifies read/write/TTL/stream/destructive effects and requires exact-key `confirmKey` for single-key `DEL`/`PERSIST`. The sidebar now has bounded string/hash/list/set/zset mutation controls plus expire/persist/delete preview/confirm semantics, and unsupported/partial mutation surfaces fail visibly. The query tab can dispatch selected Redis read/write/TTL/stream commands through a backend allowlist and project results to the tabular grid. Redis command editor completion suggests the same bounded allowlist with command names, arity hints, and snippets. Catalog-aware key completion, stream consumer UI, language-core parser ownership, broader command coverage, multi-key destructive commands, cluster/pubsub/modules/consumer-group flows are future work. |
| Redis test coverage recheck | `docs/contributor-guide/testing-and-quality.md`, `src-tauri/src/commands/kv.rs`, `src-tauri/src/db/kv_types.rs`, `src-tauri/src/db/redis/mod.rs`, `src-tauri/src/db/redis/command.rs`, `src-tauri/src/db/redis/command_parser.rs`, `src-tauri/tests/redis_integration.rs`, `src/lib/tauri/kv.test.ts`, `src/lib/redis/redisCommandCompletion.test.ts`, `src/components/query/RedisCommandEditor.test.tsx`, `src/components/query/QueryTab/useQueryExecution.kvDispatch.test.tsx`, `src/types/queryLanguage.docs.test.ts`, `src/components/workspace/KvSidebar.test.tsx`, `src/components/workspace/KvSidebar.mutations.test.tsx`, #540/#481 | Final test recheck maps backend runtime/query/edit primitives, source-equivalent value-panel mutation paths, backend command parser/safety allowlist and unsupported-boundary tests, fixture inventory, and future smoke routing before Redis milestone closure. Redis command completion has bounded TypeScript vocabulary evidence for command names, arity hints, and snippets. Catalog-aware key completion remains a future context surface. |
