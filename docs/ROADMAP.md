...
| Non-RDBMS smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current desktop E2E claim covers MongoDB, Redis, Valkey, and Elasticsearch representative workflows. OpenSearch live connection/catalog/query has focused runtime/UI tests only; OpenSearch Runtime Happy Path smoke, actual Search admin execution, and broader Search error/observability smoke remain future promotion gates. |
...
| Search | Keep actual Elasticsearch/OpenSearch admin execution deferred. Elasticsearch/OpenSearch live connection/catalog/query, backend-bounded Search DSL validation, and delete-by-query safety planning are active; promote broader live HTTP only after OpenSearch Runtime Happy Path smoke, admin execution policy, observability/error surface, editor parser/completion, and product-specific delta contracts are explicit. |
