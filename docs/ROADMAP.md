...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  service-name local connection/test baseline 로만 연다. SQL Server auth/TDS/T-SQL
...
| H6 | 더 넓은 paradigm | Cassandra, DynamoDB, graph DB, vector DB, stream source 는 active work 전 명확한 workflow proof 가 필요하다. | MSSQL 은 bounded catalog/query/cancel/tabular capability 로, Oracle 은 service-name local connection/test baseline 만 허용한다. Wider source 는 candidate-only 계약으로 정렬된다. Profile target, connection kind, language, catalog model, result envelope, safety policy, fixture strategy 가 문서화되고 각 source 의 matching evidence 없이 query/catalog/edit/DDL/parser/completion/runtime smoke/admin claim 은 생기지 않는다. |
...
H6 wider source 는 **bounded-runtime / connection-only enterprise RDBMS +
...
service-name local connection/test baseline 만 허용한다. MSSQL DDL/admin/runtime
smoke/broad parser/completion claim 과 Oracle SID/TNS/wallet/advanced auth,
query/catalog/edit/DDL/parser/completion/PLSQL/runtime smoke claim 은 각 source 의
...
advanced auth/raw DDL/admin/PLSQL semantics 는 별도 source-specific evidence
전까지 out of scope 다.
...
| Oracle service-name connection guardrail | `docker-compose.yml`, `scripts/fixtures/oracle.ts`, `scripts/fixtures/oracle.test.ts`, `src/types/dataSource.ts`, `src/types/connection.ts`, `src/types/adapterConformance.test.ts`, `src-tauri/src/models/data_source.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs`, `docs/product/README.md`, `docs/product/query-language-support.md`, `docs/product/known-limitations.md`, this roadmap, #904 | `oracle` keeps source-specific identity, backend adapter id, dialect id, labels, service-name defaults, URL parsing, and seed/spec inventory. #904 allows only local-first `host:port/serviceName` connection/test with default fixture service `XEPDB1`. SID, TNS, wallet, TLS, advanced auth, query, catalog, edit, DDL, parser, completion, PLSQL, and runtime smoke stay unclaimed until matching evidence lands. |
...
| Parser/completion/runtime non-claim | `docs/product/query-language-support.md`, `docs/product/known-limitations.md` | Deferred language ids, MSSQL runtime/edit slice, and Oracle service-name connection baseline do not create active structured DDL, admin, full parser/completion, query/catalog/edit, PLSQL, or E2E smoke claims by themselves. Runtime changes must land in later source-specific PRs with matching smoke evidence. |
| H6 smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current E2E smoke proves PostgreSQL, MySQL, MariaDB, SQLite, DuckDB `.duckdb`, DuckDB file analytics, MongoDB, Redis, Valkey, Elasticsearch, and OpenSearch journeys only. MSSQL smoke specs/fixtures remain inventory until #907 wires smoke evidence, Oracle smoke specs/fixtures are inventory beyond #904 service-name connection/test, and wider candidates have future smoke inventories only. |
...
H6 umbrella closure means MSSQL bounded runtime, connection-only Oracle, wider
candidate contracts, and smoke routing are aligned. It does not mean MSSQL
edit/DDL/parser/completion/runtime smoke/admin support, or Oracle connection/
query/catalog/edit/DDL/parser/completion/runtime smoke/admin support, has shipped.
...
| MSSQL/Oracle | Keep MSSQL at bounded catalog/query/cancel/tabular enterprise RDBMS support with source-specific SQL-auth/TDS/encryption contract, and keep Oracle at service-name local connection/test only. Future promotion must add matching query, catalog, edit, DDL, parser, completion, docs, and smoke evidence without hiding SQL Server and Oracle auth/dialect differences behind a shared abstraction. Keep full admin parity, import/export, profiler/activity, role/user/permission UI, broad scripting, MSSQL admin/full T-SQL semantics, and Oracle SID/TNS/wallet/advanced auth/raw DDL/admin/TLS/PLSQL semantics out of scope until separately proven. |
