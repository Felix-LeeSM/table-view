...
| Valkey delta | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/models/connection.rs`, `src-tauri/src/models/data_source.rs`, `docs/product/README.md`, `docs/product/known-limitations.md` | Active capability-empty `valkey` `DatabaseType`/profile identity exists with server connection kind, product label, KV paradigm, marker-only backend adapter, and deferred `redis-command` compatibility target. No connection UI/runtime, fixture, live, or Redis-compatibility evidence exists. |
...
| Non-RDBMS runtime | Redis 와 MongoDB 는 runtime slice 가 있다. Valkey 는 capability-empty KV identity 만 있다. Elasticsearch/OpenSearch 는 live HTTP 전까지 fixture-backed 다. Cassandra/Scylla, DynamoDB, graph, vector, stream 은 gated candidate 다. 새 runtime promotion 은 active one-DBMS parity lane 뒤로 둔다. | `memory/engineering/architecture/data-source/memory.md`, `docs/phases/phase-28.md` |
