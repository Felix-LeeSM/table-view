...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  deferred 다. Oracle 은 bounded static SQL editor autocomplete vocabulary 만
  active 로 분리하고 runtime/query/catalog/edit/parser/Safe Mode promotion 은
  planned 상태로 둔다.
...
parser/Safe Mode metadata 까지만 active 이고, Oracle 은 planned RDBMS identity 에
bounded static SQL editor autocomplete vocabulary 만 더한다.
...
catalog/edit/runtime Safe Mode smoke/runtime fixture/E2E support 를 출시하지
...
| Oracle planned RDBMS identity | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/tests/backend_adapter_contract_profile.rs`, `src/lib/sql/sqlDialectProfile.test.ts`, `src/lib/sql/sqlCompletionRequest.test.ts`, `src/lib/sql/sqlWasmArtifact.test.ts`, `src-tauri/sql-parser-core/src/completion/completion_tests.rs` | `oracle` has `server` connection kind, `sql` language, `rdb` catalog, `tabular` result, `rdb-default` safety, and `declared-rdb` backend identity. Capabilities are empty. Bounded static SQL editor autocomplete vocabulary covers selected Oracle keywords, functions, package symbols, sequences, synonyms, and bind placeholders. Connection UI, runtime query/catalog/edit, Oracle SQL/PL/SQL parser/Safe Mode, service/SID/wallet/TNS behavior, runtime fixture/live evidence, desktop E2E, and full PL/SQL executable semantics are not claimed. |
...
| Parser/completion/runtime non-claim | `docs/product/query-language-support.md`, `docs/product/known-limitations.md` | Deferred language ids do not create active parser, completion, connection, query, catalog, edit, or E2E claims by themselves. MSSQL connection probe, bounded query runtime/result envelope, editor-assistance completion, static parser/Safe Mode metadata, and Oracle's bounded static editor autocomplete vocabulary are explicit exceptions; they do not create MSSQL catalog/edit/runtime Safe Mode smoke/runtime fixture/live/desktop E2E claims. Runtime changes must land in later source-specific PRs with matching smoke evidence. |
| H6 smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current E2E smoke proves PostgreSQL, MySQL, MariaDB, SQLite, DuckDB `.duckdb`, and MongoDB journeys only. MSSQL has focused connection-routing, live Docker probe, bounded query runtime/result-envelope, static parser/Safe Mode, and completion evidence, not current desktop E2E smoke. Oracle has focused profile/request/WASM/core completion evidence and a future runtime smoke inventory, not current desktop E2E claims. Wider candidates have future smoke inventories only. |
...
| MSSQL/Oracle | MSSQL now has lifecycle-backed SQL authentication, `SERVERPROPERTY` version probe evidence, bounded query runtime/result-envelope support, bounded editor completion vocabulary, and bounded static parser/Safe Mode metadata. Keep MSSQL catalog/edit/runtime Safe Mode smoke/runtime fixture/E2E widening split behind their own evidence and smoke routing. Keep Oracle capability-empty for runtime while allowing bounded static SQL editor autocomplete vocabulary only. Future promotion must split SQL Server and Oracle auth/dialect/catalog/safety contracts and add fixture/live evidence before any runtime support claim widens. |
