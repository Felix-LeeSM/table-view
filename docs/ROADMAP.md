...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  deferred 다. Oracle 은 service-name connection-only lifecycle 과 bounded static
...
| Parser/completion/runtime non-claim | `docs/product/query-language-support.md`, `docs/product/known-limitations.md` | Deferred language ids do not create active parser, completion, connection, query, catalog, edit, or E2E claims by themselves. MSSQL connection probe, bounded query runtime/result envelope, editor-assistance completion, static parser/Safe Mode metadata, and Oracle service-name connection plus bounded static editor autocomplete vocabulary are explicit exceptions; they do not create MSSQL catalog/edit/runtime Safe Mode smoke/runtime fixture/live/desktop E2E or Oracle query/catalog/edit/parser/Safe Mode/runtime fixture/live/desktop E2E claims. Runtime changes must land in later source-specific PRs with matching smoke evidence. |
