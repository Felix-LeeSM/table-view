...
| MSSQL/Oracle | Keep MSSQL to lifecycle-only SQL authentication connection testing, `SERVERPROPERTY` version probe evidence, and bounded editor completion vocabulary until a later PR splits query/catalog/edit/parser/Safe Mode/smoke support. Keep Oracle capability-empty for runtime while allowing bounded static SQL editor autocomplete vocabulary only. Future promotion must split SQL Server and Oracle auth/dialect/catalog/safety contracts and add fixture/live evidence before any runtime support claim widens. |
