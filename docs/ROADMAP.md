...
| H5 | First-class non-RDBMS | Redis/Valkey, Elasticsearch/OpenSearch, MongoDB 가 가장 명확한 non-RDBMS 사용자 workflow 를 덮는다. | MongoDB 는 whitelisted document workflow 로, Redis 는 bounded KV browser/value mutation + command/query/completion + representative smoke 로, Valkey 는 read-only KV runtime slice 로, Elasticsearch/OpenSearch 는 fixture-backed Search slice 로 support claim 이 정렬돼 있다. Search live HTTP 는 active parity lane 을 약화시키지 않고 promotion gate 를 통과할 때까지 deferred 다. |
...
| Valkey read-only runtime | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src/components/connection/ConnectionDialog/ConnectionDialogBody.tsx`, `src/components/workspace/KvSidebar.tsx`, `src/components/query/QueryTab/useQueryExecution.ts`, `src-tauri/src/commands/connection.rs`, `src-tauri/src/db/redis/mod.rs`, `src-tauri/src/models/data_source.rs`, `src-tauri/tests/redis_integration.rs`, `docs/product/README.md`, `docs/product/query-language-support.md`, `docs/product/known-limitations.md`, #488 | Active `valkey` `DatabaseType`/profile identity now has server connection kind, product label, KV paradigm, Valkey backend adapter, connection/key-browser/value-preview capabilities, and focused local testcontainer evidence for connect/key scan/value preview. Command query, mutation, Runtime Happy Path smoke, and Redis compatibility remain future gates. |
| Valkey fixture/live evidence strategy | `e2e/fixtures/seed.valkey.json`, `scripts/fixtures/dbms-seeds.test.ts`, `src-tauri/tests/redis_integration.rs`, `docs/contributor-guide/testing-and-quality.md`, `docs/product/README.md`, `docs/product/known-limitations.md`, #486/#488 | Valkey static Redis-compatible deterministic seed/reset contract remains fixture inventory. Current runtime support is read-only and local-first. Future promotion must wire Valkey-specific smoke and separately prove command dispatch, mutation, TTL/delete behavior, and Redis compatibility before those claims widen. |
| Valkey Redis compatibility matrix | `e2e/fixtures/valkey.redis-compatibility.json`, `scripts/fixtures/dbms-seeds.test.ts`, `docs/product/query-language-support.md`, `docs/product/README.md`, `docs/product/known-limitations.md`, `docs/contributor-guide/testing-and-quality.md`, #487 | Static matrix covers the bounded Redis command families, fixture reset commands, Valkey identity-detection requirement, known Table View Valkey deltas, and rejected Redis assumptions. Candidate families still require local Valkey runtime proof before support claims; admin/server-control, broad destructive, cluster, pub/sub, modules/functions, scripting, and consumer-group workflows remain separate promotion gates. |
...
| Non-RDBMS runtime | Redis 와 MongoDB 는 runtime slice 가 있다. Valkey 는 read-only KV runtime slice 와 static fixture inventory 가 있다. Elasticsearch/OpenSearch 는 live HTTP 전까지 fixture-backed 다. Cassandra/Scylla, DynamoDB, graph, vector, stream 은 gated candidate 다. 새 runtime promotion 은 active one-DBMS parity lane 뒤로 둔다. | `memory/engineering/architecture/data-source/memory.md`, `docs/phases/phase-28.md` |
...
| Redis/Valkey | Redis first slice is backend KV primitives, key browser/value preview/edit UI, bounded command query runtime/completion, current-DB/type-filtered key suggestions, and representative connect/scan/preview/GET/guarded-write/TTL/delete smoke. Valkey first slice is read-only connection/key scan/value preview with focused local testcontainer evidence. Define contracts and evidence for stream consumer UI, full CLI/admin parity, language-core parser/completion ownership, broader command coverage, cluster, pub/sub, modules, consumer-group management, Valkey command/mutation parity, Valkey smoke, and Redis compatibility before broader support claims. |
