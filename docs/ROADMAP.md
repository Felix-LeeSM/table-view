...
| H5 | First-class non-RDBMS | Redis/Valkey, Elasticsearch/OpenSearch, MongoDB 가 가장 명확한 non-RDBMS 사용자 workflow 를 덮는다. | MongoDB 는 whitelisted document workflow 로, Redis 는 bounded KV browser/value mutation + command/query/completion + representative smoke 로, Valkey 는 capability-empty KV identity 로, Elasticsearch/OpenSearch 는 fixture-backed Search slice 로 support claim 이 정렬돼 있다. Search live HTTP 는 active parity lane 을 약화시키지 않고 promotion gate 를 통과할 때까지 deferred 다. |
...
| Valkey delta | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/models/connection.rs`, `src-tauri/src/models/data_source.rs`, `docs/product/README.md`, `docs/product/known-limitations.md` | Active capability-empty `valkey` `DatabaseType`/profile identity exists with server connection kind, product label, KV paradigm, marker-only backend adapter, and deferred `redis-command` compatibility target. No connection UI/runtime, local Valkey container/testcontainer run, live, or Redis-compatibility evidence exists. |
| Valkey fixture/live evidence strategy | `e2e/fixtures/seed.valkey.json`, `scripts/fixtures/dbms-seeds.test.ts`, `docs/contributor-guide/testing-and-quality.md`, `docs/product/README.md`, `docs/product/known-limitations.md`, #486 | Valkey now has a static Redis-compatible deterministic seed/reset contract as fixture inventory only. Future promotion must run that seed against a local Valkey container or equivalent local-first fixture and then wire Valkey-specific smoke before Redis compatibility, runtime, or live support is claimed. |
...
| Non-RDBMS runtime | Redis 와 MongoDB 는 runtime slice 가 있다. Valkey 는 capability-empty KV identity 와 static fixture inventory 만 있다. Elasticsearch/OpenSearch 는 live HTTP 전까지 fixture-backed 다. Cassandra/Scylla, DynamoDB, graph, vector, stream 은 gated candidate 다. 새 runtime promotion 은 active one-DBMS parity lane 뒤로 둔다. | `memory/engineering/architecture/data-source/memory.md`, `docs/phases/phase-28.md` |
...
| Redis/Valkey | Redis first slice is backend KV primitives, key browser/value preview/edit UI, bounded command query runtime/completion, current-DB/type-filtered key suggestions, and representative connect/scan/preview/GET/guarded-write/TTL/delete smoke. Define contracts and evidence for stream consumer UI, full CLI/admin parity, language-core parser/completion ownership, broader command coverage, cluster, pub/sub, modules, consumer-group management, and Valkey-specific profile/runtime/fixture/live evidence before broader support claims. |
