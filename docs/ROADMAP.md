...
| Redis KV + command first slice | `src-tauri/src/db/redis/**`, `src-tauri/tests/redis_integration.rs`, `src/lib/tauri/kv.ts`, `src/components/query/QueryTab/useQueryExecution.kvDispatch.test.tsx`, `src/components/workspace/KvSidebar.tsx`, `src/components/workspace/KvMutationPanel.tsx`, #477, #480 | Backend KV primitives cover database/key scan, typed value reads, guarded string set, delete confirmation, TTL expire/persist, and bounded stream read. The sidebar now has bounded string/hash/list/set/zset mutation controls plus expire/persist/delete preview/confirm semantics, and unsupported/partial mutation surfaces fail visibly. The query tab can dispatch selected Redis read/write/TTL/stream commands through a backend allowlist and project results to the tabular grid. Stream consumer UI, language-core parser/completion ownership, broader command coverage, cluster/pubsub/modules/consumer-group flows are future work. |
