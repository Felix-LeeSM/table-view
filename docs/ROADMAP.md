...
| Valkey bounded command runtime | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src/components/connection/ConnectionDialog/ConnectionDialogBody.tsx`, `src/components/workspace/KvSidebar.tsx`, `src/components/query/QueryTab/useQueryExecution.ts`, `src-tauri/src/commands/connection.rs`, `src-tauri/src/db/redis/mod.rs`, `src-tauri/src/models/data_source.rs`, `src-tauri/tests/redis_integration.rs`, `docs/product/README.md`, `docs/product/query-language-support.md`, `docs/product/known-limitations.md`, #488/#489 | Active `valkey` `DatabaseType`/profile identity now has server connection kind, product label, KV paradigm, Valkey backend adapter, connection/key-browser/value-preview capabilities, `query.query` capability, and focused local testcontainer evidence for connect/key scan/value preview plus bounded Redis-compatible command query dispatch. Direct mutation controls, command completion, Runtime Happy Path smoke, and full Redis compatibility remain future gates. |
| Valkey fixture/live evidence strategy | `e2e/fixtures/seed.valkey.json`, `scripts/fixtures/dbms-seeds.test.ts`, `src-tauri/tests/redis_integration.rs`, `docs/contributor-guide/testing-and-quality.md`, `docs/product/README.md`, `docs/product/known-limitations.md`, #486/#488/#489 | Valkey static Redis-compatible deterministic seed/reset contract remains fixture inventory. Current runtime support is local-first and covers connection/key scan/value preview plus selected read/write/TTL/stream/destructive command query families. Future promotion must wire Valkey-specific smoke and separately prove direct mutation controls, command completion, broader Redis compatibility, and full compatibility before those claims widen. |
| Valkey Redis compatibility matrix | `e2e/fixtures/valkey.redis-compatibility.json`, `scripts/fixtures/dbms-seeds.test.ts`, `docs/product/query-language-support.md`, `docs/product/README.md`, `docs/product/known-limitations.md`, `docs/contributor-guide/testing-and-quality.md`, #487/#489 | Static matrix covers the bounded Redis command families, fixture reset commands, Valkey identity-detection requirement, known Table View Valkey deltas, and rejected Redis assumptions. Local Valkey runtime evidence now covers selected bounded command families, but admin/server-control, broad destructive, cluster, pub/sub, modules/functions, scripting, consumer-group workflows, command completion, direct mutation UI, smoke, and full compatibility remain separate promotion gates. |
...
| Redis/Valkey | Redis first slice is backend KV primitives, key browser/value preview/edit UI, bounded command query runtime/completion, current-DB/type-filtered key suggestions, and representative connect/scan/preview/GET/guarded-write/TTL/delete smoke. Valkey first slice is connection/key scan/value preview plus bounded command query with focused local testcontainer evidence. Define contracts and evidence for stream consumer UI, full CLI/admin parity, language-core parser/completion ownership, broader command coverage, cluster, pub/sub, modules, consumer-group management, Valkey direct mutation controls, Valkey command completion, Valkey smoke, and full Redis compatibility before broader support claims. |
