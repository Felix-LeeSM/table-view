...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  query runtime/result envelope, and bounded static SQL editor autocomplete
  vocabulary 까지만 active 이고 catalog/edit/parser/Safe Mode/runtime fixture/live
  promotion 은 planned 상태로 둔다.
...
| H6 | 더 넓은 paradigm | Cassandra, DynamoDB, graph DB, vector DB, stream source 는 active work 전 명확한 workflow proof 가 필요하다. | MSSQL 은 SQL authentication connection test + `SERVERPROPERTY` version probe + bounded SELECT/DML query runtime/result envelope + bounded T-SQL editor completion vocabulary + bounded static parser/Safe Mode metadata 로 정렬되고, Oracle 은 service-name connection test + bounded SELECT/DML query runtime/result envelope + bounded static editor autocomplete vocabulary 로 정렬된다. Wider source 는 candidate-only 계약으로 정렬된다. Profile target, connection kind, language, catalog model, result envelope, safety policy, fixture strategy 가 문서화되고 MSSQL catalog/edit/runtime Safe Mode smoke/runtime fixture/E2E/Oracle catalog/edit/parser/Safe Mode/SID/TNS/wallet/TLS/fixture/live/E2E 밖의 runtime support claim 은 생기지 않는다. |
...
lifecycle, bounded SELECT/DML query runtime/result envelope, and bounded static
...
catalog/edit/parser/Safe Mode/SID/TNS/wallet/TLS/runtime fixture/live/E2E support 를 출시하지 않으며, MSSQL/Oracle
...
| Oracle bounded query-plus-completion support | `src/components/connection/forms/OracleFormFields.tsx`, `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/oracle.rs`, `src-tauri/src/db/oracle/runtime.rs`, `src-tauri/tests/backend_adapter_contract_profile.rs`, `src/lib/sql/sqlDialectProfile.test.ts`, `src/lib/sql/sqlCompletionRequest.test.ts`, `src/lib/sql/sqlWasmArtifact.test.ts`, `src-tauri/sql-parser-core/src/completion/completion_tests.rs` | `oracle` has `server` connection kind, `sql` language, `rdb` catalog, `tabular` result, `rdb-default` safety, a backend `oracle` adapter with `Lifecycle` and `RelationalQuery`, service-name connection UI/runtime test/connect, bounded SELECT tabular envelopes, transactional DML batch commit/dry-run, cancellation/error surfacing, and bounded static SQL editor autocomplete vocabulary for selected Oracle keywords, functions, package symbols, sequences, synonyms, and bind placeholders. SID, TNS alias, wallet/TLS, catalog/edit/structured DDL, Oracle SQL/PL/SQL parser/Safe Mode, runtime fixture/live evidence, desktop E2E, and full PL/SQL executable semantics are not claimed. |
...
| Parser/completion/runtime non-claim | `docs/product/query-language-support.md`, `docs/product/known-limitations.md` | Deferred language ids do not create active parser, completion, connection, query, catalog, edit, or E2E claims by themselves. MSSQL connection probe, bounded query runtime/result envelope, editor-assistance completion, static parser/Safe Mode metadata, and Oracle service-name connection plus bounded SELECT/DML query runtime/result envelope plus bounded static editor autocomplete vocabulary are explicit exceptions; they do not create MSSQL catalog/edit/runtime Safe Mode smoke/runtime fixture/live/desktop E2E or Oracle catalog/edit/parser/Safe Mode/runtime fixture/live/desktop E2E claims. Runtime changes must land in later source-specific PRs with matching smoke evidence. |
| H6 smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current E2E smoke proves PostgreSQL, MySQL, MariaDB, SQLite, DuckDB `.duckdb`, and MongoDB journeys only. MSSQL has focused connection-routing, live Docker probe, bounded query runtime/result-envelope, static parser/Safe Mode, and completion evidence, not current desktop E2E smoke. Oracle has focused service-name connection evidence, bounded backend query runtime unit/profile evidence, and profile/request/WASM/core completion evidence, not fixture/live or current desktop E2E claims. Wider candidates have future smoke inventories only. |
...
| MSSQL/Oracle | MSSQL now has lifecycle-backed SQL authentication, `SERVERPROPERTY` version probe evidence, bounded query runtime/result-envelope support, bounded editor completion vocabulary, and bounded static parser/Safe Mode metadata. Oracle now has service-name connection lifecycle, bounded SELECT/DML query runtime/result-envelope support, and bounded static SQL editor autocomplete vocabulary. Keep MSSQL catalog/edit/runtime Safe Mode smoke/runtime fixture/E2E widening split behind their own evidence and smoke routing. Keep Oracle catalog/edit/parser/Safe Mode/SID/TNS/wallet/TLS/fixture/live/E2E support split behind source-specific evidence before any support claim widens. |
