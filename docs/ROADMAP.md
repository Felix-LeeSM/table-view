...
- 기존 지원 DBMS 하나를 데스크톱 DB 클라이언트 수준의 query/workbench parity 까지 끌어올리는
  ...
  query/admin 은 deferred 로 유지한다. MSSQL/Oracle 은 8.11/8.12 에서 full
  support lane 으로 승격됐지만 server/session admin, native cancel, deep dialect
  scripting 은 deferred 로 둔다.
...
| H6 | 더 넓은 paradigm | Cassandra, DynamoDB, graph DB, vector DB, stream source 는 active work 전 명확한 workflow proof 가 필요하다. | MSSQL/Oracle 은 full RDBMS support claim 으로 승격되고 fixture smoke gate 에 연결된다. Wider source 는 candidate-only 계약으로 남는다. |
...
H6 wider source 는 **RDBMS promotion + candidate contract 정합성 gate**다.
MSSQL/Oracle 은 `DatabaseType`/profile identity, runtime adapter, SQL language
surface, catalog/query/edit path, bounded DDL/admin guard, completion vocabulary,
catalog completion, fixture seed, and desktop smoke gate 를 갖는 full RDBMS support
claim 으로 승격됐다. Cassandra/Scylla, DynamoDB, graph, vector, stream 은 아직
active `DatabaseType`/profile/runtime 이 없는 candidate 다.

| Gate | Current owner | H6 boundary |
|---|---|---|
| MSSQL full support contract | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/mssql.rs`, `scripts/fixtures/mssql.ts`, `e2e/smoke/mssql.spec.ts`, `scripts/e2e-smoke-ci.sh` | `mssql` has server connection UI, SQL authentication, runtime query execution, schema/catalog metadata browse, tabular results, row edit with SQL preview, bounded DML/DDL/admin execution, SQL vocabulary/catalog completion, fixture seed/reset, and Runtime Happy Path smoke. Deferred: named instances, Windows auth, server/session admin, native cancel, deep T-SQL scripting/procedure body authoring, broad import/export/admin parity. |
| Oracle full support contract | `src/types/connection.ts`, `src/types/dataSource.ts`, `src/types/dataSourceRuntime.ts`, `src-tauri/src/db/oracle.rs`, `scripts/fixtures/oracle.ts`, `e2e/smoke/oracle.spec.ts`, `scripts/e2e-smoke-ci.sh` | `oracle` has server connection UI, service-name connection (`XEPDB1` local default), runtime query execution, schema/catalog metadata browse, tabular results, row edit with SQL preview, bounded DML/DDL/admin execution, SQL vocabulary/catalog completion, fixture seed/reset, and Runtime Happy Path smoke. Deferred: SID/wallet/TNS, server/session admin, native cancel, deep PL/SQL/package scripting, broad import/export/admin parity. |
...
| Parser/completion/runtime non-claim | `docs/product/query-language-support.md`, `docs/product/known-limitations.md` | MSSQL/Oracle have SQL editor vocabulary and catalog completion for the supported runtime slice, but deep T-SQL/PL/SQL scripting is not claimed. Deferred language ids do not create active parser, completion, connection, query, catalog, edit, or E2E claims. |
| H6 smoke matrix | `docs/contributor-guide/testing-and-quality.md` | Current E2E smoke proves PostgreSQL, MySQL, MariaDB, MSSQL, Oracle, SQLite, DuckDB `.duckdb`, MongoDB, Redis, Valkey, Elasticsearch, and OpenSearch journeys. Wider candidates have future smoke inventories only. |
...
H6 umbrella closure means MSSQL/Oracle support claims, contracts, and smoke
routing are aligned, while wider candidate contracts remain non-runtime. It does
not mean server/session admin, native cancel, deep dialect scripting, or
Cassandra/Scylla, DynamoDB, graph, vector, stream runtime support has shipped.
...
| MSSQL/Oracle | Keep support to the documented full RDBMS slice: connection UI, query execution, schema/catalog metadata, row edit, bounded DDL/admin, SQL vocabulary/catalog completion, and fixture smoke gates. Server/session admin, native cancel, deep T-SQL/PL/SQL scripting, and broad import/export/admin parity remain follow-up. |
...
2. connect/browse/query 만 노출하는 runtime 을 하나 더 붙이는 것보다, 기존 runtime
   깊이를 우선한다.
3. Runtime promotion freeze: Search live query/admin 과 기타 새 DBMS lane 은
