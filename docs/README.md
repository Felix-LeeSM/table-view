...
**cap 은 파일 하나마다 걸리고 묶음 합계에는 걸리지 않는다.** 상한은 파일당
**33,000 chars** 이고, 단위는 `wc -m` 의 문자(code point)이지 바이트가 아니다.
목적은 도입 당시에 적어 둔 그대로 agent 가 읽을 때의 context 부하를 제한하는
것이다. 규율로만 두고 자동 검사는 두지 않는다 (아래 「자동 검사를 두지 않는 이유」).

### 모집단은 이 명령이 정의한다

묶음을 산문으로 정의하던 자리다. 앞판은 `git show 47eb7e00:docs/README.md` 의
「분량 cap」에 있다. 디렉터리 이름은 적혀 있었지만 그 아래 하위 디렉터리가 드는지 ·
`docs root` 가 어디까지인지 · 단위가 `wc -m` 인지 `wc -c` 인지는 안 적혀 있었고,
그래서 읽는 사람마다 다른 집합을 잡아 값이 갈렸다(#2266). 명령을 SOT 로 둔다.
...
- 로케일이 UTF-8 이 아니면 `wc -m` 이 문자가 아니라 바이트를 센다. **같은 rev 라도
  로케일이 다르면 합계가 갈린다.** `47eb7e00` 에서 `LC_ALL=en_US.UTF-8` 은
  564,569 이고 `LC_ALL=C` 는 586,884 다(파일 수는 양쪽 46). 위 명령의 `LC_ALL` 만
  바꿔 재현한다. 쓰려는 로케일이 문자를 세는지는 한 줄로 확인한다.
  `printf '가나다' | LC_ALL=en_US.UTF-8 wc -m | tr -d ' '` 이 `3` 이 아니면 그
  로케일로는 못 잰다.
...
| cap 초과 파일 | 0 |
| 묶음 합계 | 564,569 chars / 46 파일이고 **cap 이 재는 값이 아니다** |

합계는 위 명령 뒤에 `| awk '{s+=$1} END {print s, NR}'` 를 붙이면 나온다. 합계를
같이 적어 두는 이유는 이 값을 cap 과 견주던 오독이 실제로 있었기 때문이다 (#2266).

표의 값은 `REV=47eb7e00` 으로 잰 것이다. 위 스니펫의 기본값은 `REV=HEAD` 라 그
뒤에 쌓인 커밋만큼 다른 값을 낸다(이 절 자신도 모집단에 든다). 다시 잴 때는 rev 를
바꿔 돌리고 그 rev 를 값 옆에 같이 적는다.

### 왜 33,000 인가

앞선 값은 120,000 이었고, 위에 rev 를 적어 둔 앞판 본문은 그 수의 근거를 안 적는다.
근거는 히스토리에 있다. 도입 커밋
`3b3d38d2` (#970) 이 파일당 검사 스크립트 `scripts/hooks/check-doc-size.sh` 를 같이
넣었고, 그 스크립트는 파일마다 `wc -m` 을 재 threshold 와 견줬다. 넘어도 막지 않는
advisory 였다(스크립트 헤더 「기본: 경고만 출력 (exit 0)」, 커밋 메시지
「chars cap(120000, advisory)」).

`git log -1 --format=%B 3b3d38d2` 이 그 커밋 메시지 전문을 찍는다. 거기에 cap 도입
사실 · 도입 시 위반 0 · 「threshold ratchet 과 한 줄 cap 은 후속 Phase.」 는 적혀
있는데 **120,000 이라는 수를 어떻게 잡았는지는 안 적혀 있다.** 그래서 근거로 쓸 수
있는 것은 그 수가 그때의 최대에 대해 가졌던 비율이다. 위 모집단 명령을 `3b3d38d2`
에 대고 재면 그때의 최대가 106,766 (`docs/contributor-guide/testing-and-quality.md`)
...
그 스크립트는 `0b6fd9fd` (#1860) 가 `scripts/hooks/policy/` 로 옮겼고, 2026-07-30
`6cced3ab` (#2033) 의 workflow 철거가 옮긴 본체와 원래 자리에 남은 shim 을 같이
지웠다. 다음 날 `46ca4799` (#2034) 가 그 스크립트를 가리키던 문장을 「cap 은
규율로만 남았고 자동 검사가 없다」로 갈았다.

파일당이라는 축은 그 스크립트와 위 커밋 메시지가 갖고 있었다. 이 절 산문 쪽은
아래 다섯 문구로 히스토리를 훑으면 값이 전부 0 이다. 스크립트가 사라진 뒤 남은
산문이 묶음 합계로 읽힌 것이 #2266 이다.

```sh
# rev 를 못 박아야 이 값이 뒤 커밋에 안 흔들린다 (HEAD 로 재면 이 절 자신이 잡힌다)
...
→ **33,000** (`awk 'BEGIN{printf "%.0f\n", 29472*120000/106766}'`). 도입 커밋이
「threshold ratchet 과 한 줄 cap 은 후속 Phase.」로 미뤄 둔 그 ratchet 이다. 120,000
은 오늘 최대의 4.07 배(`awk 'BEGIN{printf "%.2f\n", 120000/29472}'`)라 상한으로
쓰기에 너무 멀고, 33,000 에서는 위 표대로 초과 파일이 0 이라 이 값이 지금 어느
문서에도 축소를 요구하지 않는다. 다음에 또 조일 때도 같은 구성을 쓴다. 그 시점의
최대에, 직전 cap 이 자기 시점의 최대에 대해 가졌던 배수(지금이면 33,000 ÷ 29,472)를
곱한다.

### 자동 검사를 두지 않는 이유

#2266 이 기록한 실패는 「파일이 몰래 cap 을 넘었다」가 아니라 「cap 이 무엇을
재는지가 읽는 사람마다 갈렸다」다. 위 명령이 그것을 닫고 CI 잡은 안 닫는다. 위
표대로 지금 초과가 0 이고 최대가 cap 보다 3,528 chars 낮아(29,472 대 33,000),
게이트를 넣어도 당장 잡을 파일이 없다.

다시 볼 조건은 이렇다. 위 명령에서 33,000 을 넘은 파일이 나왔는데 리뷰가 그것을 못
잡은 사례가 생기면 그때 만든다. 베낄 형태는 `scripts/check-memory-doc-size.sh` 다.
...
- 현재 사용자-visible 사실이면 `product/**`, 그중 per-source boundary 행은
...
범위가 넘치기 때문이다. `--no-ignore-dot` 은 `.ignore`·`.rgignore` 만 끄고
`.gitignore` 는 남긴다.
...
훅이 검색어에 걸린다. 검색어와 사본에 따라 달라지므로 파일 이름을 명시하지 않는다.

### 이 harness 의 `grep` 은 `rg` 와 다른 집합을 낸다

**`grep` 은 바이너리가 아니라 셸 함수다.** `type grep` 이 그렇게 답한다. 본체가
ugrep 을 이렇게 부른다 (스냅샷 경로는 세션마다 다르므로 명시하지 않는다).
...
`git grep` 은 커밋을 인자로 받아 rev 를 고정했고, `grep -rl`·`rg -l` 은 워킹트리를
읽으므로 위 체크아웃이 rev 를 고정한다. 두 필터가 이 문서 자신을 빼므로 위 「둘은
근사지 등식이 아니다」의 자기-계수 문제는 여기 없다.
...
「3단계」가 그 자리다. 좌표를 안 쓰는 이유는 두 파일이 편집될 때마다 줄 번호가
밀려서다.
...
아래 재현은 커밋 `f41e0a10` 을 고정하고 돌린 것이다. 이 문서 자신이 검색어를 품고
있어 현재 트리로 돌리면 자기를 세고, 커밋을 고정하면 값이 앞으로도 그대로다.
...
실패해 바로 드러난다. 조용한 쪽은 `git grep` 이고, 위 「저장소 전수」가 전수 도구로
...
값은 2026-08-07 에 `f41e0a10` 을 고정하고 잰 것이다 (zsh 5.9 / bash 3.2.57 /
