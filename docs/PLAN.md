...
| 14 | 223+ | refactor | P10 step 2-4 (schemaStore optimistic refresh / connectionStore session persistence / IPC bridge 분리) | step 2: `schemaStore` 의 `dropTable` / `renameTable` 의 `try { reload } catch { fallback }` → use-case hook. step 3: connectionStore session persistence (`persistFocusedConnId` / `persistActiveStatuses` / `hydrateFromSession`). step 4 (가장 위험, 마지막): `attachZustandIpcBridge` 모듈-load attach 분리. **사용자 hooks/lib 작업 안정 후 진입**. |
