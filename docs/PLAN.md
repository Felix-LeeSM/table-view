...
#### F2.1: 데이터베이스/스키마 트리
- **Done when:**
  - [ ] 활성 연결의 데이터베이스 목록이 트리로 표시됨
  - [ ] 데이터베이스 노드 확장 시 스키마 목록 표시 (PostgreSQL)
...
#### F2.2: 테이블 목록
- **Done when:**
  - [ ] 선택한 스키마의 모든 테이블이 목록으로 표시됨
  - [ ] 테이블 이름 옆에 row count 표시 (옵션)
  - [ ] 테이블 클릭 시 데이터 탭 열림 (FA-3 참조)
...
#### F3.1: 데이터 그리드 뷰
- **Done when:**
  - [ ] 테이블 선택 시 데이터가 그리드(표) 형태로 표시됨
  - [ ] 페이지네이션 지원 (기본 300 rows/page)
  - [ ] 컬럼 헤더에 컬럼명과 데이터 타입 표시
  - [ ] 컬럼 폭 조절 가능 (드래그)
  - [ ] 컬럼 순서 변경 가능 (드래그앤드롭)
  - [ ] 가로 스크롤로 많은 컬럼 탐색 가능
  - [ ] NULL 값이 시각적으로 구분되어 표시됨
...
**Phase 2 Done when:**
- [ ] 연결 시 사이드바에 DB → Schema → Tables 트리 전개
- [ ] 테이블 선택 시 Structure 탭과 Data 탭 열림
- [ ] Data 탭에서 페이지네이션된 데이터 그리드 표시
- [ ] 컬럼 정렬 및 기본 필터 동작
