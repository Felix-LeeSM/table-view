...
| 16 | 225+ | refactor | P10 step 3b-4 (persist 3 site / IPC bridge) | step 3b: `persistFocusedConnId` / `persistActiveStatuses` 3 site (`connectToDatabase` / `disconnectFromDatabase` / `setFocusedConn` body) — cross-window broadcast / IPC bridge ordering invariant + 5 callers update. step 4 (가장 위험, 마지막): `attachZustandIpcBridge` module-load attach 분리. **사용자 hooks/lib 작업 안정 후 진입**. |
