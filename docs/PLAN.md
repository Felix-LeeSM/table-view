...
| 6 | 231+ | feature | (TBD) | Phase 27 sprint 6 폴리시 후보: (a) reorder ↑/↓ buttons (column / index / FK / CHECK / UNIQUE rows), (b) table-level `COMMENT ON TABLE`, (c) schema picker position move (header → form section), (d) type combobox color coding (Sprint 230 의 `type_kind` 필드 사용 — base / domain / enum / range / composite), (e) cross-tab visual feedback / empty column-name handling, (f) `invalidatePostgresTypesCache` 를 `connectionStore.disconnect` lifecycle 에 wire (Sprint 230 에서 free function 만 export). |
