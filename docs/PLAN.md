...
- **Done when:**
  ...
  - [ ] 연결 수준과 스키마/테이블 수준의 명확한 시각적 분리 (구분선 또는 배경색 차이)
  - [ ] 각 계층 레벨(연결 > 스키마 > 테이블)에 서로 다른 아이콘 스타일 적용
...
- **Done when:**
  ...
  - [ ] 페이지네이션 지원 (기본 300 rows/page, 설정 가능)
  ...
  - [ ] 페이지 크기 선택기 제공 (100, 300, 500, 1000)
  - [ ] 첫 페이지 / 마지막 페이지 이동 버튼
  - [ ] 특정 페이지 번호로 직접 이동 (Jump to page)
...
- **Done when:**
  ...
  - [ ] 쿼리 에디터 상단 툴바에 실행(Run) 버튼 표시 (▶ 아이콘)
  - [ ] 쿼리 실행 중 툴바에 취소(Cancel) 버튼으로 전환 (■ 아이콘)
  - [ ] Cmd+Return (macOS) / Ctrl+Return (Windows/Linux) 단축키 안내 표시
...
- **Done when:**
  ...
  - [ ] 앱 재시작 시 이전 탭 상태 복원
  - [ ] 테이블 클릭 시 "미리보기" 탭으로 열림 (임시)
  - [ ] 다른 항목 탐색 시 기존 미리보기 탭이 교체됨
  - [ ] 편집, 페이지 이동, 또는 명시적 핀 클릭 시 영구 탭으로 승격
  - [ ] 미리보기 탭은 이탤릭체 제목으로 영구 탭과 시각적 구분
...
> 참고: F4.6의 기능은 F5.3 쿼리 로그 패널에 통합됨
...
- **Done when:**
  ...
  - [x] Cmd+Return / Ctrl+Return: 쿼리 실행
  - [x] Cmd+. / Ctrl+.: 쿼리 취소
  - [ ] Cmd+,: 설정 열기
...
- **Done when:**
  - [ ] 사이드바 모든 인터랙티브 요소에 텍스트 선택 방지 (select-none)
  - [ ] 연결 항목, 그룹 헤더, 스키마/테이블 노드, 컨텍스트 메뉴 포함
  - [ ] 더블클릭/드래그 시 텍스트 선택되지 않음
...
- [ ] Light/Dark 테마 전환 동작
- [ ] 핵심 단축키 모두 동작
- [ ] 테이블 클릭 시 미리보기 탭 동작, 승격 로직 정상
- [ ] 쿼리 에디터에 실행/취소 버튼 표시
- [ ] 페이지 크기 선택 및 페이지 이동 동작
- [ ] 사이드바 인터랙티브 요소 텍스트 선택 방지
...
| 쿼리 즐겨찾기 (F4.7) | F4.7 | |
| 결과 분할 (F4.8) | F4.8 | |
| 콘솔 로그 (F5.3) | F5.3 | F5.6 이후 구현, F4.6 통합 |
| 패널 관리 시스템 (F5.6) | F5.6 | 하단/우측 패널, 접기/펼치기 |
...
## Open Design Questions (설계 논의 필요)

### Q1: 장시간 실행 쿼리 전략 — 해결됨 (옵션 D 채택)

**전략: 설정 가능한 타임아웃 + 진행 표시기 + 의심 쿼리 경고**
- 기본 쿼리 타임아웃: 60초 (사용자 설정 가능, 0 = 무제한)
- 진행 표시기: 경과 시간 실시간 표시 + 눈에 띄는 취소 버튼
