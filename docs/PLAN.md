...
| 3 | 228+ | feature | (TBD) | 후보: Phase 27 sprint 3 (Indexes tab functional — `+/−` rows + name/columns/type[btree/hash/gin/gist]/unique + chained `tauri.createIndex` post-CREATE TABLE per partial-atomic policy C) / Phase 27 sprint 4 (Foreign Keys + CHECK/UNIQUE constraints) / Phase 26 (Trigger Read+Write) / Phase 13 (PG preview tab parity, multi-window 파트는 e2e 복구 후). |
