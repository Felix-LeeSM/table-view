...
| 15 | 224+ | refactor | P10 step 3-4 (connectionStore session / IPC bridge) | step 3: connectionStore session persistence (`persistFocusedConnId` / `persistActiveStatuses` / `hydrateFromSession`). step 4 (가장 위험, 마지막): `attachZustandIpcBridge` 모듈-load attach 분리. **사용자 hooks/lib 작업 안정 후 진입**. |
