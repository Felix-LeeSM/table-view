...
| Backend contract test ownership | `src/types/adapterContractTestMatrix.ts` — query · result · catalog · explain · completion · safety are rows in that file and each row carries its own `childIssue`; `src/types/adapterConformance.test.ts` locks the row set and order |
