...
| semantic zoom 3단계 | shipped | #2149, ADR 0054 (2) | 고정 컬럼 상한 폐기. elkjs 슬롯은 full-detail 높이 그대로라 줌이 레이아웃을 다시 돌리지 않는다 — 대신 줌아웃해도 다이어그램이 촘촘해지지 않는다. 성능 절반은 아래 행. |
| FK 엣지 컬럼 앵커 + cardinality | shipped | #2151 | 엣지가 카드가 아니라 FK 컬럼 행에 붙고, 줌 단계가 그 행을 안 그리면 카드 가장자리로 물러난다. 표기는 한 행으로 고정된 끝의 개수만 세므로 어느 쪽이 1인지는 말하지 않고, 도착한 metadata 만 읽는다 — 도착 상태별로 무엇이 나오는지는 `src/components/schema/erdGraphModel.test.ts` 의 `cardinality arrival states` 표가 고정한다. |
...
| 가상 FK 렌더/추론/referential action | v2 | #1659 / #1668 / #1665, ADR 0055 | 편집 계열은 v2 이월. 렌더·저장·reconcile 은 #1659 에서 갈라진 #2150 이 배송했고, 추론과 referential action 이 이 행에 남는다. |
