...
| FK 엣지 컬럼 앵커 + cardinality | shipped | #2151 | 엣지가 카드가 아니라 FK 컬럼 행에 붙고, 줌 단계가 그 행을 안 그리면 카드 가장자리로 물러난다. 표기는 한 행으로 고정된 끝의 개수만 세므로 어느 쪽이 1인지는 말하지 않고, 컬럼 metadata 가 도착하기 전 창에서는 N:M 으로 뜬다. |
