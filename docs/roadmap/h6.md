...
전까지 out of scope 다. 예외는 하나다 — SQL Server 는 `sys.server_principals` 기반
read-only users/roles listing 을 `VIEW ANY DEFINITION` probe 뒤에서 제공한다
(#1077 Stage 2). Oracle 은 이 경계를 그대로 유지한다.
