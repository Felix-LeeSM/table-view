...
seed-spec inventory 는 그대로 남기되, MSSQL 에는 connection.test/connect/ping 과
catalog/query/cancel/tabular result, key-projected editRows 만 허용하고, Oracle 에는
...
matching runtime, contract, docs, and smoke evidence 가 갖추어지기 전에는 만들지 않는다.
Cassandra/Scylla, DynamoDB, graph, vector, stream 은 candidate-only 다.
이들에는 아직 active `DatabaseType`/profile/runtime/parser/completion 도, fixture/live
evidence 도, E2E smoke claim 도 없다.
...
advanced auth/structured DDL/raw DDL/admin/PLSQL/future smoke semantics 는 별도의
source-specific evidence 가 나오기 전까지 out of scope 다.
