...
import/export, profiler/activity, user/role write management (create/alter/drop),
permission-editing UI, broad scripting,
...
전까지 out of scope 다. SQL Server 는 이 경계에서 두 갈래가 이미 열려 있다 —
`sys.dm_exec_sessions` 기반 activity/kill session (#1073) 과 `sys.server_principals`
기반 read-only users/roles listing (`VIEW ANY DEFINITION` probe 뒤, #1077 Stage 2).
둘 다 read-only 조회이거나 세션 종료까지이고, user/role write management 와
permission-editing UI 는 그대로 out of scope 다. Oracle 은 이 경계를 그대로 유지한다.
