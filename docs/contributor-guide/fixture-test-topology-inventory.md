...
| `e2e/fixtures/seed-smoke.ts` | PostgreSQL, MongoDB, MySQL, MariaDB, Redis, Valkey, Elasticsearch, OpenSearch; MSSQL/Oracle only when explicitly selected | authored smoke seed orchestrator | `scripts/e2e-smoke-ci.sh`, `scripts/fixtures/dbms-seeds.test.ts` | Runtime Happy Path seed routing for wired external-service smoke targets; dormant seed inventory for MSSQL/Oracle | `docs/product/README.md` Current Support Snapshot and Fixture Coverage Snapshot rows for the routed DBMSs; declared-only rows for MSSQL/Oracle | invoked before `scripts/e2e-smoke-ci.sh` runs wired specs; maps SQLite/DuckDB to no external seed; MSSQL/Oracle are not in the default seed set | consumed; keep |
...
| `e2e/fixtures/seed.oracle.sql` | `oracle` | dormant E2E | no | no routine runtime cost while declared-only | dormant until source-specific connection.test promotion | seed/spec inventory only; no Oracle support claim | keep dormant |
