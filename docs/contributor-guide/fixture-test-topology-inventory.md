...
| `e2e/fixtures/seed-smoke.ts` | PostgreSQL, MongoDB, MySQL, MariaDB, Redis, Valkey, Elasticsearch, OpenSearch, MSSQL, Oracle | authored smoke seed orchestrator | `scripts/e2e-smoke-ci.sh`, `scripts/fixtures/dbms-seeds.test.ts` | Runtime Happy Path seed routing for wired external-service targets | `docs/product/current-support-snapshot.md` and `docs/product/fixture-coverage-snapshot.md` for the routed DBMSs; MSSQL/Oracle bounded runtime and #907 smoke entries | invoked before that script runs wired specs; maps SQLite/DuckDB to no external seed | consumed; keep |
