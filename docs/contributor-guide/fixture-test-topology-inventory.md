...
| `e2e/fixtures/seed.mssql.sql` | `mssql` | blocking E2E | yes | existing on-demand MSSQL route; no new #753 cost | higher cold-start risk; workflow waits before WDIO | bounded SQL Server support only | keep blocking |
| `e2e/fixtures/seed.oracle.sql` | `oracle` | blocking E2E | yes | existing on-demand Oracle route; 12-minute startup step | higher cold-start risk; workflow waits before WDIO | bounded Oracle support only | keep blocking |
