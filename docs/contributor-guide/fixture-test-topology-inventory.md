...
| `e2e/fixtures/seed.oracle.sql` | Oracle | authored Oracle seed | `e2e/fixtures/seed-smoke.ts`, `scripts/fixtures/dbms-seeds.test.ts`, `src-tauri/tests/oracle_smoke_boundary_probe.rs` ignored probes | dormant E2E seed inventory; not routine smoke, structured DDL, full parser/completion promotion, or PL/SQL evidence | Oracle #905/#906 bounded runtime/edit row; Fixture Coverage Snapshot Oracle row | explicit `oracle` seed target only; not wired into routine smoke | consumed; keep |
...
| `e2e/fixtures/seed.oracle.sql` | `oracle` | dormant E2E | no | no routine runtime cost beyond #905/#906 focused runtime/edit evidence | dormant until #907 routine Oracle smoke promotion | seed/spec inventory only; no Oracle structured DDL/PLSQL/runtime-smoke claim | keep dormant |
