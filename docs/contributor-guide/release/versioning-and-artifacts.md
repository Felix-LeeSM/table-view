...
| Checksums | `Upload SHA256 checksums` step | Every uploaded bundle file gets a sibling `.sha256` in standard `shasum -a 256` format. The step ends the leg red when an `artifactPaths` entry resolves to nothing, and when nothing was hashed at all, so a lane that produced no checksum cannot finish green (#2207: Git Bash cannot stat the backslash paths `tauri-action` reports on Windows, and `[ -f "$f" ] || continue` turned that into a silent skip — `v0.7.0` and `v0.7.1` published `.msi` and `.exe` with no checksum under a green step). The macOS `.app` bundle is a directory, has no sidecar by design, and is logged as skipped. `scripts/release/checksum-sidecars.test.sh` pins that shape. |
