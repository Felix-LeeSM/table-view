...
| SQLite | File-backed workflow for open/create, browse, read query, writable-file DML, primary-key row edit, read-only rejection, and internal app-state DB separation. The Structure dialog claims table creation only; the adapter runs the natively supported drop/rename/column/index DDL since #1804 without a capability flip. `ALTER TABLE` rebuilds, constraint DDL, and sqlite-cli execution remain unsupported. | [`docs/product/README.md`](../../product/README.md), [`known-limitations.md`](../../product/known-limitations.md) |
