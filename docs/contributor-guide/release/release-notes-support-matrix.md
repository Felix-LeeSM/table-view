...
| SQLite | File-backed workflow for open/create, browse, read query, writable-file DML, primary-key row edit, read-only rejection, and internal app-state DB separation. The Structure surface offers table creation, including the btree indexes declared inside the Create Table dialog, which the adapter applies in the same transaction as the CREATE TABLE (the dialog's hash/gin/gist methods are refused before the file is touched); the adapter also runs the natively supported drop/rename/column/index DDL since #1804 without a capability flip, so those standalone entry points stay hidden. `ALTER TABLE` rebuilds, constraint DDL, and sqlite-cli execution remain unsupported. | [`docs/product/README.md`](../../product/README.md), [`known-limitations.md`](../../product/known-limitations.md) |
