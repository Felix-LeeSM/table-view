...
| MariaDB deltas | Keep `RETURNING`, routine/default behavior, procedure-management, completion-runtime, admin/import/export, and full workbench claims behind separate MariaDB-specific promotion gates. |
...
| MariaDB connect -> browse/edit -> SELECT/DML batch -> cancellation + history evidence | `e2e/smoke/mariadb.spec.ts`, `e2e/fixtures/seed.mariadb.sql`, `scripts/e2e-smoke-ci.sh`, `.github/workflows/e2e-smoke.yml` | H2 MariaDB engine baseline proves the same routine connect, seeded table browse, SELECT result grid, DML batch result-envelope, row edit, cancellation/retry, and history/source-label journey against MariaDB itself. It does not widen MariaDB-only `RETURNING`, routine/default behavior, procedure-management, completion-runtime, admin/import/export, or full workbench claims. |
...
| MySQL/MariaDB/SQLite support claims | MySQL and MariaDB now have routine runtime smoke baselines plus focused evidence; SQLite remains unit/integration/fixture-backed until its runtime smoke is promoted. | Add DBMS-specific runtime smoke when each remaining parity lane becomes active. |
...
|---|---|---|
| Active parity lane selection | `docs/ROADMAP.md` H2 진행 기준 | PostgreSQL remains the strongest lane; MySQL and MariaDB have narrower wired runtime-smoke baselines. SQLite/DuckDB wait for their source-specific promotion gates. |
...
| Claim / journey | Current evidence | Current gap / routing |
|---|---|---|
| Current desktop E2E claim | `e2e/smoke/postgres.spec.ts`, `e2e/smoke/postgres-safe-mode.spec.ts`, `e2e/smoke/mysql.spec.ts`, `e2e/smoke/mariadb.spec.ts`, `e2e/smoke/mongodb.spec.ts`, `e2e/smoke/phase-28-slice-A.spec.ts` | Current E2E smoke proves PostgreSQL, MySQL, MariaDB, and MongoDB journeys only. H6 adds no MSSQL, Oracle, Cassandra/Scylla, DynamoDB, graph, vector, or stream runtime E2E claim. |
