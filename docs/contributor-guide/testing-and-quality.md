...
| MySQL connect -> browse/edit -> SELECT/DML batch -> cancellation + history evidence | `e2e/smoke/mysql.spec.ts`, `e2e/fixtures/seed.mysql.sql`, `src-tauri/tests/mysql_integration.rs`, `src-tauri/tests/cancel_mysql.rs` | H2 MySQL runtime-smoke slice plus catalog metadata integration coverage for databases/schemas, tables, views, columns, indexes, constraints/FKs, and column CHECK hints; version-gated claims, completion context, and support-claim closure remain separate MySQL issues. |
