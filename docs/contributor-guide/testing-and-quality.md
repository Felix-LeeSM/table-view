...
| MySQL connect -> browse/edit -> SELECT/DML batch -> cancellation + history evidence | `e2e/smoke/mysql.spec.ts`, `e2e/fixtures/seed.mysql.sql`, `src-tauri/tests/mysql_integration.rs`, `src-tauri/tests/cancel_mysql.rs` | H2 MySQL runtime-smoke slice plus catalog metadata integration coverage for databases/schemas, tables, views, columns, indexes, constraints/FKs, and live version-gated column CHECK hints; completion context and support-claim closure remain separate MySQL issues. |
...
| MySQL runtime/query/edit/cancel adapter | `e2e/smoke/mysql.spec.ts`, `src-tauri/tests/mysql_integration.rs`, `src-tauri/tests/cancel_mysql.rs`, `src/lib/sql/mysqlScriptingBoundary.test.ts`, `e2e/fixtures/seed.mysql.sql` | Routine MySQL smoke now covers connect, table browse, SELECT result grid, DML batch per-statement result, row edit, cancellation, retry, and history/source labels. CHECK/constraint catalog support is gated by detected server version context. |
| MySQL smoke scenario inventory | `docs/product/query-language-support.md`, `docs/product/known-limitations.md`, this matrix | Covered baseline: connect, browse seeded table, SELECT, DML batch, row edit, cancellation, source labels, and tabular result evidence. Remaining promotion slices: broader tables/views/functions/procedures browse, unsupported `DELIMITER`/`LOAD DATA`, and bounded DDL smoke. |
