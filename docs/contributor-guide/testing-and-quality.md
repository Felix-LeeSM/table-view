...
|---|---|---|
| PostgreSQL connect -> browse/edit -> query result + Explain + extension completion + Safe Mode confirmation | `e2e/smoke/postgres.spec.ts`, `e2e/smoke/postgres-explain.spec.ts`, `e2e/smoke/postgres-extension-completion.spec.ts`, `e2e/smoke/postgres-safe-mode.spec.ts`, `src-tauri/tests/schema_integration.rs`, `src-tauri/tests/query_integration.rs` | H2 one-DBMS parity hardening. |
...
| PostgreSQL connect -> browse/edit -> query -> Explain -> extension completion -> Safe Mode | `e2e/smoke/postgres.spec.ts`, `e2e/smoke/postgres-explain.spec.ts`, `e2e/smoke/postgres-extension-completion.spec.ts`, `e2e/smoke/postgres-safe-mode.spec.ts`, `src-tauri/tests/schema_integration.rs`, `src-tauri/tests/query_integration.rs` | This is the only RDBMS remote E2E smoke-backed lane today. Explain evidence is plan inspection/source-label only; installed-extension completion smoke covers seeded `pgcrypto` gating only; Safe Mode smoke covers info/warn/destructive, raw DDL, and grid-edit preview paths. Broader parity hardening stays in the PostgreSQL milestone. |
...
| Claim / journey | Current evidence | Current gap / routing |
|---|---|---|
| Routine desktop E2E claim | `scripts/e2e-smoke-ci.sh`, `e2e/smoke/postgres.spec.ts`, `e2e/smoke/postgres-explain.spec.ts`, `e2e/smoke/postgres-extension-completion.spec.ts`, `e2e/smoke/postgres-safe-mode.spec.ts`, `e2e/fixtures/seed.sql` | GitHub Runtime Happy Path proves connect, browse seeded `users`, edit Alice's `name`, run a SQL preview, verify the updated query result, open an Explain plan with an Explain history source label, verify seeded `pgcrypto` installed-extension completion gating, and cover Safe Mode info/warn/destructive, raw DDL preview, and grid-edit confirmation paths on Ubuntu. It does not prove structured DDL flows, broader history-source labeling, cancellation, ERD, admin, arbitrary extension semantics, or profiler/activity scenarios. |
