...
| E2E isolation | App-local state (`connections.json`, prefs, safe-mode flags) is emptied per session by `beforeSession` in `wdio.smoke.conf.ts` (`e2e/support/smoke-data-dir.ts`), so a `specFileRetries` retry no longer inherits the previous attempt's connections (#1836). Remaining: DB-server fixtures are still seeded once per spec-file run, not per retry. A live instance is `e2e/smoke/redis.spec.ts`, whose first scan step waits for `tv:string` — the key the same spec's `Confirm Delete` step later removes — so a retry that inherits the deletion fails at that wait instead of where the run broke. Dropping the key-count literal from the filtered-scan assertion further down that spec (#2545) does not move that first failure; #2568 holds the spec redesign that would. |
...
돌린다. `scripts/review/measure-rounds.test.sh` 의 「인자 판정의 회귀 가드 — 자리 ×
축 (#2356)」 절이 그 형태이고, 고른 이름을 단언 label 에 찍는다. 갈리는 로케일이
러너에 하나도 없으면 통과가 아니라 red 다 — 가드가 무력해진 것을 조용히 넘기지
않기 위해서다.

**자리를 손으로 들지 않는다.** 같은 절이 (플래그, 판정자) 짝을 대상 스크립트의
소스에서 뽑고, 표가 그 집합을 정확히 덮는지를 먼저 단언한 뒤 **짝마다 개행 축과
로케일 축**을 건다. 파이프 축은 짝과 무관한 단일 자리다 — 판정자가 전부
`matches_ere()` 를 지나므로 그 함수 하나에 형태로 건다. 자리를 손으로 들면 축이
자리마다 갈린다: #2330 이 파이프를 뗀 뒤 개행 축이 걸려 있던 자리는 `--since` ·
`--limit` · `--top`, 로케일 축이 걸려 있던 자리는 `--limit`, `--until` 에는 둘 다 안
걸려 있었다 (#2356).

**소스에서 뽑는 것은 그 추출기가 인식하는 표기까지다.** 인식 목록 밖의 표기로
판정자를 배선하면 짝이 아예 안 생기고, 짝이 없으면 축이 통째로 안 걸린 채 스위트가
green 이다. 그래서 인식하는 표기와 못 보는 표기(천장)를 추출기 머리 주석이 실측과
함께 들고, 넓힐 때 같이 고친다. 라운드 1 리뷰가 값 대입 `X="$2"` 와 별칭 flag
`--x | -m)` 로 그 자리를 뚫었고, 별칭은 가정이 아니라 대상 스크립트에 이미 있던
표기였다 (`-h | --help)`).
