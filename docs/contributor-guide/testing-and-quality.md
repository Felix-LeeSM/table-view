...
| MySQL connect -> browse/edit -> SELECT/DML batch -> cancellation + history evidence | `e2e/smoke/mysql.spec.ts`, `e2e/fixtures/seed.mysql.sql`, `src-tauri/tests/mysql_integration.rs`, `src-tauri/tests/cancel_mysql.rs`, `src/lib/sql/mysqlScriptingBoundary.test.ts`, `src/components/datagrid/sqlGenerator.test.ts`, `src/components/datagrid/useDataGridEdit.mixed-batch.test.ts`, `src/components/query/QueryTab/useQueryExecution.test.tsx`, `src/components/structure/IndexesEditor.test.tsx`, `src/components/schema/StructurePanel.triggers.test.tsx`, `src-tauri/src/commands/rdb/query.rs` | H2 MySQL runtime-smoke slice plus catalog metadata integration coverage for databases/schemas, tables, views, columns, indexes, constraints/FKs, live version-gated column CHECK hints, row-edit generated SQL quoting/key projection and preview/commit/discard coverage, bounded table/index/constraint DDL preview/confirmation coverage, read-only Structure trigger metadata, and explicit unsupported scripting boundaries; completion context and broader support-claim closure remain separate MySQL issues. |
