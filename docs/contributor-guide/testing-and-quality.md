...
| MySQL connect -> browse/edit -> SELECT/DML batch -> cancellation + history evidence | `e2e/smoke/mysql.spec.ts`, `e2e/fixtures/seed.mysql.sql`, `src-tauri/tests/mysql_integration.rs`, `src-tauri/tests/cancel_mysql.rs`, `src/lib/sql/mysqlScriptingBoundary.test.ts`, `src/components/query/QueryTab/useQueryExecution.test.tsx`, `src-tauri/src/commands/rdb/query.rs` | H2 MySQL runtime-smoke slice plus catalog metadata integration coverage for databases/schemas, tables, views, columns, indexes, constraints/FKs, live version-gated column CHECK hints, and explicit unsupported scripting boundaries; completion context and broader support-claim closure remain separate MySQL issues. |
...
| MySQL runtime/query/edit/cancel adapter | `e2e/smoke/mysql.spec.ts`, `src-tauri/tests/mysql_integration.rs`, `src-tauri/tests/cancel_mysql.rs`, `src/lib/sql/mysqlScriptingBoundary.test.ts`, `src/components/query/QueryTab/useQueryExecution.test.tsx`, `src-tauri/src/commands/rdb/query.rs`, `e2e/fixtures/seed.mysql.sql` | Routine MySQL smoke now covers connect, table browse, SELECT result grid, DML batch per-statement result, row edit, cancellation, retry, and history/source labels. CHECK/constraint catalog support is gated by detected server version context. Unsupported scripting boundaries reject stored routine/event bodies, control-flow fragments, `DELIMITER`, and `LOAD DATA` before dispatch. |
