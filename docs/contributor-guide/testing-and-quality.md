...
| PostgreSQL connect -> browse/edit -> query -> history | `e2e/smoke/postgres.spec.ts`, `e2e/smoke/postgres-explain.spec.ts`, `src-tauri/tests/schema_integration.rs`, `src-tauri/tests/query_integration.rs` | This is the only RDBMS remote E2E smoke-backed lane today. Explain evidence is plan inspection/source-label only; broader parity hardening stays in the PostgreSQL milestone. |
...
| Claim / journey | Current evidence | Current gap / routing |
|---|---|---|
| Routine desktop E2E claim | `scripts/e2e-smoke-ci.sh`, `e2e/smoke/postgres.spec.ts`, `e2e/smoke/postgres-explain.spec.ts`, `e2e/fixtures/seed.sql` | GitHub Runtime Happy Path proves connect, browse seeded `users`, edit Alice's `name`, run a SQL preview, verify the updated query result, and open an Explain plan with an Explain history source label on Ubuntu. It does not prove extension completion, Safe Mode dialogs, DDL structure flows, broader history-source labeling, cancellation, ERD, admin, or profiler/activity scenarios. |
