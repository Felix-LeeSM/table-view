...
| `tvw` CLI live engines | `src-tauri/tvw/tests/query_url_live.rs` proves ADR 0061's four SQL engines answer `tvw query --url … "SELECT 1"`, and **no workflow calls it**. What blocks wiring it into `Integration Tests (Docker)`: that job runs `cargo llvm-cov nextest`, and `cargo llvm-cov` sets its own instrumentation `RUSTFLAGS` — the tool's, not a line in `ci.yml`, which sets none (`git grep -n RUSTFLAGS -- '.github'`) — so a plain `cargo test` step beside it builds under different flags and cannot reuse those artifacts. How much that second build costs is **not measured anywhere in this repository**; the graph carries DuckDB as `bundled` C++ source, which is why nobody has tried. Adding `-p tvw` to the coverage command instead pulls the crate into the `--fail-under-lines/-functions/-regions` denominator, which the job's own comment warns against for `--workspace` by the same mechanism — package selection decides what the report grades. `scripts/check-ci-test-calls.sh` does not catch the omission either — it enumerates `src-tauri/tests/` only, so a target under a member crate's `tests/` is invisible to it. Until then the file is a hand-run test: `pnpm db:up` at the repo root, then `cd src-tauri && PGHOST=127.0.0.1 MYSQL_HOST=127.0.0.1 MARIADB_HOST=127.0.0.1 cargo test -p tvw --test query_url_live` — the manifest is under `src-tauri/`, so a bare `cargo` at the root finds none. The SQLite arm needs no server and the CLI's other behaviour (argument parsing, exit codes, the three output formats) lives in `--lib` tests, which CI does run. |
