...
| Backend core crate                | `src-tauri/table-view-core/`                                         | Forbidden                                                                           | Allowed only inside a dedicated Refactor 03 PR                                  | Path dependency of `src-tauri/Cargo.toml`, not a workspace member: a move must update that manifest's `path`, the crate's own `Cargo.lock`, and every CI step and `lefthook.yml` hook that names the crate (`git grep -n 'table-view-core' -- .github/workflows lefthook.yml`). Keep both compatibility seams — `src-tauri/src/lib.rs`'s `pub use table_view_core::{db, error, models}` and the `src-tauri/src/storage/mod.rs` shim's `pub use table_view_core::storage::*` — so existing `crate::db` / `crate::error` / `crate::models` / `crate::storage` paths still resolve. |
