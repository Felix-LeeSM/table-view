...
| Backend core crate                | `src-tauri/table-view-core/`                                         | Forbidden                                                                           | Allowed only inside a dedicated Refactor 03 PR                                  | Path dependency of `src-tauri/Cargo.toml`, not a workspace member: a move must update both `path = "table-view-core"` entries in that manifest (`[dev-dependencies]` and `[dependencies]`) and every file that hardcodes the directory — `.github/workflows/ci.yml` (the rust-cache `workspaces` lists, the `--manifest-path` test step, the two `working-directory` fmt/clippy steps), `lefthook.yml`, `package.json`'s `format:rust`, `.gitignore`, `.dockerignore`, the `sourceGates` path in `tests/fixtures/unsupported_boundary_contracts.json` (read with `readFileSync` and substring-asserted by its sibling `.test.ts`, so a stale path throws before the assertion), and both the path regexes and the `--self-test` fixtures in `e2e/scope-map.mjs`. The regexes are fail-closed — leaving them stale runs every mapped spec instead of the scoped subset. Neither `Cargo.lock` is a target: lockfiles record no path for this crate, so the crate's own lock travels with the directory unchanged. Enumerate the remainder, prose mentions included, with `git grep -lE 'table[-_]view[-_]core' -- ':!src-tauri/table-view-core'` — match the bare crate name, because a literal `src-tauri/` prefix misses the manifest, the two compatibility seams, the `e2e/scope-map.mjs` regexes that escape their slashes, and every prose mention that names the crate without a path. Keep both compatibility seams — `src-tauri/src/lib.rs`'s `pub use table_view_core::{db, error, models}` and the `src-tauri/src/storage/mod.rs` shim's `pub use table_view_core::storage::*` — so existing `crate::db` / `crate::error` / `crate::models` / `crate::storage` paths still resolve. |
