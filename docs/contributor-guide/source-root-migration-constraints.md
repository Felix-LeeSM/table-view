...
| Rust integration tests            | `src-tauri/tests/` (app package), `src-tauri/<member>/tests/` (workspace member) | Forbidden                                                                           | Allowed only when the moved backend module contract is exercised in the same PR | Tests move with the contract they prove; do not delete coverage as part of root cleanup.                                                 |
| Backend core crate                | `src-tauri/table-view-core/`                                         | Forbidden                                                                           | Allowed only inside a dedicated Refactor 03 PR                                  | A move must update every file that hardcodes the directory. Enumerate them, prose mentions included, with `git grep -lE 'table[-_]view[-_]core' -- ':!src-tauri/table-view-core'`. |
