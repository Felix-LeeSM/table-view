...
- Tracked source roots include `src/`, `public/`, `src-tauri/src/`,
  `src-tauri/tests/`, parser crates, CI workflows, docs, and memory.
...
- Primary checkout is orchestration-only. Source, docs, app config, manifests,
  and CI files are changed in linked worktrees, while primary may create linked
  worktree targets and update allowed agent contracts.
...
| `worktrees/`, `.claude/worktrees/` | local linked worktree state | Active agents and user sessions | Created by `git worktree add`; may contain dirty user or agent work |
...
| `e2e/` | mixed tracked and gitignored | Frontend route locally; no CI job runs the specs — `.github/workflows/e2e-smoke.yml` only reports a required context | Delete report contents only, keep `.gitkeep` | Report output is not source |
...
| `wdio-report/`, `e2e/wdio-report/` | report contents gitignored; `.gitkeep` tracked | Written only by a local `pnpm test:e2e:smoke` run; nothing uploads them | Delete report contents only | Host-only report output |
