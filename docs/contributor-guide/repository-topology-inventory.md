...
| `src-tauri/icons-src/`, `src-tauri/icons/` | source plus generated committed assets | Release and packaging owners | Source SVGs feed `scripts/build-icons.sh`; generated icon outputs are committed except ignored `partial-info.plist` |
| `scripts/` | tooling source | Developer tooling owners | Hand-authored scripts and CLIs |
| `scripts/hooks/`, `.githooks/`, `lefthook.yml` | hook source | Workflow and git-policy owners | Hand-authored hook/router source |
| `.github/` | CI workflow source | CI and release owners | Hand-authored GitHub Actions workflows |
| `scripts/fixtures/`, `fixtures/`, `tests/fixtures/`, `e2e/fixtures/` | fixtures | Fixture and test owners | Generator code plus static DBMS/test seed inputs |
...
| `memory/` | agent rule memory | Agent workflow and engineering rule owners | Active rule files named `memory.md`; generated indexes under `memory/index/` |
| `.agents/`, `.claude/`, `.codex/` | agent source and wrapper config | Agent workflow owners | Tracked skills, agents, hook wrappers, and local agent config source |
...
| `worktrees/`, `.claude/worktrees/` | local linked worktree state | Active agents and user sessions | Created by worktree helpers; may contain dirty user or agent work |
...
| --- | --- | --- | --- | --- |
| `src/` | tracked | Pre-push frontend route runs TypeScript, lint, Vitest, and coverage; CI builds and tests it | No automatic cleanup | Source root; not generated or cache |
| `public/` | tracked | Pre-push frontend route; Vite build input | No automatic cleanup | Product asset root |
| `src-tauri/src/` | tracked | Pre-push Rust route; CI cargo checks, tests, and coverage | No automatic cleanup | Backend source root |
| `src-tauri/tests/` | tracked | Pre-push Rust route; CI cargo test targets | No automatic cleanup | Test source, not fixture cache |
| `src-tauri/sql-parser-core/`, `src-tauri/mongosh-parser-core/` | tracked | Pre-push Rust route; wasm build scripts consume these crates | No automatic cleanup | Source crates that generate committed wasm artifacts |
...
| `src-tauri/gen/` | tracked | Pre-push Rust route because it is under `src-tauri/`; CI packaging can consume schemas | Regenerate from Tauri tooling, do not ad hoc edit when stale | Generated but committed reference output |
| `src-tauri/icons-src/`, `src-tauri/icons/` | mostly tracked; `src-tauri/icons/Compiled/partial-info.plist` gitignored | Pre-push Rust route by path; release builds consume committed icons | Re-run `pnpm icons:build`; do not clean icon tree wholesale | Generated output is product packaging input, not disposable cache |
| `scripts/` | tracked | `.ts` scripts trigger frontend route; hook scripts trigger hook route; unknown script paths route conservatively | No automatic cleanup | Tooling source, not scratch |
| `scripts/hooks/`, `.githooks/`, `lefthook.yml` | tracked | Hook route runs shell syntax, lefthook validation, router tests, target-cache tests, and related checks | No automatic cleanup | Path-router ownership lives here; the hook-routing child owns routing changes |
...
| `docs/` | tracked | Docs-only pre-push skips TS/Rust gates after signed-commit and coverage-ratchet checks | No automatic cleanup; archive retired records instead | The final-docs child owns durable topology publication |
...
| `.agents/`, `.claude/`, `.codex/` | tracked except local-only settings | Agent route skips TS/Rust unless mixed with other paths; app source edits from primary are blocked by edit policy | No automatic cleanup; `.claude/worktrees/` is separate local state | Skill source is `.agents/skills/<name>/SKILL.md`; wrappers point to memory |
