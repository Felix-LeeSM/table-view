...
| `src-tauri/table-view-core/` | source crate | Rust backend owners | Hand-authored headless backend core: DB adapters, wire models, local storage, error types. A member of the Cargo workspace rooted at `src-tauri/Cargo.toml`, which owns the only `Cargo.lock` |
...
| `src-tauri/tvw/` | source crate | CLI owners | Hand-authored `tvw` CLI (ADR 0061). Depends on `table-view-core` and nothing from Tauri. `src-tauri/tvw/Cargo.toml` declares a `[[bin]]`; the logic sits in the lib so `cargo test --workspace --lib` reaches it. Its integration targets live beside that manifest in `src-tauri/tvw/tests/`, which `--lib` does not reach |
...
| `src-tauri/tvw/` | tracked | `cargo fmt --all` / `cargo clippy --workspace` / `cargo test --workspace --lib` reach it through the workspace member list; `src-tauri/tvw/tests/` is reached by the `--test` names the `CLI Live Query (Docker)` job spells out, not by `--lib`, and since #2336 `scripts/check-ci-test-calls.sh` grades that directory too — dropping one of those `--test` names without an entry in `ci-uncalled-tests.txt` is red | No automatic cleanup | CLI source crate; not packaged or released yet (#1775) |
