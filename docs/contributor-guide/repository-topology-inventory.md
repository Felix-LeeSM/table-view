...
| `src-tauri/tvw/` | source crate | CLI owners | Hand-authored `tvw` CLI (ADR 0061). Depends on `table-view-core` and nothing from Tauri. `src-tauri/tvw/Cargo.toml` declares a `[[bin]]`; the logic sits in the lib so `cargo test --workspace --lib` reaches it |
