...
| `worktrees/`, `.claude/worktrees/` | gitignored; excluded from Vite, Vitest, ESLint, and dev-server fs access | Not CI input; primary worktree edit guard allows linked target paths but blocks primary source edits | Use ; dirty worktrees are preserved and reported | Local state, not source, cache, fixture, or planning surface |
