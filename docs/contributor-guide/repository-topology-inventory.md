...
| `src-tauri/table-view-core/` | tracked | Compiled by every app-level step that builds the app — `cargo test`, `cargo clippy`, the llvm-cov gate, and the `tauri build` jobs in other workflows among them — because it is a path dependency; `cargo fmt` builds nothing and never reaches it. Its source is format-, lint-, and test-checked by exactly three `.github/workflows/ci.yml` steps: `Run table-view-core unit tests` through `--manifest-path`, and `Rustfmt (table-view-core)` / `Clippy (table-view-core)` through `working-directory` — `cargo fmt` / `cargo clippy` / `cargo test` act on the package they are invoked on, not on its path dependencies. The rust-cache `workspaces` lists in the `rust` and `rust-static` jobs name the crate and check nothing, while the `cargo deny` steps check it without naming it, because they walk `src-tauri/Cargo.lock` where the crate is a node. The `Integration Tests (Docker)` llvm-cov gate does not report it | No automatic cleanup | Backend source crate; the coverage boundary is measured in [`testing-and-quality.md`](testing-and-quality.md) |
...
| `memory/` | tracked | Nothing checks memory structure or size, and the indexes are hand-maintained | No cleanup; split memory files through memory workflow only | Not a docs backlog; active agent rules |
