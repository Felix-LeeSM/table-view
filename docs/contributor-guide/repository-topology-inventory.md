...
| `src-tauri/tvw/` | tracked | `cargo fmt --all` / `cargo clippy --workspace` / `cargo test --workspace --lib` reach it through the workspace member list | No automatic cleanup | CLI source crate; not packaged or released yet (#1775) |
