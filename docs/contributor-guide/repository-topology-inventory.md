...
| `src-tauri/table-view-core/` | tracked | Compiled by every app-level step that builds the app — `cargo test`, `cargo clippy`, the llvm-cov gate, and the `tauri build` jobs among them — because it is a path dependency; `cargo fmt` builds nothing and never reaches it. Checked only by the `.github/workflows/ci.yml` steps that name the crate: `Run table-view-core unit tests` through `--manifest-path`, and `Rustfmt (table-view-core)` / `Clippy (table-view-core)` through `working-directory`. A per-package check never descends into a path dependency, and the `Integration Tests (Docker)` llvm-cov gate does not report it | No automatic cleanup | Backend source crate; the coverage boundary is measured in [`testing-and-quality.md`](testing-and-quality.md) |
