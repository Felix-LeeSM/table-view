...
| `src-tauri/table-view-core/` | tracked | Compiled by every app-level cargo step as a path dependency, but checked only by the steps in `.github/workflows/ci.yml` that name its manifest — its `--lib` tests, `cargo fmt --check`, and Clippy — because a per-package check never descends into a path dependency. The `Integration Tests (Docker)` llvm-cov gate does not report it | No automatic cleanup | Backend source crate; the coverage boundary is measured in [`testing-and-quality.md`](testing-and-quality.md) |
