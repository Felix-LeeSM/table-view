...
| `src-tauri/tvw/` | source crate | CLI owners | Hand-authored `tvw` CLI (ADR 0061). Depends on `table-view-core` and nothing from Tauri. The only workspace member that produces a `[[bin]]`; its logic sits in the lib so `cargo test --workspace --lib` reaches it |
...
| `src-tauri/tvw/` | tracked | `cargo fmt --all` / `cargo clippy --workspace` / `cargo test --workspace --lib` reach it through the workspace member list. Its `tests/query_url_live.rs` is not called by any workflow — see [`testing-and-quality.md`](testing-and-quality.md) | No automatic cleanup | CLI source crate; not packaged or released yet (#1775) |
