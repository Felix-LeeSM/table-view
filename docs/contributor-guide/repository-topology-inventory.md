...
| `src-tauri/tvw/` | tracked | `cargo fmt --all` / `cargo clippy --workspace` / `cargo test --workspace --lib` reach it through the workspace member list; `src-tauri/tvw/tests/` is reached by the `--test` names the `CLI Live Query (Docker)` job spells out, not by `--lib`, and `scripts/check-ci-test-calls.sh` does not grade that directory (#2336) | No automatic cleanup | CLI source crate; not packaged or released yet (#1775) |
