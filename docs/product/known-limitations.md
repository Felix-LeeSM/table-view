...
| ERD / SchemaGraph | schemaStore owns cached schemas/tables/views/functions/postgresExtensions/tableColumnsCache/triggers. StructurePanel tabs and migration export fetch indexes/constraints live through delegate actions, not cache entries. Production ERD/SchemaGraph input currently comes from schema/table/column cache; the graph derives PK/FK/CHECK from `ColumnInfo` synthetic constraints and passes empty `indexesByTable`/`constraintsByTable` for explicit graph nodes. Explicit constraint/index graph inputs are fixture/test-covered only. Dependency view, migration-impact analysis, and explicit constraint/index production wiring remain roadmap follow-ups (#189, #200). #211 is documentation-only and does not change runtime behavior. |
