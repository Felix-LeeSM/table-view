...
bounded static SQL editor autocomplete vocabulary for selected keywords,
functions, package symbols, sequences, synonyms, and bind placeholders. Oracle
also has a bounded static parser/Safe Mode boundary for common SELECT/DML/DDL
safety classification, with PL/SQL package/admin paths unsupported. Those
slices do not create Oracle query/catalog/edit, runtime fixture/live evidence,
...
| MSSQL SQL | SQL Server is an RDBMS identity with bounded connection/query/catalog metadata runtime support, bounded SQL editor completion, and bounded static parser/Safe Mode metadata. The active runtime slice is SQL authentication connection testing with TCP/TDS login, `SERVERPROPERTY('ProductVersion')` probe evidence, bounded SELECT tabular result envelopes, DML/DDL execution, transactional DML batch commit/dry-run, cancellation, server error surfacing, and catalog/workbench metadata for databases, schemas, tables, views, procedures/functions, columns, indexes, constraints, and FKs. Completion covers a curated keyword/function slice plus bracket identifier quoting for editor suggestions only. Static parser/Safe Mode recognizes bounded T-SQL slices: bracket identifiers, `N'...'` strings, `SELECT TOP`, common DML, bounded CREATE/ALTER/DROP/TRUNCATE DDL, and SQL Server type synonyms that map to existing AST types. Unsupported scripting/admin boundaries such as `EXEC`, `EXECUTE`, `GO`, `USE`, `DBCC`, `DENY`, `BACKUP`, and `RESTORE` classify as warn/danger safety metadata instead of active execution support. | MSSQL has Lifecycle, RelationalCatalog, and RelationalQuery runtime capability. Row edit, runtime Safe Mode/destructive preview evidence, runtime fixture smoke, desktop E2E smoke, TLS-required workflow, broader SQL Server instance/auth behavior, `TOP PERCENT`, `WITH TIES`, procedure bodies, batches, SQL Server admin tooling, and full T-SQL semantic completion remain unsupported. Completion suggestions and static parser/Safe Mode metadata do not widen runtime SQL Server support. |
| Oracle SQL | Oracle has service-name connection UI/runtime support, bounded static Oracle SQL editor autocomplete vocabulary for selected keywords, functions, package symbols, sequences, synonyms, and bind placeholders, and bounded static parser/Safe Mode evidence for SELECT including `DUAL`, INSERT, bounded UPDATE/DELETE, WHERE-less UPDATE/DELETE destructive detection, narrow MERGE, CREATE TABLE/INDEX/VIEW, Oracle `NUMBER`/`VARCHAR2`/`CLOB`/`BLOB` column types, ALTER ADD/RENAME, and DROP/TRUNCATE/ALTER DROP destructive detection. Static SQL seed contract still declares future SQL promotion evidence. | Oracle has Lifecycle-only runtime capability. Runtime query/catalog/edit, SID/TNS alias/wallet/TLS behavior, runtime fixture/live evidence, desktop E2E smoke, and full PL/SQL executable semantics remain unsupported. PL/SQL blocks, package/routine DDL, routine execution, and admin/privilege paths are outside the static safety boundary. Completion suggestions and parser/Safe Mode metadata do not widen runtime Oracle support. |
...
- MSSQL runtime is limited to SQL authentication connection test, SQL Server
  ...
  procedures/columns/indexes/constraints/FKs. It also has bounded T-SQL editor
  completion vocabulary and bounded static parser/Safe Mode metadata. These do
  not imply row edit, runtime Safe Mode/destructive preview, runtime fixture
  smoke, E2E smoke, TLS-required workflow, or full T-SQL semantic completion
  support. Oracle is connection-only for runtime
  through service-name connection test plus bounded static SQL editor
  autocomplete vocabulary and bounded static parser/Safe Mode metadata; this
  does not imply Oracle query, catalog, edit, runtime fixture/live, desktop E2E
  smoke, or full Oracle SQL/PL/SQL executable semantics.
