...
| Redis command | Redis connection/profile, backend KV primitives, key browser, value preview, static KV/stream fixture inventory, and a bounded command query editor runtime are active. Backend primitives are typed IPC calls for database/key scan, typed value reads, guarded string set, delete confirmation, TTL expire/persist, and bounded stream reads. The command editor dispatches selected read, write, TTL, and stream commands through a backend allowlist and renders string/hash/list/set/zset/stream results through the tabular grid projection. The backend parser classifies read/write/TTL/stream/destructive effects and only allows single-key destructive `DEL`/TTL-removal `PERSIST` when the request carries an exact `confirmKey`. | Redis command parser/completion is not owned by language-core yet, and the current backend parser is an allowlist, not arbitrary Redis CLI support. Unsupported command families reject with explicit messages. Full value editing, TTL/write controls outside the command runtime, stream consumer UI, broader command coverage, cluster, pub/sub, modules, consumer-group management, multi-key destructive commands, and Valkey support are not claimed. |
...
- Redis command execution is limited to the backend allowlist. Selected read,
  ...
  grid projection; unsupported command families fail clearly. This is not full
  Redis CLI parity, language-core parser/completion ownership, cluster/pubsub/
  modules/consumer-group management, or Valkey support.
