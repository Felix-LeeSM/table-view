...
primary-key-scoped row edit/write workflow support. Catalog-aware T-SQL completion
...
instance/auth behavior, SQLCMD/admin support, structured DDL/admin support, or
...
vocabulary for selected keywords, functions, package symbols, sequences,
synonyms, and bind placeholders. Oracle also has a bounded static parser/Safe
Mode boundary for common SELECT/DML/DDL safety classification, with PL/SQL
package/admin paths unsupported. Those slices do not create Oracle
catalog/edit/structured DDL, runtime fixture/live evidence,
desktop E2E smoke, or full Oracle SQL/PL/SQL executable-semantics claims.
...
| MSSQL SQL | SQL Server is an RDBMS identity with bounded connection/query/catalog/edit metadata runtime support, catalog-aware SQL editor completion, and bounded static parser/Safe Mode metadata. The active runtime slice is SQL authentication connection testing with TCP/TDS login, `SERVERPROPERTY('ProductVersion')` probe evidence, bounded SELECT tabular result envelopes, table data reads, DML/DDL execution, transactional DML batch commit/dry-run, cancellation, server error surfacing, primary-key-projected row edits, and catalog/workbench metadata for databases, schemas, tables, views, procedures/functions, columns, indexes, constraints, and FKs. Row edit generated SQL uses bracket identifiers and PK-only WHERE projection; no-PK tables can be browsed but write controls stay disabled. Completion uses the live cached catalog for database, schema, table/view, procedure/function, and column suggestions where metadata is loaded, and keeps curated keyword/function plus bracket identifier quoting support for editor assistance only. Static parser/Safe Mode recognizes bounded T-SQL slices: bracket identifiers, `N'...'` strings, `SELECT TOP`, common DML, bounded CREATE/ALTER/DROP/TRUNCATE DDL, and SQL Server type synonyms that map to existing AST types. Unsupported scripting/admin boundaries such as `EXEC`, `EXECUTE`, `GO`, `USE`, `DBCC`, `DENY`, `BACKUP`, and `RESTORE` classify as warn/danger safety metadata instead of active execution support. | MSSQL has Lifecycle, RelationalCatalog, and RelationalQuery backend capability plus primary-key-scoped row-edit profile/runtime support. Runtime Safe Mode/destructive preview evidence, runtime fixture smoke, desktop E2E smoke, TLS-required workflow, broader SQL Server instance/auth behavior, `TOP PERCENT`, `WITH TIES`, procedure bodies, batches, SQL Server admin tooling, structured DDL/admin support, and full T-SQL semantic completion remain unsupported. Completion suggestions and static parser/Safe Mode metadata do not widen runtime SQL Server support. |
| Oracle SQL | Oracle has service-name connection UI/runtime support, bounded SELECT tabular result envelopes, transactional DML batch commit/dry-run, cancellation/error surfacing, bounded static Oracle SQL editor autocomplete vocabulary for selected keywords, functions, package symbols, sequences, synonyms, and bind placeholders, and bounded static parser/Safe Mode evidence for SELECT including `DUAL`, INSERT, bounded UPDATE/DELETE, WHERE-less UPDATE/DELETE destructive detection, narrow MERGE, CREATE TABLE/INDEX/VIEW, Oracle `NUMBER`/`VARCHAR2`/`CLOB`/`BLOB` column types, ALTER ADD/RENAME, and DROP/TRUNCATE/ALTER DROP destructive detection. Static SQL seed contract still declares future SQL promotion evidence. | Oracle has Lifecycle and RelationalQuery runtime capability only. Catalog/edit/structured DDL, SID/TNS alias/wallet/TLS behavior, runtime fixture/live evidence, desktop E2E smoke, and full PL/SQL executable semantics remain unsupported. PL/SQL blocks, package/routine DDL, routine execution, and admin/privilege paths are outside the static safety boundary. Completion suggestions and parser/Safe Mode metadata do not widen runtime Oracle support. |
...
- MSSQL runtime is limited to SQL authentication connection test, SQL Server
  ...
  table data browse, primary-key-projected row edit, and catalog/workbench metadata
  ...
  workflow, SQLCMD/admin/full T-SQL semantics, or runtime support for every
  suggested object. Oracle has a bounded runtime slice for
  ...
  Oracle catalog/edit/structured DDL, runtime fixture/live, desktop E2E smoke,
  or full Oracle SQL/PL/SQL executable semantics.
