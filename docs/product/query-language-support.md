...
delete-by-query safety planning.
...
- Search DSL has bounded live Elasticsearch `_search` dispatch, backend request
  validation for the supported query/filter/aggs subset, response parsing, and
  fixture/live delete-by-query safety plans with estimate + confirmation
  gates. Actual live `_delete_by_query` execution, editor parser/completion
  ownership, broader admin, observability, OpenSearch live behavior, and
  product-delta gates remain deferred.
