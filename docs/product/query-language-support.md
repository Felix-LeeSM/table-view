...
OpenSearch now has focused live connection/catalog tests for URL/auth/TLS root
probe, product/version/distribution detection, indexes, aliases, data streams,
mappings, settings/analyzers, composable/legacy templates, and field paths, but
no live query claim.
...
- Fixture / live evidence: `seed.search.elasticsearch.json` and
  ...
  OpenSearch has focused live connection/catalog evidence for indexes, aliases,
  data streams, mappings, settings/analyzers, templates, and field paths; query
  remains fixture-only until a separate live runtime/smoke path lands.
- Remaining unsupported work: actual live `_delete_by_query` execution,
  OpenSearch live query, profile/explain request workflow, broader admin
...
- Search DSL has bounded live Elasticsearch `_search` dispatch, backend request
  ...
  query behavior, and product-delta gates remain deferred.
