...
Search DSL validator for supported query/filter/aggs request clauses.
...
| Search DSL | Fixture-backed Search identities and bounded fixture DSL exist for Elasticsearch/OpenSearch fixture result paths. Elasticsearch connection/auth/TLS root probe is active, detects product/version/distribution, live catalog reads indexes, aliases, data streams, mappings, settings/analyzers, templates, and field paths, and bounded live `_search` dispatch validates `match_all`, `term`, `terms`, `match`, `bool` filter clauses, `range`, `exists`, and `terms`/`value_count` aggregations before HTTP dispatch. The response parser renders hits/source/fields/highlights/sort, shard/timeout metadata, aggregations, and explain/profile payloads returned by the server. | Editor parser/completion ownership, admin APIs, profile/explain request workflow, observability, OpenSearch live connection/catalog/query, and full query-language support are deferred. Unsupported Search DSL body keys, unsupported aggregation kinds, raw/admin targets, and destructive/admin APIs are rejected before live Search dispatch. Search fixture files mirror embedded adapter contracts only. |
...
- Search DSL has bounded live Elasticsearch `_search` dispatch, backend request
  validation for the supported query/filter/aggs subset, and response parsing,
  plus fixture-backed Elasticsearch/OpenSearch renderer/admin contracts. Editor
  parser/completion ownership, admin, observability, OpenSearch live behavior,
