...
| MongoDB Mongosh/MQL | Whitelisted `db...` collection/admin commands, JSON-like bodies, BSON literals, `find(filter, projection)`, cursor-chain `sort`/`skip`/`limit` dispatch, aggregate cursor-chain lowering, operator/stage/expression completion, destructive collection/admin confirmations, and transaction-helper unsupported gates are supported. | Arbitrary JavaScript, shell helpers such as `use`/`show`, multiple statements, unsupported cursor helpers, cross-db shell navigation, server-version feature promotion gates, and native document-first result panels remain out of scope. |
...
- MongoDB support is limited to the tested whitelist. Arbitrary JavaScript,
  ...
  intentionally unsupported. Destructive collection/admin commands require a
  confirmation before the backend safety acknowledgement is sent. Transaction
  helpers (`startSession`, `startTransaction`, `withTransaction`,
