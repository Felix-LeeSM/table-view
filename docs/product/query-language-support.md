...
- Valkey uses `redis-command` for bounded Redis-compatible command query
  ...
  rejected command-family rows; it is not command completion, direct key
  mutation, full Redis compatibility, or smoke support.
...
| Valkey `redis-command` target | Valkey has a KV runtime slice for connection, database/key scan, typed value preview, and bounded Redis-compatible command query dispatch. Focused local Valkey testcontainer evidence covers selected string preview/write, hash read, TTL mutation/confirmation, stream read, destructive confirmation, and unsupported-family rejection through the shared bounded allowlist. Static fixture inventory includes `e2e/fixtures/valkey.redis-compatibility.json`, which separates proven local runtime rows from candidate families and rejected Redis assumptions. | The matrix is not direct key mutation, completion, full Redis compatibility, or smoke evidence. Future support must prove Valkey identity with Valkey-specific server fields for broader compatibility claims and keep admin/server-control, broad destructive, cluster, pub/sub, modules/functions, scripting, and consumer-group commands rejected until separate workflow-specific safety/result-envelope decisions land. |
...
- Compatibility matrix: `e2e/fixtures/valkey.redis-compatibility.json` separates
  ...
  beyond the focused local Valkey testcontainer path, and unsupported Redis
  families cannot widen for Valkey without separate safety and result-envelope
  decisions.
