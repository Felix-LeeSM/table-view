...
Search DSL validator for supported query/filter/aggs/sort/source-filter request
clauses plus delete-by-query safety planning. Elasticsearch also has bounded
TypeScript editor completion for index/alias/data-stream names, mapping fields,
supported query clauses, supported aggregation kinds, and body keys. Full
language-core parser ownership and OpenSearch completion stay deferred.
...
| Search DSL | Fixture-backed Search identities and bounded fixture DSL exist for Elasticsearch/OpenSearch fixture result paths. Elasticsearch connection/auth/TLS root probe is active, detects product/version/distribution, live catalog reads indexes, aliases, data streams, mappings, settings/analyzers, templates, and field paths, bounded live `_search` dispatch validates `match_all`, `term`, `terms`, `match`, `bool` filter clauses, `range`, `exists`, supported `sort` and `_source` filters, and `terms`/`value_count` aggregations before HTTP dispatch, and delete-by-query safety planning estimates matching documents through a safe `_search` request before requiring acknowledged risk plus exact target confirmation for execution intent. Elasticsearch editor completion suggests live catalog targets, mapping fields, supported query clauses, aggregation kinds, and body keys from TypeScript runtime context. The response parser renders hits/source/fields/highlights/sort, shard/timeout metadata, aggregations, and explain/profile payloads returned by the server. | Full editor parser ownership, OpenSearch editor completion, actual live `_delete_by_query` execution, broader admin APIs, profile/explain request workflow, observability, OpenSearch live connection/catalog/query, and full query-language support are deferred. Unsupported Search DSL body keys, unsupported aggregation kinds, raw/admin targets, unsupported delete-by-query body keys, and destructive/admin APIs are rejected before live Search dispatch. Search fixture files mirror embedded adapter contracts only. |
...
- Search DSL has bounded live Elasticsearch `_search` dispatch, backend request
  validation for the supported query/filter/aggs/sort/source-filter subset,
  Elasticsearch-only TypeScript editor completion, response parsing, wired
  representative smoke, and fixture/live delete-by-query safety plans with
  estimate + confirmation gates. Actual live `_delete_by_query` execution, full
  editor parser ownership, OpenSearch completion/live behavior, broader admin,
  observability, and product-delta gates remain deferred.
