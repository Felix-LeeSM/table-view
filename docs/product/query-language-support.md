...
vocabulary/profile support in the SQL editor. The runtime slice does not create
catalog, row edit, parser/Safe Mode, runtime fixture, E2E smoke, or full T-SQL
...
| MSSQL SQL | SQL Server is an RDBMS identity with bounded connection and query runtime support plus bounded SQL editor completion. The active runtime slice is SQL authentication connection testing with TCP/TDS login, `SERVERPROPERTY('ProductVersion')` probe evidence, bounded SELECT tabular result envelopes, DML/DDL execution, transactional DML batch commit/dry-run, cancellation, and server error surfacing. Completion covers a curated keyword/function slice plus bracket identifier quoting for editor suggestions only. | MSSQL has Lifecycle and RelationalQuery runtime capability only. Catalog browse, row edit, T-SQL parser/Safe Mode, runtime fixture smoke, desktop E2E smoke, broader SQL Server instance/auth behavior, and full T-SQL semantic completion remain unsupported. Completion suggestions do not widen runtime SQL Server support. |
...
- MSSQL runtime is limited to SQL authentication connection test, SQL Server
  ...
  surfacing. It also has bounded T-SQL editor completion vocabulary. These do
  not imply active catalog, edit, parser/Safe Mode, runtime fixture smoke, E2E
  smoke, or full T-SQL semantic completion support. Oracle is connection-only
  for runtime through service-name connection test plus bounded static SQL
  editor autocomplete vocabulary; this does not imply Oracle query, catalog,
