...
a connection-only SQL Server slice: source-specific SQL-auth/TDS connection
test/connect/ping with explicit encryption and trust-server-certificate inputs.
That slice does not promote SQL Server query, catalog, edit, DDL, parser,
completion, or runtime smoke support. Oracle remains declared-only: its
source-specific dialect/profile records remain for future promotion, URL
parsing, labels, defaults, and seed/spec inventory, but it is hidden from new
connection support and has no active connection/query/catalog/edit/DDL/parser/
completion/runtime smoke support claim. Future promotion beyond MSSQL
connection-only or any Oracle support must land matching source-specific runtime,
contract, docs, and smoke evidence before adding SQL Server T-SQL or Oracle
SQL/PLSQL parser/completion/runtime claims.
...
target for proven local Valkey runtime rows, while direct mutation controls are
limited to proven UTF-8 string-key paths for Valkey. The Redis command editor
...
| MSSQL SQL | Connection-only identity. `mssql` exposes SQL Server SQL-auth connection test/connect/ping with host, port, database, user, password, encryption, and trust-server-certificate inputs. | No SQL Server query, catalog, edit, DDL, parser, completion, or runtime smoke support is claimed. Named instances, Windows authentication, Azure AD/authSource modes, SQLCMD/batch scripting, admin/security/backup/jobs/users/roles, import/export, profiler/activity, and full T-SQL semantics remain out of scope. |
...
- Redis has backend KV primitives, key browser/value preview/edit UI, bounded
  ...
  completion. Direct UTF-8 string-key mutation controls have focused
  backend/component evidence; hash/list/set/zset writes and full Redis
...
- MSSQL is a connection-only SQL identity. Its labels, defaults, URL parsing,
  dialect/profile ids, SQL-auth/TDS connection test/connect/ping path, and
  seed/spec inventory do not create active parser, completion, query, catalog,
  edit, DDL, or runtime smoke support. Oracle remains declared-only and hidden
  from new connection support. Promotion requires source-specific runtime,
  contract, docs, and smoke evidence before any SQL Server or Oracle support
  claim widens beyond those boundaries.
