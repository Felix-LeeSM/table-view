...
| Valkey `redis-command` target | Valkey has a KV runtime slice for connection, database/key scan, typed value preview, bounded Redis-compatible command query dispatch, and TypeScript command completion for proven local-runtime rows (`GET`, `HGETALL`, `XRANGE`, `TYPE`, `EXISTS`, `SET`, `EXPIRE`, `PERSIST`, `DEL`). Focused local Valkey testcontainer evidence covers selected string preview/write, hash read, TTL mutation/confirmation, stream read, destructive confirmation, and unsupported-family rejection through the shared bounded allowlist. Completion key suggestions use the current DB scan cache and stay hidden for unpromoted command families. Static fixture inventory includes `e2e/fixtures/valkey.redis-compatibility.json`, which separates proven local runtime rows from candidate families and rejected Redis assumptions. | The matrix is not direct key mutation, full Redis compatibility, or smoke evidence. Future support must prove Valkey identity with Valkey-specific server fields for broader compatibility claims and keep admin/server-control, broad destructive, cluster, pub/sub, modules/functions, scripting, and consumer-group commands rejected until separate workflow-specific safety/result-envelope decisions land. |
...
- Current status: Valkey has a KV runtime slice for connection test/connect,
  ...
  mutation controls, Runtime Happy Path smoke, and full Redis compatibility are
  not claimed.
...
- Redis has backend KV primitives, key browser/value preview/edit UI, bounded
  ...
  dispatch plus proven-row command completion. Direct key mutation controls,
  Runtime Happy Path smoke, and full Redis compatibility are not claimed.
