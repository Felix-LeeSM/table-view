...
| MongoDB Mongosh/MQL | Whitelisted `db...` collection/admin commands, JSON-like bodies, BSON literals, `find(filter, projection)`, cursor-chain `sort`/`skip`/`limit` dispatch, aggregate cursor-chain lowering, operator/stage/expression completion, cached collection and field-name suggestions, active-collection index-name suggestions for `dropIndex`, destructive collection/admin confirmations, and transaction-helper unsupported gates are supported. | Completion suggestions are editor assistance and stay aligned to the runtime whitelist. Arbitrary JavaScript, shell helpers such as `use`/`show`, multiple statements, unsupported cursor helpers, cross-db shell navigation, server-version feature promotion gates, and native document-first result panels remain out of scope. |
...
- MongoDB support is limited to the tested whitelist. Arbitrary JavaScript,
  ...
  runtime support. Destructive collection/admin commands require a confirmation
  before the backend safety acknowledgement is sent. Transaction helpers
