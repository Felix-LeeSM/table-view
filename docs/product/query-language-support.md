...
bounded T-SQL completion vocabulary/profile support in the SQL editor, but its
runtime capabilities remain empty. That completion slice does not create SQL
Server connection UI, runtime query, catalog, edit, parser/Safe Mode, or E2E
smoke claims. Oracle carries planned `sql` profile metadata only; that metadata
does not create Oracle SQL/PL/SQL runtime query, catalog, edit, parser,
completion, or E2E smoke claims.
...
| MSSQL SQL | Planned profile metadata, a static SQL seed contract, and bounded T-SQL SQL editor completion vocabulary declare SQL Server as a future RDBMS identity with `sql`, `rdb`, `tabular`, and `rdb-default` contract shape. Completion covers a curated keyword/function slice plus bracket identifier quoting for editor suggestions only. | Capabilities are empty. There is no SQL Server connection UI, runtime query/catalog/edit path, parser/Safe Mode claim, auth/TLS/encryption/instance contract, runtime fixture/live evidence, or desktop E2E smoke. Completion suggestions do not imply SQL Server runtime support or full T-SQL semantic coverage. |
...
- MSSQL is a planned SQL/RDBMS identity with bounded T-SQL editor completion
  vocabulary only. Declared profile metadata, static seed contracts, and
  completion suggestions do not imply SQL Server connection, query, catalog,
  edit, parser/Safe Mode, runtime fixture/live, or E2E smoke support. Oracle
  remains a planned SQL/RDBMS identity only; its declared metadata and static
  seed contract do not imply Oracle SQL/PL/SQL connection, query, catalog, edit,
  parser/completion, runtime fixture/live, or E2E smoke support.
