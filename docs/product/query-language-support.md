...
- MSSQL is connection-only for runtime: SQL authentication connection test plus
  ...
  completion support. Oracle is connection-only for runtime through service-name
  connection test plus bounded static SQL editor autocomplete vocabulary; this
  does not imply Oracle query, catalog, edit, parser/Safe Mode,
