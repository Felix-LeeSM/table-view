...
Oracle now have full-support RDBMS slices for connection UI, query execution,
schema/catalog metadata, row edit, bounded DDL/admin, SQL vocabulary/catalog
completion, fixture seed, and E2E smoke. That support does not create deep
T-SQL or Oracle SQL/PL/SQL scripting/admin parity.
...
| MSSQL SQL | SQL Server is active for connection UI/runtime, SQL authentication, schema/catalog metadata, SELECT/DML execution, row edit, bounded DDL/admin, SQL vocabulary/catalog completion, fixture seed, and desktop E2E smoke with `sql`, `rdb`, `tabular`, and `rdb-default` profile metadata. | Named instances, Windows auth, server/session admin, native cancel, deep T-SQL scripting/procedure bodies, broad import/export/admin parity, and Postgres-only extension/type tools are not claimed. |
| Oracle SQL | Oracle is active for service-name connection UI/runtime, schema/catalog metadata, SELECT/DML execution, row edit, bounded DDL/admin, SQL vocabulary/catalog completion, fixture seed, and desktop E2E smoke with `sql`, `rdb`, `tabular`, and `rdb-default` profile metadata. | SID/wallet/TNS, server/session admin, native cancel, deep Oracle SQL/PL/SQL/package scripting, broad import/export/admin parity, and Postgres-only extension/type tools are not claimed. |
...
- MSSQL and Oracle are full-support SQL/RDBMS identities for the documented
  desktop slice: connection UI, query execution, schema/catalog metadata,
  row edit, bounded DDL/admin, completion vocabulary/catalog completion, and
  fixture smoke gates. They do not imply server/session admin, native cancel,
  deep T-SQL/PL/SQL scripting, broad import/export/admin parity, or
  Postgres-only extension/type tools.
