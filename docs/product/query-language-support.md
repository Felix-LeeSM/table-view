...
a bounded SQL Server catalog/query/cancel/tabular runtime slice:
...
promote SQL Server edit, structured DDL, parser, completion, or runtime smoke
support. Oracle remains declared-only: its source-specific dialect/profile
records remain for future promotion, URL parsing, labels, defaults, and
seed/spec inventory, but it is hidden from new connection support and has no
active connection/query/catalog/edit/DDL/parser/completion/runtime smoke support
claim. Future promotion beyond the MSSQL runtime slice or any Oracle support
must land matching source-specific runtime, contract, docs, and smoke evidence
before adding broader SQL Server T-SQL or Oracle SQL/PLSQL parser/completion/
runtime claims.
...
| MSSQL SQL | Bounded SQL Server catalog/query/cancel/tabular runtime. `mssql` exposes SQL-auth connection test/connect/ping, catalog browse/schema/indexes/constraints/relationships, query, multi-statement execution, cancellation, and tabular result rendering with host, port, database, user, password, encryption, and trust-server-certificate inputs. | SQL Server edit, structured DDL, parser, completion, and runtime smoke support remain unclaimed. Named instances, Windows authentication, Azure AD/authSource modes, SQLCMD/batch scripting, admin/security/backup/jobs/users/roles, import/export, profiler/activity, and full T-SQL semantics remain out of scope. |
...
- MSSQL is a bounded SQL Server catalog/query/cancel/tabular runtime identity.
  ...
  support. Oracle remains declared-only and hidden from new connection support.
