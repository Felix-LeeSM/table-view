...
probe only. Oracle carries planned `sql` profile metadata, and MSSQL still does
not claim T-SQL runtime query, catalog, edit, parser, completion, or E2E smoke
support.
...
| MSSQL SQL | SQL Server is an RDBMS identity with connection-only runtime support. The active slice is SQL authentication connection testing with TCP/TDS login and `SERVERPROPERTY('ProductVersion')` probe evidence. | MSSQL has Lifecycle-only capability. Runtime query/catalog/edit, T-SQL parser/completion, runtime fixture smoke, desktop E2E smoke, and broader SQL Server instance/auth behavior remain unsupported. |
...
- MSSQL is connection-only: SQL authentication connection test plus SQL Server
  version probe. This does not imply active T-SQL query, catalog, edit,
  parser/completion, runtime fixture smoke, or E2E smoke support. Oracle remains
  a planned SQL/RDBMS runtime identity with bounded static SQL editor
  autocomplete vocabulary only; this does not imply Oracle connection UI,
  runtime adapter, query, catalog, edit, parser/Safe Mode, runtime fixture/live,
