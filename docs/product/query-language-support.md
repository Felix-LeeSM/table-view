...
- DuckDB `.duckdb` raw SQL uses the RDBMS adapter path for statement-level
  ...
  raw external-file functions, and string replacement scans; extension autoload
  is disabled. Read-only files reject writes.
- DuckDB file analytics has registered local source preview basics and
  ...
  parity/history/import and E2E smoke coverage remain follow-ups (#188, #246).
