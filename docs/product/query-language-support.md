...
placeholders. Oracle also has primary-key-scoped row edit and a bounded static
parser/Safe Mode boundary for common SELECT/DML/DDL safety classification, with
PL/SQL package/admin paths unsupported. Those slices do not create Oracle
structured DDL, sequence/synonym DDL/admin workflows, runtime fixture/live
evidence, desktop E2E smoke, or full Oracle SQL/PL/SQL executable-semantics
claims.
...
| Oracle SQL | Oracle has service-name connection UI/runtime support, bounded SELECT tabular result envelopes, transactional DML batch commit/dry-run, cancellation/error surfacing, table-data browse, primary-key-scoped row edit, live catalog/workbench metadata for current service/database, schemas, tables, views, columns, indexes, constraints/FKs, routines/packages, and read-only sequences/synonyms through the shared RDB model, bounded static Oracle SQL editor autocomplete vocabulary for selected keywords, functions, package symbols, sequences, synonyms, and bind placeholders, and bounded static parser/Safe Mode evidence for SELECT including `DUAL`, INSERT, bounded UPDATE/DELETE, WHERE-less UPDATE/DELETE destructive detection, narrow MERGE, CREATE TABLE/INDEX/VIEW, Oracle `NUMBER`/`VARCHAR2`/`CLOB`/`BLOB` column types, ALTER ADD/RENAME, and DROP/TRUNCATE/ALTER DROP destructive detection. Static SQL seed contract still declares future SQL promotion evidence. | Oracle has Lifecycle, RelationalCatalog, and RelationalQuery runtime capability only. Row edit uses Oracle quoted identifiers and no all-column WHERE fallback; no-PK tables remain browse-only. Metadata-denied dictionary reads fail safe-empty where the adapter can identify Oracle metadata denial. Structured DDL, sequence/synonym DDL/admin workflows, SID/TNS alias/wallet/TLS behavior, runtime fixture/live evidence, desktop E2E smoke, and full PL/SQL executable semantics remain unsupported. PL/SQL blocks, package/routine DDL, routine execution, and admin/privilege paths are outside the static safety boundary. Completion suggestions and parser/Safe Mode metadata do not widen runtime Oracle support. |
...
- MSSQL runtime is limited to SQL authentication connection test, SQL Server
  ...
  Oracle structured DDL, sequence/synonym DDL/admin workflows, runtime
  fixture/live, desktop E2E smoke, or full Oracle SQL/PL/SQL executable
  semantics.
