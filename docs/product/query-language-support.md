...
- Completion / autocomplete: MariaDB shares MySQL-family vocabulary and exposes
  the profile/completion `RETURNING` delta for unknown server versions and
  known MariaDB versions at `>= 10.5.0`; known older versions suppress the
  suggestion. That delta is not a runtime support guarantee.
- MariaDB-specific deltas: the active adapter still reports `mariadb`, the
  ...
  only uses MariaDB `>= 10.5.0` as a completion-suggestion gate and does not
