...
detail, search render, delete-plan, and error-surface path; static
...
- Runtime: Elasticsearch and OpenSearch live connection test,
  ...
  OpenSearch connect/auth/TLS, catalog metadata, selected-index detail,
  search/render, delete-plan, and error-surface paths on Ubuntu.
...
- Remaining unsupported work: actual live `_delete_by_query` execution,
  profile/explain request workflow, broader admin APIs, broader observability
  workflows, and product-specific destructive deltas remain future gates. Search
...
- Search DSL has bounded live Elasticsearch/OpenSearch `_search` dispatch,
  ...
  delete-by-query safety plans with preview-only estimates. Bounded
