...
`sql` is active for connection-supported SQL/RDBMS profiles only. MSSQL carries
planned `sql` profile metadata plus a bounded static T-SQL parser/Safe Mode
boundary for supported `SELECT`/DML/DDL slices and rejected scripting/admin
paths; its capabilities are still empty. Oracle carries planned `sql` profile
metadata only. That metadata does not create SQL Server or Oracle runtime query,
catalog, edit, completion, fixture/live, or E2E smoke claims.
...
| MSSQL SQL | Planned profile metadata and a static SQL seed contract declare SQL Server as a future RDBMS identity with `sql`, `rdb`, `tabular`, and `rdb-default` contract shape. Static parser/Safe Mode recognizes bounded T-SQL slices: bracket identifiers, `N'...'` strings, `SELECT TOP`, common DML, bounded CREATE/ALTER/DROP/TRUNCATE DDL, and SQL Server type synonyms that map to existing AST types. Unsupported scripting/admin boundaries such as `EXEC`, `EXECUTE`, `GO`, `USE`, `DBCC`, `DENY`, `BACKUP`, and `RESTORE` classify as warn/danger safety metadata instead of active execution support. | Capabilities are empty. There is no SQL Server connection UI, runtime query/catalog/edit path, completion claim, auth/TLS/encryption/instance contract, runtime fixture/live evidence, or desktop E2E smoke. `TOP PERCENT`, `WITH TIES`, procedure bodies, batches, SQL Server admin tooling, and full T-SQL dialect parity remain unsupported. |
...
- MSSQL is a planned SQL/RDBMS identity with bounded static T-SQL parser/Safe
  Mode metadata only. Oracle remains a planned SQL/RDBMS identity only. Declared
  profile metadata and static seed contracts do not imply SQL Server or Oracle
  connection, query, catalog, edit, completion, runtime fixture/live, or E2E
  smoke support.
