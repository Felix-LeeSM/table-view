...
`sql` runtime/query/parser/Safe Mode support is active for
connection-supported SQL/RDBMS profiles only. MSSQL and Oracle carry planned
`sql` profile metadata, and their capabilities are empty. MSSQL still has no
active T-SQL parser/completion claim. Oracle has bounded static SQL editor
autocomplete vocabulary for selected keywords, functions, package symbols,
sequences, synonyms, and bind placeholders; that vocabulary is editor
assistance only and does not create Oracle connection UI, runtime adapter,
query/catalog/edit, parser/Safe Mode, runtime fixture/live evidence, desktop E2E
...
- MSSQL is a planned SQL/RDBMS identity only. Oracle is a planned runtime
  identity with bounded static SQL editor autocomplete vocabulary. Declared
  profile metadata and static seed contracts do not imply active T-SQL, Oracle
  connection UI, runtime adapter, query, catalog, edit, parser/Safe Mode,
