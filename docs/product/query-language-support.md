...
catalog-aware T-SQL completion support in the SQL editor. Bounded
...
| MSSQL SQL | SQL Server is an RDBMS identity with bounded connection/query/catalog metadata runtime support, catalog-aware SQL editor completion, and bounded static parser/Safe Mode metadata. The active runtime slice is SQL authentication connection testing with TCP/TDS login, `SERVERPROPERTY('ProductVersion')` probe evidence, bounded SELECT tabular result envelopes, DML/DDL execution, transactional DML batch commit/dry-run, cancellation, server error surfacing, and catalog/workbench metadata for databases, schemas, tables, views, procedures/functions, columns, indexes, constraints, and FKs. Completion uses the live cached catalog for database, schema, table/view, procedure/function, and column suggestions where metadata is loaded, and keeps curated keyword/function plus bracket identifier quoting support for editor assistance only. Static parser/Safe Mode recognizes bounded T-SQL slices: bracket identifiers, `N'...'` strings, `SELECT TOP`, common DML, bounded CREATE/ALTER/DROP/TRUNCATE DDL, and SQL Server type synonyms that map to existing AST types. Unsupported scripting/admin boundaries such as `EXEC`, `EXECUTE`, `GO`, `USE`, `DBCC`, `DENY`, `BACKUP`, and `RESTORE` classify as warn/danger safety metadata instead of active execution support. | MSSQL has Lifecycle, RelationalCatalog, and RelationalQuery runtime capability. Row edit, runtime Safe Mode/destructive preview evidence, runtime fixture smoke, desktop E2E smoke, TLS-required workflow, broader SQL Server instance/auth behavior, `TOP PERCENT`, `WITH TIES`, procedure bodies, batches, SQL Server admin tooling, and full T-SQL semantic completion remain unsupported. Completion suggestions and static parser/Safe Mode metadata do not widen runtime SQL Server support. |
...
- MSSQL runtime is limited to SQL authentication connection test, SQL Server
  ...
  and catalog/workbench metadata browse for databases/schemas/tables/views/
  procedures/functions/columns/indexes/constraints/FKs. Completion uses that
  ...
  bracket identifier quoting. These do not imply row edit, runtime Safe
  Mode/destructive preview, runtime fixture smoke, E2E smoke, TLS-required
