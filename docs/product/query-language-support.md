...
Oracle also has a bounded static parser/Safe Mode boundary for common
SELECT/DML/DDL safety classification; that metadata does not create Oracle
runtime query, catalog, edit, completion, fixture/live, or E2E smoke claims,
and PL/SQL package/admin paths stay unsupported.
...
| Oracle SQL | Planned profile metadata and a static SQL seed contract declare Oracle as a future RDBMS identity with `sql`, `rdb`, `tabular`, and `rdb-default` contract shape. Static parser/Safe Mode evidence classifies bounded SELECT, DML, and DDL slices: SELECT including `DUAL`, INSERT, bounded UPDATE/DELETE, WHERE-less UPDATE/DELETE destructive detection, narrow MERGE, CREATE TABLE/INDEX/VIEW, Oracle `NUMBER`/`VARCHAR2`/`CLOB`/`BLOB` column types, ALTER ADD/RENAME, and DROP/TRUNCATE/ALTER DROP destructive detection. | Capabilities are empty. There is no Oracle connection UI, runtime query/catalog/edit path, autocomplete claim, service/SID/wallet/TNS contract, runtime fixture/live evidence, or desktop E2E smoke. PL/SQL blocks, package/routine DDL, routine execution, and admin/privilege paths are outside the static safety boundary. |
