...
synonyms, and bind placeholders. Those slices do not create Oracle
...
| Oracle SQL | Oracle has service-name connection UI/runtime support, bounded SELECT tabular result envelopes, transactional DML batch commit/dry-run, cancellation/error surfacing, and bounded static Oracle SQL editor autocomplete vocabulary for selected keywords, functions, package symbols, sequences, synonyms, and bind placeholders. Static SQL seed contract still declares future SQL promotion evidence. | Oracle has Lifecycle and RelationalQuery runtime capability only. Catalog/edit/structured DDL, Oracle SQL/PL/SQL parser/Safe Mode, SID/TNS alias/wallet/TLS behavior, runtime fixture/live evidence, desktop E2E smoke, and full PL/SQL executable semantics remain unsupported. Completion suggestions do not widen runtime Oracle support. |
...
- MSSQL runtime is limited to SQL authentication connection test, SQL Server
  ...
  vocabulary; this does not imply Oracle catalog/edit/structured DDL,
  parser/Safe Mode, runtime fixture/live, desktop E2E smoke, or full Oracle
  SQL/PL/SQL executable semantics.
