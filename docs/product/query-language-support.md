...
| Search DSL | Fixture-backed Search identities and bounded fixture DSL exist for Elasticsearch/OpenSearch fixture catalog/search result paths. Search result rendering stays Search-native for hits, shard/timeout metadata, aggregations, and explain/profile payloads. | Live HTTP execution, connection/auth/TLS handling, response parsing, admin APIs, observability, and full query-language support are deferred. Search fixture files mirror embedded adapter contracts only. |
...
- Search DSL is fixture-backed for Elasticsearch/OpenSearch result rendering and
  adapter contracts only. Search result state hardening does not promote live
  HTTP Search support, which still waits for explicit connection/auth/TLS,
  catalog/search execution, admin, observability, and product-delta gates.
