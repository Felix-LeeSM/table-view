...
| Oracle SQL | Bounded #905/#906 Oracle catalog/query/cancel/tabular runtime plus primary-key-projected row edit through the frontend SQL batch path. `oracle` keeps Oracle profile/dialect metadata, labels, service-name defaults, URL parsing, and seed/spec inventory while enabling service-name lifecycle, catalog metadata, SELECT/DML batch execution, cooperative cancellation, tabular table-data query, key-projected editRows, Oracle identifier/literal SQL generation, tested SELECT/DML/DDL Safe Mode classification, and bounded editor assistance. #907 Runtime Happy Path smoke covers representative service-name connect, seeded catalog/routine browse, SELECT/DML, destructive Safe Mode confirmation, cancellation, and grid edit. #1065 adds SID connections and wallet mTLS; #1609 adds bounded structured table/index/constraint DDL and PL/SQL body/package source; #1779 adds read-only trigger listing; #1072 adds database switching as a service-name re-dial (Oracle has no `USE <db>`), where the picker offers only names the same credentials can dial — the stored service plus the open PDBs, never the `CDB$ROOT` container — and SID profiles fail closed. | Full Oracle SQL parser/completion promotion remains unclaimed. Raw DDL/admin execution, trigger DDL and single-trigger source, PL/SQL body/package authoring, free-form TNS descriptors, wallet-less TLS, advanced auth, users/roles/grants/session/storage/admin paths, import/export, profiler/activity dashboards, switching from a SID profile, and full Oracle semantics are not enabled. |
