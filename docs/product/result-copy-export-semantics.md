...
Search hits 에서는 화면에 표시된 hit 를 JSON 으로 copy 하거나 CSV/TSV 로 export 할 수
있지만, SQL INSERT 는 disabled reason 을 노출한다. Scalar/list 결과는 표시된 값을
copy 할 수 있고, non-empty result 에 대해서만 CSV/TSV export 를 허용한다. Empty result
에서는 copy 와 export 를 모두 막고 그 사유를 disabled reason 으로 보여준다. Write summary
는 JSON copy 만 지원하고 grid row export 는 unsupported 로 표시한다.
...
projection 이 active capability 다. #907 adds representative Runtime Happy Path
...
가능하다.
Shared enterprise abstraction 이 SQL Server auth/TDS/T-SQL contract 와 Oracle
service/SID/TNS/wallet/Oracle SQL contract 를 하나로 합쳐서 숨기면 안 된다.
Valkey 는 KV runtime slice 이며 `connection.test`, `query.query`,
`catalog.browse`, `edit.editKeys` 가 true 다. KV 사이드바 라우팅은
`paradigm === "kv"` 로 판정하고 (#1463 에서 redundant 하던 `paradigmSpecific.keyBrowser`
flag 는 삭제했다), key 편집은 `edit.editKeys` 로 게이팅한다.
...
command query rows, 그리고 Redis 와 공유하는 string/hash/list/set/zset KvMutationPanel
...
이것이 full Redis compatibility claim 은 아니다.
...
scripting parity 와 Oracle SID/TNS/wallet/advanced auth, structured DDL, raw
DDL/admin, full parser/completion promotion, PL/SQL work 는 각각
source-specific promotion issue 에서 evidence 를 잠근 뒤에 capability/profile
claim 을 바꾼다.
...
index tab 에서 사용자가 명시적으로 선택한 mappings/settings/analyzers/templates/field
stats 를 lazy fetch 한다. OpenSearch detail 은 sample documents 를 지원하고,
query tab 은 selected index/alias target 으로 범위를 좁힌 bounded Search DSL 을
live `_search` 로 dispatch 한다. Delete-by-query
safety planning 은 Elasticsearch 와 OpenSearch 모두에서 safe `_search` estimate 를
preview plan 으로 보여준 뒤, Safe Mode confirm gate 를 통과하면 live `_delete_by_query`
를 실행하며, wildcard/broad target 은 막는다. Search DSL editor
completion 은 Elasticsearch 와 OpenSearch 의 product identity 를 분리하고, catalog/mapping
context 를 활용해 index/alias/data-stream/field/type/sort/source suggestions 를 제공한다.
...
은 fixture 와 live 모두에서 safe `_search` estimate 를 계산해 preview plan 으로
...
execution 은 막는다.
Initial sidebar load 는 index-catalog-first shell 이며, search hits 와
explain/profile/destructive plan 은 가져오지 않는다. Selected-index
mappings/settings/templates/field stats/samples 는 lazy detail tab 이나 explicit
...
observability, `_explain` request workflow, full language-core
parser/completion ownership 은 아직 deferred 상태다. `_search` 의 `profile` 계획은
search 쿼리 탭의 Explain 버튼으로 요청하고, 그 응답을 계획 트리 형태로 그린다 (#2153).
...
없다. 이 후보군은 [`docs/roadmap/h6.md`](../roadmap/h6.md) 계약과
adding-data-source checklist 를
통과하기 전까지 candidate-only 상태로 남는다.
...
defer 를 요구한다. Kafka 는 future baseline fixture target 이고 Redpanda 는
compatibility delta 이며, 둘 다 routine Runtime Happy Path wiring 이 아니라 future
