...
- Elasticsearch/OpenSearch: embedded fixture-backed Search catalog, selected-index
  lazy detail, mapping/template, and result rendering contract 만 있다. live HTTP
  connection/query/admin 은 없다.
...
Search hits/explain/profile/admin plan 은 query follow-up gate 전까지 initial sidebar
load 에서 가져오지 않는다. live HTTP connection, catalog/query execution,
admin/observability 는 아직 deferred 다.
