...
- Elasticsearch/OpenSearch: embedded fixture-backed Search admin/result
  ...
  분리해서 기록한다.
- MSSQL: declared planned RDBMS identity plus bounded static T-SQL parser/Safe
  Mode metadata only. connection UI/runtime/completion support 는 아직 없다.
...
| MSSQL | planned declared RDBMS identity; no runtime | bounded static T-SQL parser/Safe Mode metadata; no runtime safety claim | deferred | `mssql` profile/dialect identity exists as capability-empty `declared-rdb`. Static parser/Safe Mode covers bounded `SELECT TOP`, bracket identifiers, `N'...'` strings, common DML, bounded DDL, and warn/danger metadata for unsupported SQL Server scripting/admin boundaries. Connection UI, runtime query/catalog/edit, completion, SQL Server auth/TLS/encryption/instance behavior, runtime fixture, and live evidence are not implemented |
