...
- Elasticsearch/OpenSearch: embedded fixture-backed Search admin/result
  ...
  live catalog, bounded live `_search` query dispatch, and backend Search DSL
  validator 로 URL/auth/TLS, product/version detection, indexes, aliases, data
  streams, mappings, settings/analyzers, templates, field paths,
  query/filter/aggs preflight, hits/fields/highlights/sort/aggs response
  parsing 을 지원한다. OpenSearch catalog/query 와 Search admin 은
...
| Elasticsearch/OpenSearch | Elasticsearch live connection + live catalog + bounded live Search query plus fixture-backed Search admin slice | index-catalog sidebar shell plus selected-index lazy detail and mapping/search envelope guardrails | Backend-bounded JSON Search DSL validator; editor parser/completion deferred | Elasticsearch exposes URL/auth/TLS connection UI, a live HTTP root probe that detects product/version/distribution and surfaces network/auth failures, live catalog reads for indexes, aliases, data streams, mappings, settings/analyzers, templates, and field paths, and bounded live `_search` execution with backend validation for `match_all`, `term`, `terms`, `match`, `bool` filters, `range`, `exists`, and `terms`/`value_count` aggregations plus hits/source/fields/highlights/sort/shards/aggs response parsing. Unsupported body keys, unsupported aggregation kinds, raw/admin targets, and destructive/admin APIs reject before live Search dispatch. Fixture destructive plan contracts remain available for the embedded Search slice. OpenSearch live connection/catalog/query, Search admin APIs, observability, profile/explain request workflow, and product-specific live deltas beyond the Elasticsearch query/catalog slice are deferred |
...
payload 를 Search-native renderer 에 연결한다.
Initial sidebar load 는 search hits, explain/profile/admin plan 을 가져오지
않는다. Elasticsearch live admin 과 OpenSearch live
