...
| Redis | connection/profile + backend KV primitives + key browser/value preview UI + bounded command editor runtime | backend KV guardrails plus bounded command allowlist; destructive typed-confirm commands stay routed to follow-up | redis-command profile; backend allowlist parser active, completion future-contract | key browser/value preview are live. Backend guarded string set, delete confirmation, TTL expire/persist, and bounded stream read exist as typed IPC primitives. The command editor supports selected read/write/TTL/stream commands with grid projection. Full value editor, broad TTL/write UI, cluster/pubsub/modules/consumer-group management, destructive command confirmation, and Valkey support are follow-up |
