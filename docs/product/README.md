...
- MSSQL: SQL authentication connection UI/runtime probe, SQL Server version
  ...
  active. The runtime returns the shared tabular result envelope and supports
  DML batch commit/dry-run plus cancellation/error surfacing. Bounded T-SQL
  editor completion vocabulary and bounded static parser/Safe Mode metadata are
  active as editor/safety assistance. Catalog browse, row edit, runtime Safe
  Mode smoke, runtime fixture smoke, desktop E2E, and full T-SQL semantic parity
  remain deferred.
...
| MSSQL | bounded SQL Server auth/query runtime | bounded static T-SQL parser/Safe Mode metadata; no runtime safety smoke claim | bounded T-SQL editor vocabulary | `mssql` profile/dialect identity exists with Lifecycle and RelationalQuery backend capability. Connection UI/runtime test covers SQL authentication, TCP/TDS login, timeout-bounded `SERVERPROPERTY('ProductVersion')` probe, and live testcontainer evidence. Query runtime supports bounded SELECT tabular envelopes plus DML/DDL execution, transactional DML batches, cancellation, and server error surfacing. Static parser/Safe Mode covers bounded `SELECT TOP`, bracket identifiers, `N'...'` strings, common DML, bounded DDL, and warn/danger metadata for unsupported SQL Server scripting/admin boundaries. Bounded editor completion covers curated T-SQL keywords/functions and bracket identifier quoting as editor assistance only. catalog browse, row edit, runtime Safe Mode/destructive preview evidence, runtime fixture smoke, TLS-required workflow, broader SQL Server instance/auth behavior, and full T-SQL semantic completion remain follow-up |
...
| MSSQL | `e2e/fixtures/seed.mssql.sql`, `src-tauri/tests/mssql_connection_routing.rs` | static SQL seed contract plus live SQL Server test evidence for TCP/TDS login, `SERVERPROPERTY` version probe, bounded SELECT tabular envelope, DML batch commit, server error surfacing, and query cancellation |
...
SELECT/DML query runtime, and transactional DML batch support 이다. Bounded editor
completion 은 curated T-SQL vocabulary 와 bracket identifier quoting 으로 제한된다.
bounded static parser/Safe Mode metadata 는 editor/safety assistance 로 active 이고,
catalog/edit/runtime Safe Mode smoke/runtime fixture smoke 는 follow-up 이다.
...
completion, bounded static parser/Safe Mode metadata 로 승격됐지만,
catalog/edit/runtime Safe Mode/runtime smoke claim 은 아직 없다. Oracle 은
