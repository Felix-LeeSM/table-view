...
- Redis workflow: connection/profile, database/key scan, typed value preview,
  backend guarded KV primitives, and bounded command editor runtime 가 있다.
...
| Redis | connection/profile + backend KV primitives + key browser/value preview UI + bounded command editor runtime | backend KV guardrails plus bounded command allowlist; single-key destructive command paths require typed `confirmKey` | redis-command profile; backend allowlist parser active, completion future-contract | key browser/value preview are live. Backend guarded string set, delete confirmation, TTL expire/persist, and bounded stream read exist as typed IPC primitives. The command editor supports selected read/write/TTL/stream commands with grid projection, and backend command safety rejects single-key `DEL`/`PERSIST` unless the caller supplies the exact key as `confirmKey`. Full value editor, broad TTL/write UI, cluster/pub/sub/modules/consumer-group management, broader destructive command UI, and Valkey support are follow-up |
