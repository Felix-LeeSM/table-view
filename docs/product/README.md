...
- MSSQL: SQL authentication connection UI/runtime probe is active for
  ...
  `SERVERPROPERTY`. query/catalog/edit/parser/completion/runtime smoke remain
  deferred.
...
claim 은 아직 없다. Oracle 의 bounded static SQL editor autocomplete vocabulary 는
capability-empty runtime boundary 를 바꾸지 않는다.
