...
| Oracle | planned declared RDBMS identity only | bounded static Oracle SQL parser/Safe Mode boundary; no runtime safety claim | deferred | `oracle` profile/dialect identity exists as capability-empty `declared-rdb`. Static parser/Safe Mode evidence classifies bounded SELECT/DML/DDL slices and marks PL/SQL package/admin paths unsupported, but connection UI, runtime query/catalog/edit, autocomplete, service/SID/wallet/TNS behavior, runtime fixture, smoke, and live evidence are not implemented |
