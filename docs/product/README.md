...
- Redis workflow: connection/profile, database/key scan, typed value preview,
  bounded value mutation panel, backend guarded KV primitives, and bounded
  command editor runtime 가 있다.
  Full CLI/admin parity and broad destructive command execution remain follow-up.
...
| Redis | connection/profile + backend KV primitives + key browser/value preview/edit UI + bounded command editor runtime | backend KV guardrails plus bounded command allowlist and typed-confirm mutation controls | redis-command profile; backend allowlist parser active, completion future-contract | key browser/value preview are live. The value panel promotes bounded string/hash/list/set/zset edits plus expire/persist/delete preview/confirm flows, while partial/unsupported surfaces fail visibly. Backend guarded string set, delete confirmation, TTL expire/persist, bounded stream read, and exact-key `confirmKey` enforcement for single-key `DEL`/`PERSIST` exist as typed IPC/runtime primitives. The command editor supports selected read/write/TTL/stream commands with grid projection. Full CLI/admin parity, stream consumer UI, cluster/pubsub/modules/consumer-group management, multi-key destructive commands, and Valkey support are follow-up |
