...
| Valkey | capability-empty KV identity/profile only | no runtime parser/safety or connection support | deferred compatibility target only | `valkey` is an active `DatabaseType`/profile identity with server connection kind, product label, KV paradigm, marker-only backend adapter, and `redis-command` compatibility target. All runtime capabilities are empty, connection UI/runtime support is not exposed, and `e2e/fixtures/seed.valkey.json` plus `e2e/fixtures/valkey.redis-compatibility.json` are static fixture inventory only. The compatibility matrix covers the current bounded Redis command families, Valkey identity-detection requirement, known Table View deltas, and rejected Redis assumptions, but it is not live Valkey evidence. Redis compatibility remains unverified and is not a support claim |
...
| Valkey | `e2e/fixtures/seed.valkey.json`, `e2e/fixtures/valkey.redis-compatibility.json` | static Redis-compatible fixture and command-family matrix contracts only; no runtime/live support claim. Future promotion must run the seed and candidate command families against a local Valkey container or equivalent local-first fixture before Redis compatibility is claimed |
...
가 쌓이기 전까지 support claim 이 아니다. `e2e/fixtures/valkey.redis-compatibility.json`
는 Redis allowlist 후보군과 unsupported Redis assumptions 를 고정하지만, Valkey
connection UI/runtime 을 열거나 Redis smoke 를 상속하는 근거가 아니다.
