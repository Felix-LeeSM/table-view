...
- Elasticsearch/OpenSearch: embedded fixture-backed Search admin/result
  ...
  validator, TypeScript editor completion, and delete-by-query plan estimates
  로 URL/auth/TLS,
  ...
  preflight, sort/source-filter preflight, hits/fields/highlights/sort/aggs
...
| Elasticsearch/OpenSearch | Elasticsearch live connection + live catalog + bounded live Search query plus fixture/live delete-by-query safety planning | index-catalog sidebar shell plus selected-index lazy detail and mapping/search/destructive-plan guardrails | Backend-bounded JSON Search DSL validator plus Elasticsearch TypeScript editor completion; full parser/OpenSearch completion deferred | Elasticsearch exposes URL/auth/TLS connection UI, a live HTTP root probe that detects product/version/distribution and surfaces network/auth failures, live catalog reads for indexes, aliases, data streams, mappings, settings/analyzers, templates, and field paths, bounded live `_search` execution with backend validation for `match_all`, `term`, `terms`, `match`, `bool` filters, `range`, `exists`, supported `sort` and `_source` filters, and `terms`/`value_count` aggregations plus hits/source/fields/highlights/sort/shards/aggs response parsing, mapping/catalog-aware editor completion for supported JSON Search DSL contexts, and delete-by-query safety planning that estimates matching documents through a safe `_search` request before requiring explicit target confirmation for execution intent. Unsupported body keys, unsupported aggregation kinds, raw/admin targets, and destructive/admin APIs reject before live Search dispatch. Actual live `_delete_by_query` execution, OpenSearch live connection/catalog/query, broader Search admin APIs, observability, profile/explain request workflow, full parser ownership, and product-specific live deltas beyond the Elasticsearch query/catalog/destructive-plan/editor-completion slice are deferred |
...
갖고 있다. Elasticsearch 는 connection dialog 와 backend `test_connection` 에서
...
query/filter/aggs/sort/source-filter preflight, hits/source/fields/highlight/sort,
shards/timeout/total relation/took, aggregations, explain/profile response
payload 를 Search-native renderer 에 연결한다. Elasticsearch Search DSL editor
completion 은 live catalog target, selected-index mapping fields, supported
query clauses, aggregation kinds, and body keys 만 제안한다. Delete-by-query safety planning
...
connection/catalog/query/admin/observability, profile/explain request workflow
는 아직 deferred 다.
...
- Routine runtime smoke currently proves the GitHub Runtime Happy Path for
  ...
  and Elasticsearch. Other smoke specs or source inventories do not widen
  product support until the CI script and support docs promote them.
