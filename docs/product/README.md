...
- Elasticsearch/OpenSearch: embedded fixture-backed Search admin/result
  contract 가 있다. Search uses an index-catalog-first workbench boundary:
  ...
  dispatch, backend Search DSL validator, Runtime Happy Path smoke, and
  delete-by-query plan estimates 로 URL/auth/TLS, product/version detection,
  query/filter/aggs preflight, hits/fields/highlights/sort/aggs response
  parsing, and preview-only destructive planning 을 지원한다. OpenSearch 는
  URL/auth/TLS live
...
Elasticsearch/OpenSearch actual live admin execution, observability,
