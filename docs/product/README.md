...
- Redis workflow: connection/profile, database/key scan, typed value preview,
  bounded value mutation panel, backend guarded KV primitives, and focused
  backend command allowlist/dispatch evidence 가 있다. Dedicated interactive
  Redis command editor UI 는 아직 shipped surface 가 아니며 KV query editor 는
  placeholder 다. Full CLI/admin parity and broad destructive command execution
  remain follow-up.
...
| Redis | connection/profile + backend KV primitives + key browser/value preview/edit UI; backend command dispatch is focused evidence below the shipped key/value surface | backend KV guardrails plus bounded command allowlist and typed-confirm mutation controls; not language-core parser ownership | redis-command profile identity only; backend allowlist parser has tests, completion future-contract | key browser/value preview are live. The value panel promotes bounded string/hash/list/set/zset edits plus expire/persist/delete preview/confirm flows, while partial/unsupported surfaces fail visibly. Backend guarded string set, delete confirmation, TTL expire/persist, bounded stream read, selected read/write/TTL/stream command dispatch, tabular projection, and exact-key `confirmKey` enforcement for single-key `DEL`/`PERSIST` exist as focused IPC/runtime evidence. The current KV query editor still renders the Redis placeholder, so this is not a shipped interactive Redis command editor UI claim. Full CLI/admin parity, stream consumer UI, cluster/pubsub/modules/consumer-group management, multi-key destructive commands, broader command coverage, and Valkey support are follow-up |
