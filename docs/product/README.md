...
- RDB 에서 `connection.switchDatabase` 가 true 인 PostgreSQL, MySQL, MariaDB,
  ...
  쓴다. Oracle 은 `USE <db>` 가 없으므로 switch 가 저장된 service name/SID 를
  다시 dial 하고, picker 는 session container + 접근 가능한 open PDB 를 나열한다
  (#1072). SQLite/DuckDB 는 file/session scope 로 고정되고, MSSQL 은 현재 bounded
...
(service name/SID re-dial) 까지 승격한다. TNS/wallet-less TLS/advanced
