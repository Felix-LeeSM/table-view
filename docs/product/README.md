...
- MSSQL: SQL authentication connection UI/runtime probe, SQL Server version
  ...
  DML batch commit/dry-run plus cancellation/error surfacing. Catalog browse,
  row edit, parser/Safe Mode, runtime fixture smoke, desktop E2E, and full
  T-SQL semantic parity remain deferred.
...
| MSSQL | bounded SQL Server auth/query runtime | no active T-SQL parser/runtime safety claim | bounded T-SQL editor vocabulary | `mssql` profile/dialect identity exists with Lifecycle and RelationalQuery backend capability. Connection UI/runtime test covers SQL authentication, TCP/TDS login, timeout-bounded `SERVERPROPERTY('ProductVersion')` probe, and live testcontainer evidence. Query runtime supports bounded SELECT tabular envelopes plus DML/DDL execution and transactional DML batches. Bounded editor completion covers curated T-SQL keywords/functions and bracket identifier quoting as editor assistance only. catalog browse, row edit, parser/Safe Mode, runtime fixture smoke, TLS-required workflow, broader SQL Server instance/auth behavior, and full T-SQL semantic completion remain follow-up |
...
catalog/edit/parser/Safe Mode/runtime fixture smoke 는 follow-up 이다.
...
MSSQL 은 connection plus bounded query runtime support 와 bounded editor
completion 으로 승격됐지만 catalog/edit/parser/Safe Mode/runtime smoke claim 은
아직 없다. Oracle 은 service-name lifecycle-only connection support 와 bounded
static editor completion 으로 승격됐지만 query/catalog/edit/parser/Safe
Mode/runtime smoke claim 은 아직 없다.
