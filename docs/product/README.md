...
- Elasticsearch/OpenSearch: embedded fixture-backed Search admin/result
  ...
  Elasticsearch endpoint rejection, and auth/network error surfacing 을 지원한다.
  Elasticsearch smoke 는 live runtime evidence 이고 Search fixture files 는
  contract evidence 다. OpenSearch catalog/query 와 actual Search admin execution
...
| Elasticsearch/OpenSearch | Elasticsearch live connection + live catalog + bounded live Search query plus fixture/live delete-by-query safety planning; OpenSearch live connection only | index-catalog sidebar shell plus selected-index lazy detail and mapping/search/destructive-plan guardrails for Elasticsearch; OpenSearch catalog/query deferred | Editor parser/completion deferred; backend Search DSL validator only | Elasticsearch exposes URL/auth/TLS connection UI, a live HTTP root probe that detects product/version/distribution and surfaces network/auth failures, live catalog reads for indexes, aliases, data streams, mappings, settings/analyzers, templates, and field paths, bounded live `_search` execution with backend validation for `match_all`, `term`, `terms`, `match`, `bool` filters, `range`, `exists`, and `terms`/`value_count` aggregations plus hits/source/fields/highlights/sort/shards/aggs response parsing, and delete-by-query safety planning that estimates matching documents through a safe `_search` request before requiring explicit target confirmation for execution intent. OpenSearch exposes URL/auth/TLS connection UI and a live HTTP root probe that verifies OpenSearch product/version/distribution, rejects Elasticsearch endpoints, and surfaces auth/network failures. Unsupported body keys, unsupported aggregation kinds, raw/admin targets, and destructive/admin APIs reject before live Search dispatch. Actual live `_delete_by_query` execution, OpenSearch live catalog/query, broader Search admin APIs, observability, profile/explain request workflow, and product-specific live deltas beyond these slices are deferred |
...
| OpenSearch | `e2e/fixtures/seed.search.opensearch.json`, `src-tauri/src/db/search.rs`, `src-tauri/src/db/search_http.rs`, `src-tauri/src/db/search/tests.rs`, `src-tauri/src/commands/connection/crud.rs`, `src/types/connection.ts`, `src/types/dataSource.ts` | embedded Search fixture contract plus focused live HTTP root-probe connection tests for URL/auth/TLS, product/version/distribution detection, Elasticsearch endpoint rejection, and auth/network failures; no Runtime Happy Path smoke or OpenSearch catalog/query runtime claim |
...
OpenSearch probe 는 Elasticsearch endpoint 를 거부한다. Elasticsearch live catalog
...
catalog/query/admin/observability, profile/explain request workflow 는 아직
deferred 다.
