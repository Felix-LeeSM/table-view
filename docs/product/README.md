...
- Full admin parity 는 staged promotion 대상이다 (#1077, 2026-07-02 owner
  ...
  는 depth step 후속. Stage 1 의 첫 두 슬라이스 (SQL-file import, grid export
  계열) 는 이미 출하됐고, 잔여 in-scope 슬라이스는 다음이다: 테이블/쿼리 결과
  tabular JSON export (#1638), CSV row-level import PG-first (#1639 preview ->
  #1640 commit), MySQL restorable dump (#1641), MSSQL 확대 (#1642). 여전히
