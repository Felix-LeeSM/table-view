...
- MSSQL: SQL authentication connection UI/runtime probe, SQL Server version
  ...
  plus cancellation/error surfacing. Bounded T-SQL editor completion vocabulary
  and bounded static parser/Safe Mode metadata are active as editor/safety
  assistance. Row edit, runtime Safe Mode smoke, runtime fixture smoke, desktop
  E2E, TLS-required workflow evidence, and full T-SQL semantic parity remain
  deferred.
- Oracle: service-name connection UI/runtime probe is active for connection test
  only; bounded static SQL editor autocomplete vocabulary is active as editor
  ...
  wallet/TLS, query/catalog/edit path, runtime fixture/live evidence, desktop
  E2E, and full PL/SQL executable semantics remain deferred.
...
| MSSQL | bounded SQL Server auth/query/catalog metadata runtime | bounded static T-SQL parser/Safe Mode metadata; no runtime safety smoke claim | bounded T-SQL editor vocabulary | `mssql` profile/dialect identity exists with Lifecycle, RelationalCatalog, and RelationalQuery backend capability. Connection UI/runtime test covers SQL authentication, TCP/TDS login, timeout-bounded `SERVERPROPERTY('ProductVersion')` probe, and live testcontainer evidence. Query runtime supports bounded SELECT tabular envelopes plus DML/DDL execution, transactional DML batches, cancellation, and server error surfacing. Catalog/workbench metadata covers databases, schemas, tables, views, procedures/functions, columns, indexes, constraints, and FK references consumed by SchemaTree/DataGrid contracts. Static parser/Safe Mode covers bounded `SELECT TOP`, bracket identifiers, `N'...'` strings, common DML, bounded DDL, and warn/danger metadata for unsupported SQL Server scripting/admin boundaries. Bounded editor completion covers curated T-SQL keywords/functions and bracket identifier quoting as editor assistance only. Row edit, runtime Safe Mode/destructive preview evidence, runtime fixture smoke, desktop E2E smoke, TLS-required workflow, broader SQL Server instance/auth behavior, and full T-SQL semantic completion remain follow-up |
| Oracle | connection-only service-name probe | bounded static Oracle SQL parser/Safe Mode boundary; no runtime safety claim | bounded static SQL editor vocabulary | `oracle` profile/dialect identity exists with Lifecycle-only backend capability. Connection UI/runtime test covers Host/Port/User/Password/Service name through the backend Oracle lifecycle adapter. Static SQL seed contract remains future promotion evidence. Static parser/Safe Mode evidence classifies bounded SELECT/DML/DDL slices and marks PL/SQL package/admin paths unsupported. Existing bounded editor autocomplete can suggest selected Oracle SQL keywords, functions, package symbols, sequences, synonyms, and bind placeholders as editor assistance only. SID, TNS alias, wallet/TLS, runtime query/catalog/edit, runtime fixture/live evidence, desktop E2E, and full PL/SQL executable semantics are not implemented |
...
| MSSQL | `e2e/fixtures/seed.mssql.sql`, `src-tauri/tests/mssql_connection_routing.rs` | static SQL seed contract plus live SQL Server test evidence for TCP/TDS login, `SERVERPROPERTY` version probe, bounded SELECT tabular envelope, DML batch commit, server error surfacing, query cancellation, and catalog/workbench metadata for databases/schemas/tables/views/procedures/columns/indexes/constraints/FKs |
| Oracle | `e2e/fixtures/seed.oracle.sql` | planned static SQL seed contract only. Service-name connection evidence lives in focused backend/profile tests, not this fixture or Runtime Happy Path smoke. Bounded static editor autocomplete vocabulary is profile/WASM evidence, not fixture/runtime evidence |
...
SELECT/DML query runtime, transactional DML batch support, and catalog/workbench
metadata browse for databases/schemas/tables/views/procedures/columns/indexes/
constraints/FKs 이다. Bounded editor completion 은 curated T-SQL vocabulary 와
bracket identifier quoting 으로 제한된다. bounded static parser/Safe Mode metadata 는
editor/safety assistance 로 active 이고, row edit/runtime Safe Mode smoke/runtime
fixture smoke/desktop E2E/TLS-required workflow 는 follow-up 이다.
...
MSSQL 은 connection plus bounded query runtime support, bounded editor
completion, catalog/workbench metadata browse, bounded static parser/Safe Mode
metadata 로 승격됐지만, row edit/runtime Safe Mode/runtime smoke claim 은 아직 없다. Oracle 은
service-name lifecycle-only connection support 와 bounded static editor completion
으로 승격됐지만 query/catalog/edit/parser/Safe Mode/runtime smoke claim 은 아직
없다.
