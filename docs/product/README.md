...
- MSSQL: runtime catalog/query/edit-row support is active for issue #903. The SQL Server
  ...
  because that catalog view silently truncates for an unprivileged login.
...
- Full admin parity 는 staged promotion 대상이다 (#1077, 2026-07-02 owner
  ...
  없으면 `CapabilityNotEnabled` 로 fail loud 한다 (부분 목록을 완전한 목록처럼
  렌더하지 않기 위함). role 생성/변경/삭제 (CRUD) 는 depth step 후속.
