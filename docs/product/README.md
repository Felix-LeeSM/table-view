...
- DuckDB file analytics paths stay in active-session adapter state and clear on
  ...
  global query backend also accepts read-only SELECT statements that reference
  at least one registered alias without passing a source id. Grid export is the
