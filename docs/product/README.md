...
- Elasticsearch/OpenSearch: embedded fixture-backed Search admin/result
  ...
  parsing, sample documents, cancellation, and HTTP error surfacing 을 지원한다.
  Elasticsearch smoke 는 live runtime evidence 이고 Search fixture files 는
  contract evidence 다. OpenSearch Runtime Happy Path smoke, delete-by-query
  planning/execution, and actual Search admin execution 은 deferred 다.
...
| Elasticsearch/OpenSearch | Elasticsearch live connection + live catalog + bounded live Search query plus fixture/live delete-by-query safety planning; OpenSearch live connection + live catalog + bounded live Search query | index-catalog sidebar shell plus selected-index lazy catalog detail and samples for both products; mapping/search guardrails for both products; destructive-plan guardrails for Elasticsearch | Editor parser/completion deferred; backend Search DSL validator only | Elasticsearch exposes URL/auth/TLS connection UI, a live HTTP root probe that detects product/version/distribution and surfaces network/auth failures, live catalog reads for indexes, aliases, data streams, mappings, settings/analyzers, templates, and field paths, bounded live `_search` execution with backend validation for `match_all`, `term`, `terms`, `match`, `bool` filters, `range`, `exists`, and `terms`/`value_count` aggregations plus hits/source/fields/highlights/sort/shards/aggs response parsing, and delete-by-query safety planning that estimates matching documents through a safe `_search` request before requiring explicit target confirmation for execution intent. OpenSearch exposes URL/auth/TLS connection UI, a live HTTP root probe that verifies OpenSearch product/version/distribution, rejects Elasticsearch endpoints, surfaces auth/network failures, reads live indexes, aliases, data streams, mappings, settings/analyzers, composable/legacy templates, and field paths, and dispatches bounded live `_search` requests through the same validator/result renderer with cancellation and HTTP error handling. Unsupported body keys, unsupported aggregation kinds, raw/admin targets, and destructive/admin APIs reject before live Search dispatch. Actual live `_delete_by_query` execution, OpenSearch delete-by-query planning, broader Search admin APIs, observability, profile/explain request workflow, Runtime Happy Path OpenSearch smoke, and product-specific live deltas beyond these slices are deferred |
...
| OpenSearch | `e2e/fixtures/seed.search.opensearch.json`, `src-tauri/src/db/search.rs`, `src-tauri/src/db/search_http.rs`, `src-tauri/src/db/search_live_query.rs`, `src-tauri/src/db/search/tests.rs`, `src-tauri/src/db/search/tests/live_query.rs`, `src/components/workspace/SearchSidebar.test.tsx`, `src/components/search/SearchIndexDetailPanel.test.tsx`, `src/components/query/QueryTab.search-route.test.tsx`, `src/types/dataSource.ts` | embedded Search fixture contract plus focused live HTTP connection/catalog/query tests for URL/auth/TLS, product/version/distribution detection, Elasticsearch endpoint rejection, auth/network failures, indexes, aliases, data streams, mappings, settings/analyzers, composable/legacy templates, field paths, bounded `_search` dispatch/result rendering, sample documents, cancellation, and HTTP error surfacing; no Runtime Happy Path smoke or OpenSearch admin runtime claim |
...
query tab 은 bounded Search DSL 을 live `_search` 로 dispatch 한다. Elasticsearch Runtime Happy Path smoke now proves live service connect,
...
않는다. Elasticsearch actual live admin execution 과 OpenSearch Runtime Happy Path
smoke/admin/delete-by-query planning, observability, profile/explain request workflow 는 아직 deferred 다.
