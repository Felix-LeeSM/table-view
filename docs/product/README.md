...
|---|---|---|---|---|
| PostgreSQL | strong | strong bounded subset | WASM-first + installed-extension-gated packs | 현재 가장 강한 lane 이다. routine desktop smoke 는 connect/browse/edit/query, Explain plan-inspection UI/source-label path, and seeded `pgcrypto` installed-extension completion gating 을 증명한다. full dialect/admin/arbitrary extension semantics, catalog-backed enumeration of every extension symbol, Safe Mode dialog E2E 는 보장하지 않음 |
...
- PostgreSQL is the active query/workbench parity lane. Its current routine
  ...
  the Explain plan-inspection UI/source label, and seeded `pgcrypto`
  installed-extension completion gating. Safe Mode confirmations, DDL structure
  flows, broader history-source coverage, cancellation, ERD, admin, arbitrary
  extension semantics, and profiler/activity scenarios need separate promotion
  before product claims widen.
