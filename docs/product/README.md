...
- Oracle: service-name connection UI/runtime probe plus bounded SELECT query
  ...
  SELECT/DML/DDL slices and blocks PL/SQL package/admin paths. No-PK Oracle
  tables can be browsed but row edit controls stay disabled. SID, TNS alias,
  wallet/TLS, structured DDL, sequence/synonym DDL/admin workflows, runtime
  fixture/live evidence, desktop E2E, and full PL/SQL executable semantics
  remain deferred.
...
completion, and bounded static parser/Safe Mode 로 승격됐지만 structured DDL,
sequence/synonym DDL/admin workflow, runtime smoke/live/E2E claim 은 아직 없다.
