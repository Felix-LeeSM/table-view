...
- MSSQL: SQL authentication connection UI/runtime probe, SQL Server version
  ...
  path through the MSSQL fixture. TLS-required workflow evidence,
...
이다. Runtime Happy Path smoke 는 MSSQL fixture로 connect/catalog browse/SELECT/DML/
row edit/Safe Mode confirmation 대표 경로를 증명한다. Bounded editor completion 은 curated T-SQL vocabulary,
...
bounded static editor completion, and bounded static parser/Safe Mode 로
승격됐지만 table-data browse, edit/structured DDL, sequence/synonym DDL/admin
workflow, runtime smoke/live/E2E claim 은 아직 없다.
