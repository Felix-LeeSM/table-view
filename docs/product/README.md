...
- MSSQL: runtime catalog/query/edit-row support is active for issue #903. The SQL Server
  ...
  (T-SQL keeps a default in its own auto-named constraint, so a default change
  drops the current one by catalog lookup and rebinds) and the SET NOT NULL
  NULL-row pre-flight probe.
  Trigger DDL, raw DDL/admin, security/jobs/users/roles, DB-level
...
| MSSQL | SQL-auth/TDS connection plus catalog/query/cancel/tabular runtime and PK-projected row edit through SQL batch | bounded parser/Safe Mode unsupported-boundary recognition only | bounded editor assistance only | `mssql` is a source-specific profile/dialect identity with SQL Server labels, defaults, URL parsing, and seed/spec inventory. Issue #903 promotes connection test/connect/ping, catalog browse/schema/indexes/constraints/relationships, query, multi-statement execution, cancellation, tabular result rendering, and editRows through the frontend SQL batch path with primary-key projection. #907 adds representative Runtime Happy Path smoke for connect, seeded catalog browse, SELECT/DML, destructive Safe Mode confirmation, cancellation, and grid edit. `switchDatabase` remains disabled under the current connection contract. #1071 promotes bounded structured table/index/constraint DDL through the shared StructurePanel path, including column type/nullability/DEFAULT-constraint edits and the SET NOT NULL NULL-row pre-flight probe; #1778 adds `sys.triggers` introspection and #1073 adds admin ops (activity/kill/slow-query/server info). Named instances, Windows authentication, Azure AD/authSource modes, trigger DDL, raw DDL/admin, security/jobs/users/roles, DB-level import/backup/restore, profiler tracing, full T-SQL semantic parity, full workbench parity, and sqlcmd/meta-command/procedure-body scripting remain unclaimed. |
