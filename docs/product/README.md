...
- Elasticsearch/OpenSearch: embedded fixture-backed Search admin/result
  ...
  settings/analyzers, composable/legacy templates, and field paths 를 지원한다.
  Elasticsearch smoke 는 live runtime evidence 이고 Search fixture files 는
  contract evidence 다. OpenSearch query 와 actual Search admin execution
  은 fixture-backed/deferred 다.
...
| Elasticsearch/OpenSearch | Elasticsearch live connection + live catalog + bounded live Search query plus fixture/live delete-by-query safety planning; OpenSearch live connection + live catalog only | index-catalog sidebar shell plus selected-index lazy catalog detail for both products; mapping/search/destructive-plan guardrails for Elasticsearch; OpenSearch query deferred | Editor parser/completion deferred; backend Search DSL validator only | Elasticsearch exposes URL/auth/TLS connection UI, a live HTTP root probe that detects product/version/distribution and surfaces network/auth failures, live catalog reads for indexes, aliases, data streams, mappings, settings/analyzers, templates, and field paths, bounded live `_search` execution with backend validation for `match_all`, `term`, `terms`, `match`, `bool` filters, `range`, `exists`, and `terms`/`value_count` aggregations plus hits/source/fields/highlights/sort/shards/aggs response parsing, and delete-by-query safety planning that estimates matching documents through a safe `_search` request before requiring explicit target confirmation for execution intent. OpenSearch exposes URL/auth/TLS connection UI, a live HTTP root probe that verifies OpenSearch product/version/distribution, rejects Elasticsearch endpoints, surfaces auth/network failures, and reads live indexes, aliases, data streams, mappings, settings/analyzers, composable/legacy templates, and field paths. Unsupported body keys, unsupported aggregation kinds, raw/admin targets, and destructive/admin APIs reject before live Search dispatch. Actual live `_delete_by_query` execution, OpenSearch live query, broader Search admin APIs, observability, profile/explain request workflow, and product-specific live deltas beyond these slices are deferred |
...
| OpenSearch | `e2e/fixtures/seed.search.opensearch.json`, `src-tauri/src/db/search.rs`, `src-tauri/src/db/search_http.rs`, `src-tauri/src/db/search/tests.rs`, `src/components/workspace/SearchSidebar.test.tsx`, `src/components/search/SearchIndexDetailPanel.test.tsx`, `src/types/dataSource.ts` | embedded Search fixture contract plus focused live HTTP connection/catalog tests for URL/auth/TLS, product/version/distribution detection, Elasticsearch endpoint rejection, auth/network failures, indexes, aliases, data streams, mappings, settings/analyzers, composable/legacy templates, and field paths; no Runtime Happy Path smoke or OpenSearch query/admin runtime claim |
...
stats 를 lazy fetch 한다. OpenSearch detail 은 sample documents/query tab 을
노출하지 않는다. Elasticsearch Runtime Happy Path smoke now proves live service connect,
...
않는다. Elasticsearch actual live admin execution 과 OpenSearch live
query/admin/observability, profile/explain request workflow 는 아직 deferred 다.
