...
- Redis workflow: connection/profile, database/key scan, typed value preview,
  ...
  ownership, stream consumer UI, cluster/pubsub/modules, Valkey completion/smoke
  parity, and multi-key destructive command execution remain follow-up.
- Valkey workflow: connection/profile, database/key scan, typed value preview,
  and bounded Redis-compatible command query execution are active. Key mutation
  panel controls, command completion, full Redis compatibility, and Runtime Happy
  Path smoke are not claimed yet.
...
| Valkey | KV runtime for connection + key browser/value preview + bounded command query | Redis-compatible bounded command allowlist and typed confirmation; key mutation panel remains off | deferred compatibility target only | `valkey` is an active `DatabaseType`/profile identity with server connection kind, product label, KV paradigm, Valkey backend adapter profile, and `redis-command` compatibility target. Connection UI/runtime support is exposed for test/connect/key browse/value preview and selected read/write/TTL/stream/destructive command query dispatch through the same bounded Redis command allowlist. `e2e/fixtures/seed.valkey.json` plus `e2e/fixtures/valkey.redis-compatibility.json` remain static Redis-compatible fixture/command-family contracts. Direct key mutation controls, command completion, full Redis compatibility, and Runtime Happy Path smoke are not claimed |
...
| Valkey | `e2e/fixtures/seed.valkey.json`, `e2e/fixtures/valkey.redis-compatibility.json` | static Redis-compatible fixture and command-family matrix contracts plus focused local Valkey testcontainer evidence for connect/key scan/value preview and bounded command query dispatch. Mutation controls, command completion, full Redis compatibility, and Runtime Happy Path smoke remain future promotion gates |
...
`catalog.browse`, `paradigmSpecific.keyBrowser` 가 true 다. Static
`e2e/fixtures/seed.valkey.json` 는 future Redis-compatible seed/reset contract 이고,
`e2e/fixtures/valkey.redis-compatibility.json` 는 Redis allowlist 후보군과
unsupported Redis assumptions 를 고정한다. Focused local Valkey testcontainer
evidence 는 connect/key scan/value preview 와 bounded command query dispatch 까지
support claim 을 넓힌다. `redis-command` 는 bounded command query target 이지만,
completion/full Redis compatibility/smoke claim 은 아니다.
...
- Routine runtime smoke currently proves the GitHub Runtime Happy Path for
  PostgreSQL, MySQL, MariaDB, SQLite, DuckDB `.duckdb`, and MongoDB. Other smoke
  specs or source inventories do not widen product support until the CI script
  and support docs promote them.
