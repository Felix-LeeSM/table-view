...
- Elasticsearch/OpenSearch: embedded fixture-backed Search admin/result
  ...
  분리해서 기록한다.
- MSSQL: declared planned RDBMS identity and static seed contract only.
  connection UI/runtime/parser/completion support 는 아직 없다.
- Oracle: connection UI와 backend test/connect lifecycle path가 있다.
  계약은 Host/Port/User/Password/Service name(EZConnect service name)만
  지원한다. SID, TNS alias, wallet/TLS, catalog/query/edit/DDL, Oracle
  SQL/PL/SQL parser/completion, fixture/live smoke evidence 는 아직 없다.
...
| Oracle | connection-only RDBMS lifecycle | no active Oracle SQL/PL/SQL parser/runtime safety claim | deferred | `oracle` profile/dialect identity exists with `connection.test=true` and backend `oracle` lifecycle adapter. Static SQL seed contract remains future promotion evidence only. Service-name connections are supported through the existing database field; SID, TNS alias, wallet/TLS, runtime query/catalog/edit, runtime fixture, and live evidence are not implemented |
...
MSSQL 은 capability-empty declared RDB identity 다. Oracle 은 별도의
connection-only RDB identity 로, backend lifecycle/test capability 만 갖는다.
