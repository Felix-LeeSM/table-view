...
- MariaDB now has its own routine runtime-smoke baseline for connect, seeded
  ...
  `>= 10.2.1` version context. MariaDB autocomplete keeps the `RETURNING`
  suggestion for unknown versions and known versions at `>= 10.5.0`, and
  suppresses it for known older versions. This does not widen MariaDB-only
