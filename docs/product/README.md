...
- MSSQL: SQL authentication connection UI/runtime probe, SQL Server version
  ...
  assistance. Runtime Safe Mode smoke, runtime fixture smoke, desktop E2E,
  TLS-required workflow evidence, SQLCMD/admin support, structured DDL/admin
  support, and full T-SQL semantic parity remain deferred.
...
primary-key-projected row edit, and catalog/workbench metadata browse for
...
bounded editor completion, catalog/workbench metadata browse, bounded static parser/Safe Mode
metadata 로 승격됐지만, runtime Safe Mode/runtime smoke claim 은 아직 없다. Oracle 은
