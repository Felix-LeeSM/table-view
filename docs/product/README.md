...
- Active connection UI/runtime 대상: PostgreSQL, MySQL, MariaDB, SQLite,
  DuckDB, MSSQL, Oracle, MongoDB, Redis, Valkey, Elasticsearch, OpenSearch.
...
- Elasticsearch/OpenSearch: embedded fixture-backed Search admin/result
  ...
  분리해서 기록한다.
- MSSQL/Oracle: full RDBMS support slice. Connection UI/runtime, query
  execution, schema/catalog metadata, row edit, bounded DML/DDL/admin,
  SQL vocabulary/catalog completion, static seed contracts, and wired Runtime
  Happy Path smoke are active. Server/session admin, native cancel, deep
  T-SQL/PL/SQL scripting, and broad import/export/admin parity remain deferred.
...
| MSSQL | runtime/query/edit/catalog/DDL adapter active with fixture smoke | bounded SQL/Safe Mode slice; deep T-SQL scripting deferred | SQL vocabulary + current-catalog schema/table/column suggestions | SQL Server support covers connection UI/runtime, SQL authentication, schema/table catalog browse, SELECT/DML execution, tabular results, row edit with key-projected SQL preview, bounded DDL/admin execution, completion vocabulary/catalog suggestions, and wired Runtime Happy Path smoke. Named instances, Windows auth, server/session admin, native cancel, stored procedure body authoring, broad import/export/admin parity, and deep T-SQL scripting remain follow-up |
| Oracle | runtime/query/edit/catalog/DDL adapter active with fixture smoke | bounded SQL/Safe Mode slice; deep PL/SQL scripting deferred | SQL vocabulary + current-catalog schema/table/column suggestions | Oracle support covers connection UI/runtime, service-name connections, schema/table catalog browse, SELECT/DML execution, tabular results, row edit with key-projected SQL preview, bounded DDL/admin execution, completion vocabulary/catalog suggestions, and wired Runtime Happy Path smoke. SID/wallet/TNS, server/session admin, native cancel, package/procedure body authoring, broad import/export/admin parity, and deep PL/SQL scripting remain follow-up |
...
| MSSQL | `e2e/fixtures/seed.mssql.sql`, `scripts/fixtures/mssql.ts`, `e2e/smoke/mssql.spec.ts` | wired Runtime Happy Path seed/smoke for SQL Server connect, catalog browse, SELECT/DML, row edit, bounded DDL/admin, and completion evidence |
| Oracle | `e2e/fixtures/seed.oracle.sql`, `scripts/fixtures/oracle.ts`, `e2e/smoke/oracle.spec.ts` | wired Runtime Happy Path seed/smoke for Oracle connect, catalog browse, SELECT/DML, row edit, bounded DDL/admin, and completion evidence |
...
Valkey, Elasticsearch, OpenSearch 로 제한된다.
...
MSSQL 과 Oracle 은 full-support RDB identities 다. Product claim 은 connection UI,
query execution, schema/catalog metadata, row edit, bounded DDL/admin,
completion vocabulary/catalog completion, and fixture smoke gates 로 제한된다.
Server/session admin, native cancel, deep T-SQL/PL/SQL scripting, and
Postgres-only extension/type tools are not implied.
