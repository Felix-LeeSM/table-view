...
- Oracle: service-name connection UI/runtime probe plus bounded SELECT query
  ...
  tables can be browsed but row edit controls stay disabled. Bounded structured
  table/index/constraint DDL preview/execute is active through the Structure
  DDL path; raw DDL/admin execution remains blocked. SID, TNS alias,
  wallet/TLS, sequence/synonym DDL/admin workflows, runtime fixture/live
