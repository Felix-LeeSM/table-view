...
| Oracle | `e2e/fixtures/seed.oracle.sql` | wired Runtime Happy Path seed for Oracle connect, current-user schema browse, `USERS` table read, `ACTIVE_ORACLE_USERS` view and `ORACLE_CATALOG_PING` routine browse, SELECT, DML preview/readback, row edit, and destructive confirmation. SID/TNS/wallet/TLS, sequence/synonym DDL/admin, raw DDL/admin, and full PL/SQL semantics remain unpromoted |
