...
- Elasticsearch/OpenSearch: embedded fixture-backed Search admin/result
  ...
  분리해서 기록한다.
- MSSQL: SQL authentication connection UI/runtime probe is active for
  connection test only, including SQL Server version detection through
  `SERVERPROPERTY`; bounded T-SQL editor completion vocabulary is active as
  editor assistance only. query/catalog/edit/parser/Safe Mode/runtime smoke
  remain deferred.
...
| MSSQL | connection-only SQL Server auth/version probe | no active T-SQL parser/runtime safety claim | bounded T-SQL editor vocabulary | `mssql` profile/dialect identity exists with Lifecycle-only backend capability. Connection UI/runtime test covers SQL authentication, TCP/TDS login, timeout-bounded `SERVERPROPERTY('ProductVersion')` probe, and live testcontainer evidence. Bounded editor completion covers curated T-SQL keywords/functions and bracket identifier quoting as editor assistance only. query/catalog/edit, parser/Safe Mode, runtime fixture smoke, TLS-required behavior, broader SQL Server instance/auth behavior, and full T-SQL semantic completion remain follow-up |
...
| MSSQL | `e2e/fixtures/seed.mssql.sql`, `src-tauri/tests/mssql_connection_routing.rs` | static SQL seed contract plus connection-only live SQL Server test evidence for TCP/TDS login and `SERVERPROPERTY` version probe |
...
MSSQL 은 lifecycle-only connection support 와 bounded editor completion 으로
승격됐지만 query/catalog/edit/parser/Safe Mode/runtime smoke claim 은 아직 없다.
Oracle 은 service-name lifecycle-only connection support 와 bounded static editor
completion 으로 승격됐지만 query/catalog/edit/parser/Safe Mode/runtime smoke
claim 은 아직 없다.
