...
- Oracle: service-name connection UI/runtime probe is active for connection test
  ...
  assistance only. SID, TNS alias, wallet/TLS, query/catalog/edit path,
  parser/Safe Mode, runtime fixture/live evidence, desktop E2E, and full PL/SQL
...
| Oracle | connection-only service-name probe | no active Oracle SQL/PL/SQL parser/runtime safety claim | bounded static SQL editor vocabulary | `oracle` profile/dialect identity exists with Lifecycle-only backend capability. Connection UI/runtime test covers Host/Port/User/Password/Service name through the backend Oracle lifecycle adapter. Static SQL seed contract remains future promotion evidence, and editor autocomplete can suggest selected Oracle SQL keywords, functions, package symbols, sequences, synonyms, and bind placeholders. SID, TNS alias, wallet/TLS, runtime query/catalog/edit, parser/Safe Mode, runtime fixture/live evidence, desktop E2E, and full PL/SQL executable semantics are not implemented |
