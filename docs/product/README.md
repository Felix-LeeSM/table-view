...
| MongoDB | runtime-backed whitelisted document workflow | whitelisted mongosh/MQL | Rust/WASM vocabulary | connection, source-aware catalog metadata, workbench metadata panels, document query/edit, bulk/index/validator slices, cancellation, and destructive Safe Mode gates are active for tested whitelist paths. arbitrary JavaScript/shell behavior, unsupported cursor helpers, version/deployment gates, native document-first result panels, and full-support parity remain follow-up |
