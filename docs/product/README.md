...
| Elasticsearch/OpenSearch | fixture-backed Search slice only | index-catalog sidebar shell plus mapping/search envelope guardrails | bounded fixture DSL only | fixture identity/catalog summary, index/alias/data-stream shell, mapping/template/search result, and destructive plan contracts exist. live connection UI, HTTP auth/TLS, live catalog/query execution, admin APIs, observability, and product-specific live deltas are deferred |
...
shell 만 보여준다. Mapping/settings/templates/sample documents/search hits/explain/
profile/admin plan 은 selected-index 또는 query follow-up gate 전까지 initial sidebar
