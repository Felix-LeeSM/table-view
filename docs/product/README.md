...
- Full admin parity 는 staged promotion 대상이다 (#1077, 2026-07-02 owner
  decision): role/user/permission UI, extension management UI, schema
  diff/migration preview, deep activity/profiler dashboards 는 여전히 scope
  밖. import/export 는 Stage 1 로 승격됐다. Stage 1 의 첫 슬라이스는 SQL-file
  ...
  게이트를 통과한다 — 즉 자동 실행/자동 import 가 아니다. CSV/JSON row-level
  import, users/roles UI (Stage 2), profiler dashboard (Stage 3) 는 후속.
