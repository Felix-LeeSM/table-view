...
- Elasticsearch/OpenSearch: embedded fixture-backed Search admin/result
  ...
  분리해서 기록한다.
- MSSQL/Oracle: declared planned RDBMS identities and static seed contracts.
  MSSQL has no active parser/completion support. Oracle has bounded static SQL
  editor autocomplete vocabulary, but no connection UI/runtime adapter,
  query/catalog/edit path, parser/Safe Mode, runtime fixture/live evidence, or
  desktop E2E claim.
...
MSSQL 과 Oracle 은 별도의 capability-empty declared RDB identities 다. Oracle 의
bounded static SQL editor autocomplete vocabulary 는 capability-empty runtime
boundary 를 바꾸지 않는다.
