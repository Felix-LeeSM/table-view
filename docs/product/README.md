...
- MSSQL: SQL authentication connection UI/runtime probe is active for
  ...
  `SERVERPROPERTY`; bounded T-SQL editor completion vocabulary and bounded
  static parser/Safe Mode metadata are active as editor/safety assistance only.
  query/catalog/edit/runtime Safe Mode smoke remain deferred.
...
| MSSQL | connection-only SQL Server auth/version probe | bounded static T-SQL parser/Safe Mode metadata; no runtime safety claim | bounded T-SQL editor vocabulary | `mssql` profile/dialect identity exists with Lifecycle-only backend capability. Connection UI/runtime test covers SQL authentication, TCP/TDS login, timeout-bounded `SERVERPROPERTY('ProductVersion')` probe, and live testcontainer evidence. Static parser/Safe Mode covers bounded `SELECT TOP`, bracket identifiers, `N'...'` strings, common DML, bounded DDL, and warn/danger metadata for unsupported SQL Server scripting/admin boundaries. Bounded editor completion covers curated T-SQL keywords/functions and bracket identifier quoting as editor assistance only. query/catalog/edit, runtime Safe Mode/destructive preview evidence, runtime fixture smoke, TLS-required behavior, broader SQL Server instance/auth behavior, and full T-SQL semantic completion remain follow-up |
