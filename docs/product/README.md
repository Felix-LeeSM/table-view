...
- Redis workflow: connection/profile, database/key scan, typed value preview,
  ...
  ownership, stream consumer UI, cluster/pubsub/modules, Valkey smoke parity,
...
- Valkey workflow: connection/profile, database/key scan, typed value preview,
  ...
  completion for proven local-runtime rows are active. Key mutation panel
  controls, full Redis compatibility, and Runtime Happy Path smoke are not
  claimed yet.
...
| Valkey | KV runtime for connection + key browser/value preview + bounded command query | Redis-compatible bounded command allowlist and typed confirmation; key mutation panel remains off | TypeScript proven Valkey command subset + current-DB/type-filtered key suggestions | `valkey` is an active `DatabaseType`/profile identity with server connection kind, product label, KV paradigm, Valkey backend adapter profile, and `redis-command` compatibility target. Connection UI/runtime support is exposed for test/connect/key browse/value preview and selected bounded command query rows through the same Redis command allowlist. Command completion is limited to the proven local Valkey runtime rows plus safe current-keyspace hints. `e2e/fixtures/seed.valkey.json` remains a static Redis-compatible fixture contract, while `e2e/fixtures/valkey.redis-compatibility.json` separates proven local-runtime rows from candidate/rejected command families. Direct key mutation controls, full Redis compatibility, and Runtime Happy Path smoke are not claimed |
...
| Valkey | `e2e/fixtures/seed.valkey.json`, `e2e/fixtures/valkey.redis-compatibility.json` | static Redis-compatible seed contract plus a command-family matrix with focused local Valkey testcontainer proof rows for connect/key scan/value preview and selected bounded command query dispatch. Mutation controls, full Redis compatibility, and Runtime Happy Path smoke remain future promotion gates; command completion is focused editor-assistance evidence for proven rows only |
...
Redis compatibility/smoke claim 은 아니다.
