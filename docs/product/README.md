...
- MSSQL: SQL authentication connection UI/runtime probe, SQL Server version
  ...
  the bounded adapter path. Runtime Safe Mode smoke, runtime fixture smoke,
  desktop E2E, TLS-required workflow evidence, SQLCMD/admin/security/backup/jobs/
  users/roles support, and full T-SQL semantic parity remain deferred.
- Oracle: service-name connection UI/runtime probe, bounded SELECT query
  runtime, transactional DML batch commit/dry-run, cancellation/error surfacing,
  tabular result envelopes, and catalog/workbench metadata browse are active.
  Catalog metadata covers the current service/database, schemas, tables, views,
  columns, indexes, constraints/FKs, and routines/packages through the shared
  RDB model, with safe-empty behavior for metadata-denied dictionary reads.
  Bounded static SQL editor autocomplete vocabulary is active as editor
  assistance only. Bounded static parser/Safe Mode classifies common
  SELECT/DML/DDL slices and blocks PL/SQL package/admin paths. SID, TNS alias,
  wallet/TLS, sequence/synonym workbench panels, table-data browse, edit,
  structured DDL, runtime fixture/live evidence, desktop E2E, and full PL/SQL
  executable semantics remain deferred.
...
| MSSQL | bounded SQL Server auth/query/catalog/edit/DDL metadata runtime | bounded static T-SQL parser/Safe Mode metadata; no runtime safety smoke claim | catalog-aware T-SQL editor assistance | `mssql` profile/dialect identity exists with Lifecycle, RelationalCatalog, RelationalQuery, and RelationalSchemaMutation backend capability plus primary-key-scoped row-edit support. Connection UI/runtime test covers SQL authentication, TCP/TDS login, timeout-bounded `SERVERPROPERTY('ProductVersion')` probe, and live testcontainer evidence. Query runtime supports bounded SELECT tabular envelopes plus DML/DDL execution, transactional DML batches, cancellation, and server error surfacing. Catalog/workbench metadata covers databases, schemas, tables, views, procedures/functions, columns, indexes, constraints, and FK references consumed by SchemaTree/DataGrid contracts. Row edit uses bracket-quoted T-SQL generated from primary-key projection; no-PK tables can be browsed but edit controls stay disabled, and write batches use preview/discard/commit plus transactional rollback on dry-run/failure. Structured table/index/constraint DDL preview/execute is active for bounded adapter plans with bracket identifiers and transactional execution where a live connection is open. Editor completion uses the live cached catalog for database/schema/table/view/procedure/column suggestions where metadata is loaded, plus curated T-SQL keywords/functions and bracket identifier quoting as editor assistance only. Static parser/Safe Mode covers bounded `SELECT TOP`, bracket identifiers, `N'...'` strings, common DML, bounded DDL, and warn/danger metadata for unsupported SQL Server scripting/admin boundaries. Runtime Safe Mode/destructive preview evidence, runtime fixture smoke, desktop E2E smoke, TLS-required workflow, SQLCMD/admin/security/backup/jobs/users/roles support, broader SQL Server instance/auth behavior, and full T-SQL semantic completion remain follow-up |
| Oracle | bounded service-name connection/query/catalog metadata runtime | bounded static Oracle SQL parser/Safe Mode boundary; no runtime safety claim | bounded static SQL editor vocabulary | `oracle` profile/dialect identity exists with Lifecycle, RelationalCatalog, and RelationalQuery backend capability. Connection UI/runtime test covers Host/Port/User/Password/Service name through the backend Oracle lifecycle adapter. Query runtime supports bounded SELECT tabular envelopes, transactional DML batch commit/dry-run, cancellation, and server error surfacing. Catalog/workbench metadata covers current service/database, schemas, tables, views, columns, indexes, constraints/FKs, and routines/packages through the shared RDB model, with safe-empty behavior for metadata-denied dictionary reads. Static parser/Safe Mode evidence classifies bounded SELECT/DML/DDL slices and marks PL/SQL package/admin paths unsupported. Static SQL seed contract remains future promotion evidence, and editor autocomplete can suggest selected Oracle SQL keywords, functions, package symbols, sequences, synonyms, and bind placeholders as editor assistance only. SID, TNS alias, wallet/TLS, sequence/synonym workbench panels, table-data browse, edit, structured DDL, runtime fixture/live evidence, desktop E2E, and full PL/SQL executable semantics are not implemented |
...
| MSSQL | `e2e/fixtures/seed.mssql.sql`, `src-tauri/tests/mssql_connection_routing.rs`, `src-tauri/src/db/mssql/ddl_tests.rs` | static SQL seed contract plus live SQL Server test evidence for TCP/TDS login, `SERVERPROPERTY` version probe, bounded SELECT tabular envelope, table data read, DML batch commit/dry-run rollback/failure rollback, server error surfacing, query cancellation, catalog/workbench metadata for databases/schemas/tables/views/procedures/functions/columns/indexes/constraints/FKs, and focused structured table/index/constraint DDL preview evidence |
| Oracle | `e2e/fixtures/seed.oracle.sql` | planned static SQL seed contract only. Service-name connection and bounded query runtime evidence live in focused backend/profile tests, not this fixture, live evidence, or Runtime Happy Path smoke. Bounded static editor autocomplete vocabulary is profile/WASM evidence, not fixture/runtime evidence |
...
이다. Bounded editor completion 은 curated T-SQL vocabulary,
...
editor/safety assistance 로 active 이고, runtime Safe Mode smoke/runtime
fixture smoke/desktop E2E/TLS-required workflow/SQLCMD/admin/security/backup/
jobs/users/roles 는 follow-up 이다.
...
catalog/workbench metadata browse, bounded static parser/Safe Mode metadata 로
승격됐지만, runtime Safe Mode/runtime smoke/admin claim 은 아직 없다. Oracle 은
service-name connection, bounded query runtime, catalog/workbench metadata,
bounded static editor completion, and bounded static parser/Safe Mode 로 승격됐지만
sequence/synonym workbench panels, table-data browse, edit/structured DDL, runtime
smoke/live/E2E claim 은 아직 없다.
