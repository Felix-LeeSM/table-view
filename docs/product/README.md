...
- Oracle: service-name connection UI/runtime probe plus bounded SELECT query
  ...
  and tabular result envelopes are active. Bounded static SQL editor
  autocomplete vocabulary is active as editor assistance only. Bounded static
  parser/Safe Mode classifies common SELECT/DML/DDL slices and blocks PL/SQL
  package/admin paths. SID, TNS alias, wallet/TLS, catalog/edit/structured DDL,
  runtime fixture/live evidence, desktop E2E, and full PL/SQL executable
...
| Oracle | bounded service-name connection/query runtime | bounded static Oracle SQL parser/Safe Mode boundary; no runtime safety claim | bounded static SQL editor vocabulary | `oracle` profile/dialect identity exists with Lifecycle and RelationalQuery backend capability. Connection UI/runtime test covers Host/Port/User/Password/Service name through the backend Oracle lifecycle adapter. Query runtime supports bounded SELECT tabular envelopes, transactional DML batch commit/dry-run, cancellation, and server error surfacing. Static parser/Safe Mode evidence classifies bounded SELECT/DML/DDL slices and marks PL/SQL package/admin paths unsupported. Static SQL seed contract remains future promotion evidence, and editor autocomplete can suggest selected Oracle SQL keywords, functions, package symbols, sequences, synonyms, and bind placeholders as editor assistance only. SID, TNS alias, wallet/TLS, catalog/edit/structured DDL, runtime fixture/live evidence, desktop E2E, and full PL/SQL executable semantics are not implemented |
